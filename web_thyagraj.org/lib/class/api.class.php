...
class Api
{
    ...
    /* Custom added API methods */
    public static function LoginRedirectFromFundmandali($input) {
    }
    public static function AddUserFromFundmandali($input) {
        $flag = true;
        if(
            ...
            $username    = scrub_in($input["email"]);
            $fullname    = scrub_in($input["username"]);
            $email        = scrub_in($input["email"]);
            $website        = '';
            $pass        = $input["password"];
            $access        = scrub_in($input["access"]);
            /* Attempt to create the user */
            $user_id = User::create($username, $fullname, $email, $website, $pass, $access);
            if (!$user_id) {
                $flag = false;
            }
        }
        ob_end_clean();
        echo XML_Data::AddUserFromFundmandali($flag);
    }
} // API class
