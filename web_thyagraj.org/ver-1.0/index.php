...
<tr>
	...
	<form action="index.php" id="searchform" autocomplete="off">
		...
		<input type="text" class="form-control" name="undefined" id="txt" maxlength="255" class="autosugest" onkeydown="searchformKeyStroke(event)" onkeyup="ajaxRequest(baseUrl + <?php echo (NJB_DEFAULT_CHARSET == 'UTF-8') ? 'encodeURIComponent' : 'escape'; ?>(this.value),evaluateSuggest)">
	</form>
	</td>
	<td class="space"></td>
</tr>
...
function view2() {
	...
	elseif ($year) {
		...
		elseif ($filter == 'start') {
			...
			while ($genre = mysqli_fetch_assoc($query)) {
				$nav['name'][]	= $genre['genre'];
				$nav['class'][]	= (mysqli_num_rows($query) == 1) ? 'nav' : 'suggest';
				$nav['url'][]	= 'index.php?action=view1&amp;genre_id=' . $genre['genre_id'];
			}
			$nav['class'][]	= 'nav';
			$nav['name'][]	= $artist;
