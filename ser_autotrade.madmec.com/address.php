...
$parameters = array(
    ...
    "txt" => isset($_POST["addata"]["txt"]) ? $_POST["addata"]["txt"] : false
);
unset($_POST);
...
function getState($parameters) {
    ...
    $res = executeQuery("SELECT `asciiname`,`admin1_code`,`latitude`,`longitude`,`timezone` FROM `first_order_administrative_division`
                        WHERE `country_code` = '" . mysql_real_escape_string($_SESSION['address']['countryCode']) . "' AND `asciiname` LIKE '%" . $parameters["txt"] . "%';");
    if (mysql_num_rows($res) > 0) {
        $i = 1;
        while ($row = mysql_fetch_array($res)) {
...
function getDistrict($parameters) {
    $jsondistrict = array();
    $districts = array();
    $res = executeQuery("SELECT `asciiname`,`admin2_code`,`latitude`,`longitude`,`timezone` FROM `second_order_administrative_division`
			...
			AND `asciiname`  LIKE '%" . $parameters["txt"] . "%'
...
function getCity($parameters) {
    ...
    $cities = array();
    $j = 1;
    /*DISTINCT(UNIN.`asciiname`) AS asciiname,*/
    $res0 = executeQuery("SELECT UNIN.`asciiname` AS asciiname,
                        UNIN.`admin3_code`,
                        UNIN.`timezone`,
                        UNIN.`latitude`,
                        UNIN.`longitude`
        ...
        ) AS UNIN  WHERE `asciiname` LIKE '%" . $parameters["txt"] . "%' ORDER BY `asciiname` ASC;");
...
function getLocality($parameters) {
    $jsonstreet = array();
    $streets = array();
    $j = 1;
    /*DISTINCT(UNIN.`asciiname`) AS asciiname,*/
