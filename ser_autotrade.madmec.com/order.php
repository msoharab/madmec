...
class order {
    ...
    public function addClient() {
        ...
        $query = 'INSERT INTO  `order_follows` (
						...
						\'' . mysql_real_escape_string($this->parameters["handeledBy"]) . '\',
						\'' . mysql_real_escape_string($this->parameters["ReferedBy"]) . '\',
    ...
    public function listorders() {
        ...
        $query = 'SELECT * FROM `order_follows` WHERE `status`=14';
        ...
        if ($num_posts > 0) {
            ...
            for ($i = 0; $i < $num_posts; $i++) {
                ...
                $html .= '<tr>
									...
									<td class="text-right">' . $orders[$i]["date"] . '</td>
									<td class="text-right">' . $orders[$i]["comments"] . '</td>
									<td><button class="btn btn-danger btn-md" id="usr_but_trash_' . ($i + 1) . '" data-toggle="modal" data-target="#myUSRDELModal_' . ($i + 1) . '" ><i class="fa fa-trash fa-fw "></i> Delete</button>&nbsp;</td>
								</tr>';
                $html.='<div class="modal fade" id="myUSRDELModal_' . ($i + 1) . '" tabindex="-1" role="dialog" aria-labelledby="myUSRDELModalLabel_' . ($i + 1) . '" aria-hidden="true" style="display: none;">
						<div class="modal-dialog">
							<div class="modal-content" style="color:#000;">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
										<h4 class="modal-title" id="myUSRDELModalLabel_' . ($i + 1) . '">Select Cell Numbers to send SMS</h4>
								</div>
								<div class="modal-body" id="myUSRDEL_' . ($i + 1) . '">
									Do you really want to delete {' . $orders[$i]["client_name"] . '}<br />
									Press OK to delete ??
								</div>
							</div>
							<div class="modal-footer">
								<button type="button" class="btn btn-danger" data-dismiss="modal" name=".modal-backdrop" id="deleteUSRDELOk_' . ($i + 1) . '">Ok</button>
								<button type="button" class="btn btn-success" data-dismiss="modal" id="deleteUSRDELCancel_' . ($i + 1) . '">Cancel</button>
							</div>
						</div>
					</div>';
                ...
                $listusers[] = array(
                    ...
                    "uid" => $orders[$i]["id"],
                    "sr" => '#usr_row' . ($i + 1),
                    "alertUSRDEL" => '#myUSRDELModal_' . ($i + 1),
                    "usrdelOk" => '#deleteUSRDELOk_' . ($i + 1),
                    "usrdelCancel" => '#deleteUSRDELCancel_' . ($i + 1),
                );
            }
        }
        return $listusers;
    ...
    public static function listnotification($para = false) {
        ...
        if ($total) {
            for ($i = 0; $i < $total; $i++) {
                ...
                $users[$i]["email"] = explode("☻☻♥♥☻☻", $users[$i]["email_ids"]);
    ...
    public function displayNotificationList($para = false) {
        ...
        if ($num_posts > 0) {
            ...
            for ($i = $this->parameters["initial"]; $i < $this->parameters["final"] && $i < $num_posts && isset($users[$i]['usrid']); $i++) {
                ...
                $basicinfo = '<div class="row"><div class="col-lg-12">&nbsp;</div><div class="col-lg-4"><div class="panel panel-yellow"><div class="panel-heading"><h4>Photo</h4></div><div class="panel-body" id="usrphoto_' . $users[$i]["usrid"] . '"><img src="' . $users[$i]["photo"] . '" width="150" /></div><div class="panel-footer" style="display:none;"><button class="btn btn-yellow btn-md" id="usrphoto_but_edit_' . $users[$i]["usrid"] . '"><i class="fa fa-edit fa-fw "></i> Edit</button></div></div></div>
    ...
    public function fetchAdminNotify() {
        ...
        if (mysql_num_rows($result)) {
            $m = 1;
            $data .='<table class="table table-striped table-bordered table-hover" id="listnotifycations-example"><thead><tr><th>#</th><th>UserName</th><th>Owner Name</th><th>Mobile</th><th>Email</th><th>City</th><th>Zipcode</th><th>Expire on</th></tr></thead><tbody>';
    ...
    public function deleteOrder() {
        ...
        executeQuery("SET AUTOCOMMIT=0;");
        executeQuery("START TRANSACTION;");
        $query = 'UPDATE  `order_follows` SET `status`=6 WHERE `id` = "' . mysql_real_escape_string($this->parameters["entry"]) . '";';
        if (executeQuery($query)) {
            $flag = true;
            executeQuery("COMMIT;");
            $obj = new alerts();
