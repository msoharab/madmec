function DrawingCtrl(){
					...
					switch (data) {
						...
						default:
							...
							if(listofrequi != null && listofrequi.length > 0){
								...
								$('#'+requi.Qrequi_type).change({para:PCC.add,
									requi:listofrequi,
		...
		function uploadDRAW(){
			...
			if (Requi.add.ethid > 0) {
				...
				$('html, body').animate({scrollTop: Number($('#'+Requi.add.ethno_msg).offset().top) - 95}, 'slow');
