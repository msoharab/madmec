<?php
	...
	function main(){
		if(ValidateAdmin()){
			header('Location:'.URL.'php/receipts.php');
			exit(0);
		}
		...
		if(get_resource_type($link) == 'mysql link'){
			if(($db_select = selectDB(DBNAME_ZERO,$link)) == 1){
				$query = 'SELECT *
						FROM `admin` 
						WHERE 
						`user_name`=\''.mysql_real_escape_string($u).'\' 
						AND 
						`password`=\''.mysql_real_escape_string($p).'\';';
				...
				if(get_resource_type($res) == 'mysql result'){
					if(mysql_num_rows($res) > 0){
						...
						if( $row['user_name'] == $u && $row['password'] == $p ){
							...
							$_SESSION['ADMIN_name'] =  ($row['name'])? $row['name']: "Administrator";
							update_admin_log();
							$flag = 'success';
						}
		...
		<div   style="height:auto; width:400px;position:relative; top:100px; padding:50px; background:#FFF;">
			...
			<div id="reset_src">
				<form>
					...
					<br />
					<button class="btn btn-lg btn-danger btn-block" type="button" onclick="javascript:reset_now();">Reset Now</button>
				</form>
			</div>
		</div>
	</div>
