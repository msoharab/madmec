		...
		if(!$flag){
			...
			if(get_resource_type($link) == 'mysql link'){
				if(($db_select = selectDB(DBNAME_ZERO,$link)) == 1){
					...
					c.`tar_name`
					FROM 
					`receipt` AS a RIGHT JOIN `income` AS b ON a.`income_id` = b.`id`
					JOIN `target` AS c on b.`target_id` = c.`id`
					WHERE ((b.`date` LIKE '".$from."%') 
					OR (b.`date` BETWEEN '".$from."%' AND '".$to."%') 
					OR (b.`date` LIKE '".$to."%'))
					AND (b.`status`='show')
					ORDER BY `id` ASC";
	...
	function ReportForm(){
		...
		echo '<form role="form">
				<center class="form-group">
					...
					<span class="text-danger" id="err_from">Invalid</span>
					<br /><br />
					<button  onclick="javasript:gen_report();" type="button" class="btn btn-danger">Get Report</button>
				</center>
			</form>';
	}		
	main();
					...
					<div id="screen1" class="panel-body">
						<div class="row" style="color:WHITE !important;" >
							<div align="middle" class="col-md-12 report_menu">
								<div class="bg-red">
									<a id="rec_nav" href="javascript:void(0);" onclick="javascript:report_form();"><i class="fa fa-file fa-5x"></i><h2>Generate Balance Report</h2> </a>
								</div>
							</div>
