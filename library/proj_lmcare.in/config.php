<?php
	...
	define("MODULE_ZEND_2","Zend/Mail/Transport/Smtp.php");
        ...
        define("PATIENTHISTORY",DOC_ROOT.'ajax/download/');
	define("INC","inc/");
	define("ADMIN","admin/");
	define("USER","user/");
	...
	define("GYM_LOGO",URL.ASSET_IMG."short-logo.jpg");
        define("USERLOGO",URL.ASSET_IMG."icon-user-default.png");
	/* XLS FILE CONSTANT */
	...
	function generateReciept($receipt){
		...
		if($receipt["don_type"] == 'cash')
			$mop_des = 'Donation received by the mode of: CASH.';
		else if($receipt["don_type"] == 'dd')
			$mop_des = 'Donation received by the mode of: Demand Draft,<br /> numbered '.$receipt["number"].' drawn on '.$receipt["branch_of"].' Branch of '.$receipt["bank_name"].' Bank.';
		else if($receipt["don_type"] == 'cheque')
			$mop_des = 'Donation received by the mode of: CHEQUE,<br /> numbered '.$receipt["number"].' drawn on '.$receipt["branch_of"].' Branch of '.$receipt["bank_name"].' Bank.';
		else if($receipt["don_type"] == 'transfer')
			$mop_des = 'Donation received by the mode of: '.$receipt["tran_mode"].'<br />'.$receipt["number"].', Transaction number '.$receipt["number"].' draw
		...
		<body><div id="content_div" style="background-size: 100% 160px !important;font-weight:bold !important; font-size : 18px !important;padding:20px 50px;font-family: Arial, Helvetica, sans-serif !important;font-size : 20px;height:auto; width:800px; background:url(\'http://srm.madmec.com/assets/img/receipt_bg.png\') no-repeat  scroll 0% 0% / 100% 160px transparent;">
        ...
        <tr>
        <td valign="top" colspan="2">
			<center style="text-decoration:underline;color:#4169E1 !important; font-weight:bold;">DETAILS OF THE TRUST</center>
			<div class="span3">
			<p>
				PAN OF TRUST: AA0TS2166D
			</p>
			<p>
				80G NUMBER OF TRUST: DIT(E)BLR/80G/L-69/AAOTS2166D/ITO(E)-3/Vol 2014-2015
			</p>
			<p>
				12(a)A NUMBER OF TRUST: DIT(e)BLR/12AA/L-68/AAOTS2166D/ITO(E)-3/Vol 2014-2015
			</p>
			</div>
