...
require_once 'business-ids.php';
require_once 'index-ids.php';
require_once 'profile-ids.php';
require_once 'deal-ids.php';
class configure {

    private $temp, $doc_path, $libroot, $business_ids, $index_ids, $profile_ids, $deal_ids;
    ...
    public function __construct() {
        ...
        $this->config = array(
            //"URL" => "http://local.nookleads.com/",
            //"URL" => "http://nookleads.localmm.com/",
            "URL" => "http://www.nookleads.com/",
            "DBHOST" => "localhost",
            "DBUSER" => "root",
            //"DBPASS" => "9743967575",
            "DBPASS" => "madmec@418133",
            "DBNAME_ZERO" => "nookleads_india",
            ...
            "GMAIL_PORT" => "587",
            "GMAIL_USER" => "nookleads@gmail.com",
            ...
            "PLG_28" => "facebook/",
            "PLG_29" => "googleplus/",
            "ASSSET_PIC" => "assets/js/nookleads/",
        ...
        $this->config["DEFAULT_POST_IMG"] = $this->config["URL"] .
                $this->config["VIEWS"] .
                $this->config["ASSSET_IMG"] . "deal/default-thumbnail.jpg";
        ...
        $this->config["DEFAULT_CHANEL_BACK_IMG"] = $this->config["URL"] .
                $this->config["VIEWS"] .
                $this->config["ASSSET_IMG"] . "businesss/business_cover.jpg";
        $this->config["DEFAULT_CHANEL_ICON_IMG"] = $this->config["URL"] .
                $this->config["VIEWS"] .
                $this->config["ASSSET_IMG"] . "businesss/business_logo.jpg";
        $this->config["DEFAULT_CHANEL_ADV_IMG"] = $this->config["URL"] .
                $this->config["VIEWS"] .
                $this->config["ASSSET_IMG"] . "businesss/add.png";
        /* User */
        ...
        $this->business_ids = new business_ids($this->config);
        $this->index_ids = new index_ids($this->config);
        $this->profile_ids = new profile_ids($this->config);
        $this->deal_ids = new deal_ids($this->config);

        $this->idHolders = array(
            "nookleads" => array(
                "profile" => $this->profile_ids->getIds(),
                "business" => $this->business_ids->getIds(),
                "deal" => $this->deal_ids->getIds(),
    ...
    public function createdirectories($directory) {
        ...
        $sruct_array = array(
            $this->config["DOC_ROOT"] . $this->config["DIRS"] . "$curr_dir/$directory/quotations/",
            $this->config["DOC_ROOT"] . $this->config["DIRS"] . "$curr_dir/$directory/wos/",
            $this->config["DOC_ROOT"] . $this->config["DIRS"] . "$curr_dir/$directory/business/",
            $this->config["DOC_ROOT"] . $this->config["DIRS"] . "$curr_dir/$directory/leads/",
