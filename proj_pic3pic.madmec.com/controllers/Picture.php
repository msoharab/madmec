...
class Picture extends BaseController {
    ...
    private $para, $UserId;
    private $PictureMod, $absolutepath, $relativepath, $ChannelMod;
    ...
    public function UploadPic($target = false) {
        ...
        if (isset($this->postFile['file'])) {
            ...
            if (empty($errors) == true) {
                $path1 = '';
                $file = '';
                if ($target === 'channel' && isset($_SESSION["CHANNEL_ID"])) {
                    $chdir = $this->ChannelMod->getChannelDirectory($_SESSION["CHANNEL_ID"]);
