...
class NewPost_Model extends BaseModel {
    ...
    public function getSectionsNames($listtype = false) {
        ...
        $stm = $this->db->prepare('SELECT * FROM `sections_walll_post` WHERE `status_id` = :stat ORDER BY `section_name`');
        $res = $stm->execute(array(
            ":stat" => 4
        ));
        $res = $stm->execute();
    ...
    public function getUserPost($param = false) {
        ...
        $stm = $this->db->prepare('SELECT `id` FROM `post` WHERE `status_id` = :stat AND  `user_id` = :uid ORDER BY `id` DESC;');
        $res = $stm->execute(array(
            ":stat" => 4,
        ...
        $stm = $this->db->prepare('/*Post Start*/
			...
			(SELECT TRIM(`photo`.`ver5`) FROM `photo` WHERE `photo`.`id` = t1.`photo_id`)
                ...
                LEFT JOIN (
                        ...
                        LEFT JOIN `sections_walll_post`	AS t2 ON t2.`id` = t1.`section_id`
                        WHERE t1.`status_id` = 4
                        AND t2.`status_id` = 4
                        GROUP BY(t1.`post_id`)
                ...
                WHERE t1.`status_id` = 4
                AND t1.`id` NOT IN (SELECT `post_id` FROM `post_preferences` WHERE `status_id` = 4 AND (`preferences_id` IN (5,6)))
                AND (
