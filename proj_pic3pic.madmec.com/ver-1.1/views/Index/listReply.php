<?php
$listCommentReply = $this->idHolders["pic3pic"]["index"]["list"]["comment"]["list"]["reply"]["list"];
...
if (isset($newRes[$i]["posts"]["comments"]["replys"]["pcr_ct"]) && isset($newRes[$i]["posts"]["comments"]["replys"]["pcr_ct"][$lpcr]) && $newRes[$i]["posts"]["comments"]["replys"]["pcr_ct"][$lpcr] != '')
    $this->commentNoReplies = $newRes[$i]["posts"]["comments"]["replys"]["pcr_ct"][$lpcr];
if ($this->commentNoReplies > 0) :
    for ($lpcri = 0; isset($newRes[$i]["posts"]["comments"]["replys"]["pcr_id"][$lpcr]) &&
        ...
        if (isset($newRes[$i]["posts"]["comments"]["replys"]["lk_rep_ct"][$lpcr]) &&
                count($newRes[$i]["posts"]["comments"]["replys"]["lk_rep_ct"][$lpcr]) > 0 
                && isset($newRes[$i]["posts"]["comments"]["replys"]["lk_rep_ct"][$lpcr][$lpcri]) 
                && $newRes[$i]["posts"]["comments"]["replys"]["lk_rep_ct"][$lpcr][$lpcri] != '')
                ...
                <ul class="list-inline">
                    <li class="pull-right">
                        <a href="javascript:void(0);" 
                           ...
                           id="<?php echo $listCommentReply["like"]["counter"] . $newRes[$i]["posts"]["comments"]["replys"]["pcr_id"][$lpcr][$lpcri]; ?>">
                            Likes (<?php echo $commentReplyNoLikes; ?>)
                        </a> 
                    </li>
                </ul>
            </div>
    ...
    ?>
    <div class="col-lg-12">
        <h5>No replies to view folks!!! <span class="<?php echo $listCommentReply["content"]; ?>">:-O</span></h5>
    </div>
<?php
endif;
?>
