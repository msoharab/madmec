...
//						$(colls.outputDiv).html(INET_ERROR);
        ...
        function editappointmentdetails(apptid,fromtime,totime,location,frequency)
        {
         var editslott  ='<div class="form-group">'+
                    ...
                    '<input type="text" name="editfrequencyappoint" id="editfrequencyappoint" value="'+frequency+'" class="form-control"/>'+
                    '</div></div>'+
                            ...
                            console.log(data);
                                ...
                                switch (data) {
                                        ...
                                        default:
                                                ...
                                                $(doctor.appointment.appointform).get(0).reset();
                                                break;
                                }
                        },
                        error: function() {
...
//                    $(doctor.patientassesment.patientprevioushistory).val(ppdetails.previoushistory);
                    ...
                    $(doctor.patientassesment.patientpic).html('<img src="'+URL+ppdetails.photo+'" width="100" height="100"></img>');
                    break;
			}
                    },
                error: function() {
					...
					success: function(data, textStatus, xhr) {
						...
						switch (data) {
							...
							case 'login':
								...
								break;
							default:
                                                                alert("Prescribtion has been Added successfully");
         ...
         if (doctor.appointment.num == -1) $(doctor.appointment.multiple_slots).html('');
           ...
           var htm='<div id="'+doctor.appointment.form +doctor.appointment.num+'">'+
                   '<div class="form-group">'+
                    '<div class="col-md-12"><label> Slot '+doctor.appointment.j+'</label>'+
                    ...
                    '<div class="col-md-3">'+
                    '<label>no of Patient</label>'+
                    '<input type="text" name="frequencyappoint'+doctor.appointment.num+'" id="frequencyappoint'+doctor.appointment.num+'" placeholder="no of Patient" class="form-control"/>'+
                    '</div></div>'+
                   '</div>';
...
//								$(doctor.pharmacyregistration.pharmacymsgDiv).html('Pharmacy has been successfully Added');
								$('html, body').animate({
...
//								$(doctor.diagnosticregistration.diagnosticmsgDiv).html('Diagnostic has been successfully Added');
								$('html, body').animate({
...
//						$(colls.outputDiv).html(INET_ERROR);
        ...
        function addpatientdetails()
        {
            ...
            if($(doctor.patientregistration.patientage).val()== "" || Number($(doctor.patientregistration.patientage).val())==0)
                {
                flag=false;
...
//								$(doctor.patientregistration.patientmsgDiv).html('patient has been successfully Added');
								$('html, body').animate({
