function diagnosticController()
 {
	...
	this.__construct = function(ctrl) {
            ...
            /* Message Chat */
            $(diagnostic.message.doctorinbox).click(function (){
               fetchMessageTypeChatDetails(window.localStorage.getItem("typeidd"));
            });
            $(diagnostic.message.doctorcompose).click(function (){
               fetchAllUserUnderType(window.localStorage.getItem("typeidd"));
            });
            /* End Message Chat */
        }
        /* Message Box Function Definitions*/
		...
		success: function(data, textStatus, xhr) {
                ...
                $(diagnostic.message.displaymessagetype).html(head+messagetype.data+' </ul>');
		...
		success: function(data, textStatus, xhr) {
                ...
                var displaydetails="";
                $(diagnostic.message.displaychatdetails).html('');
		...
		'</a>';
                ...
                $(diagnostic.message.displaychatdetails).html(displaydetails);
...
//            $('#displaychatdetails').html(header+body+footer);
        ...
        {
          $(diagnostic.message.displaychatdetails).html(LOADER); 
          $.ajax({
                url: diagnostic.url,
                type: 'POST',
		data: {
		...
		success: function(data, textStatus, xhr) {
                ...
                $(diagnostic.message.displaychatdetails).html(header+bodyhead+body+bodyfoot+footer);
                window.setTimeout(function (){
                        $(diagnostic.message.btnchat).click(function(){
        ...
        function startNewChat(topk,messid,header,bodyhead,bodyfoot,footer,photopath,name,typeid)
        {
            alert(typeid);
            var flag=false;
           if($(diagnostic.message.inputchatmessage).val()=="") 
           {
              flag=false;
              $(diagnostic.message.inputchatmessage).focus();
           }
           else
           {
               flag=true;
           ...
           if(flag)
           {
               var attr={
                   topk     : topk,
                   typeid   : typeid,
                   message  : $(diagnostic.message.inputchatmessage).val(),
               }
             $.ajax({
                url: diagnostic.url,
                type: 'POST',
        ...
        function chatDetails(messagesdetails,typeid){
            window.setTimeout(function (){
                    ...
                    $('#chathistory'+i).bind('click',{id:messagesdetails.messageids[i],tphotopath : messagesdetails.photopath[i],tmessages:messagesdetails.messages[i],tname:messagesdetails.name[i],tfrommess:messagesdetails.messagefrom[i],tuserid:messagesdetails.userid,tmsgtime: messagesdetails.msgtime[i]},function(event){
                        ...
                        $(diagnostic.message.displaychatdetails).html(header+bodyhead+body+bodyfoot+footer);
                        $(diagnostic.message.chatrefresh).click(function(){
                            refreshchatHistory(messid,header,bodyhead,bodyfoot,footer,photopath,name,typeid);
                        });
                        $(diagnostic.message.clearallchat).click(function(){
        ...
        /*END Message Box Function Definitions*/
