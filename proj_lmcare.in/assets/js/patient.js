function patientcontroller()
{
	...
	this.__construct = function(ctrl) {
            ...
            /* Message Chat */
            $(patient.message.doctorinbox).click(function (){
               fetchMessageTypeChatDetails(window.localStorage.getItem("typeidd"));
            });
            $(patient.message.doctorcompose).click(function (){
		...
		success: function(data, textStatus, xhr) {
                ...
                $(patient.message.displaymessagetype).html(head+messagetype.data+' </ul>');
		...
		success: function(data, textStatus, xhr) {
                ...
                var displaydetails="";
                $(patient.message.displaychatdetails).html('');
		...
		'</a>';
                ...
                $(patient.message.displaychatdetails).html(displaydetails);
...
//            $('#displaychatdetails').html(header+body+footer);
        ...
        {
          $(patient.message.displaychatdetails).html(LOADER); 
          $.ajax({
                url: patient.url,
                type: 'POST',
		data: {
		...
		success: function(data, textStatus, xhr) {
                ...
                $(patient.message.displaychatdetails).html(header+bodyhead+body+bodyfoot+footer);
                window.setTimeout(function (){
                        $(patient.message.btnchat).click(function(){
        ...
        function startNewChat(topk,messid,header,bodyhead,bodyfoot,footer,photopath,name,typeid)
        {
            alert(typeid);
            var flag=false;
           if($(patient.message.inputchatmessage).val()=="") 
           {
              flag=false;
              $(patient.message.inputchatmessage).focus();
           }
           else
           {
               flag=true;
           ...
           if(flag)
           {
               var attr={
                   topk     : topk,
                   typeid   : typeid,
                   message  : $(patient.message.inputchatmessage).val(),
               }
             $.ajax({
                url: patient.url,
                type: 'POST',
        ...
        function chatDetails(messagesdetails,typeid){
            window.setTimeout(function (){
                    ...
                    $('#chathistory'+i).bind('click',{id:messagesdetails.messageids[i],tphotopath : messagesdetails.photopath[i],tmessages:messagesdetails.messages[i],tname:messagesdetails.name[i],tfrommess:messagesdetails.messagefrom[i],tuserid:messagesdetails.userid,tmsgtime: messagesdetails.msgtime[i]},function(event){
                        ...
                        $(patient.message.displaychatdetails).html(header+bodyhead+body+bodyfoot+footer);
                        $(patient.message.chatrefresh).click(function(){
                            refreshchatHistory(messid,header,bodyhead,bodyfoot,footer,photopath,name,typeid);
                        });
                        $(patient.message.clearallchat).click(function(){
