...
class doctor {
    protected $parameters = array();
                ...
                {
                    $query="SELECT `username` AS details FROM `user_profie` WHERE (`doctor_id` IS NULL) AND id=".$_SESSION['userdata']['id']." AND user_type_id=1";
...
//                    executeQuery("START TRANSACTION;");
                ...
                $query='UPDATE `passesment` SET `disease`="'.mysql_real_escape_string($this->parameters['disease']).'" '
                         . ' WHERE `patientid`='.mysql_real_escape_string($this->parameters['patientid']).' AND `doctorid`='.mysql_real_escape_string($_SESSION['userdata']['id']); 
                }
                ...
                public function configureappointment() {
                   ...
                   if(is_array($this->parameters["days"]) && sizeof($this->parameters["days"]) > -1){
                                      $num= sizeof($this->parameters['days']);
                                        for($i=0;$i<$num;$i++)
                                        {
                                    $query='UPDATE `appointment` SET `status_id`=5 WHERE `doctorid`='.mysql_real_escape_string($_SESSION['userdata']['id']).' AND `weekofday`='.mysql_real_escape_string($this->parameters['days'][$i]);
                                    executeQuery($query);
                                    $query = 'INSERT INTO  `appointment` (`id`,`doctorid`,`weekofday`,`status_id` ) VALUES(null,'
                                            . ''.mysql_real_escape_string($_SESSION['userdata']['id']).','.mysql_real_escape_string($this->parameters['days'][$i]).',4);';
                                            executeQuery($query);
                                            $lastid=  mysql_insert_id();
                                            $query='';
...
//                                            $query1='';
                ...
                public function fetchpatientdetails()
                {
                    ...
                    $query='SELECT u.`name`,u.`id`,u.`cell_number`,u.`email_id`,ud.`sex`
                            ...
                            WHERE r.`from_pk`='.$_SESSION['userdata']['id'].' AND r.`to_pk`=u.`id`
                            AND u.`id`=ud.`user_pk`
		    ...
		    $divfooter='</div></div>';
                ...
                $pdetails=array();
                    ...
                    $query='SELECT u.`name`,u.`id`,u.`clinicname`,u.`cell_number`,u.`email_id`,ud.`sex`
                            FROM `user_profie` u,`relations` r,`user_details` ud
                            WHERE r.`from_pk`='.  mysql_real_escape_string($_SESSION['userdata']['id']).' AND r.`to_pk`=u.id
		    ...
		    $divfooter='</div>
                ...
                $query='SELECT `previoushistory`,`disease` FROM `passesment` WHERE `patientid`='
                         . ''.mysql_real_escape_string($pid).' AND `doctorid`='.mysql_real_escape_string($_SESSION['userdata']['id']).' AND `status_id`=4';
                $res=  executeQuery($query);
			...
			CASE WHEN (presdescb.`tablet_name` IS NULL OR presdescb.`tablet_name` = "" ) THEN "-" ELSE GROUP_CONCAT(presdescb.`tablet_name`)  end AS tabletname,
                        ...
                        WHERE padescb.`patient_id`='
                         . ''.mysql_real_escape_string($pid).' AND padescb.`doctorid`='.mysql_real_escape_string($_SESSION['userdata']['id']).' GROUP BY presdescb.`presciption_id`
		    ...
		    $divfooter='</div>
                ...
                public function fetchconfiguredappointment() {
                    ...
                    $query='SELECT ap.`weekofday` FROM `appointment` ap
                            ...
                            WHERE ap.`doctorid`='.$_SESSION['userdata']['id'].' AND ap.`status_id`=4
                            AND apd.`status_id`=4
                    ...
                    $data .='<p class="help-block"> Select all days which have same schedule slots and to configure it once</p>'
                               . '<div class="form-group">
                                    ...
                                    <button  id="plus_slots_" type="button" class="text-primary btn btn-success  btn-md"><i class="fa fa-plus fa-fw fa-x2"></i> </button>
		...
		$config = array('auth' => 'login',
                ...
                public function fetchmessagetypeschatdel($typeid) {
                 ...
                 $query="SELECT mes.`id` AS messageid,
                        ...
                        LEFT JOIN `user_profie` up
                        ON (CASE WHEN mes.`from_message` = '".$_SESSION['userdata']['id']."' THEN up.`id`=mes.`to_message`
                        ELSE
                        ...
                        AND (mes.from_message='".$_SESSION['userdata']['id']."' OR mes.to_message='".$_SESSION['userdata']['id']."')
...
//                 echo $query;
                ...
                public function sendChatMessage($messid,$message) {
                  $query='INSERT INTO `message_details`(`id`,`mess_id`,`message`,`mess_from`,`msgtime`,`status_id`)'
                          ...
                          . '"'.mysql_real_escape_string($message).'","'.$_SESSION['userdata']['id'].'",CURRENT_TIMESTAMP,4)';  
                  $result=  executeQuery($query);
                  return $result;
                }
                ...
                $query='SELECT r.`to_pk`,up.`name`,up.`clinicname`,up.`cell_number`,up.`photo`
                        ...
                        LEFT JOIN `user_profie` up
                        ON up.`id`=r.`to_pk`
                        WHERE r.`from_pk`='.$_SESSION['userdata']['id'].' AND r.`to_pk`=up.`id` AND up.`status_id`=4 AND up.`user_type_id`='.$userid;
                }
                else
                {
                 $query='SELECT r.`from_pk` as to_pk,up.`name`,up.`clinicname`,up.`cell_number`,up.`photo`
                        ...
                        LEFT JOIN `user_profie` up
                        ON up.`id`=r.`from_pk`
                        WHERE r.`to_pk`='.$_SESSION['userdata']['id'].' AND  r.`from_pk`=up.`id` AND up.`status_id`=4 AND up.`user_type_id`='.$userid;
                }
                $result=  executeQuery($query);
                if(mysql_num_rows($result))
                {
                ...
                public function checkForChatHistory($topk,$typeid) {
                  ...
                  $query="SELECT mes.`id`,up.name,up.photo
                        ...
                        LEFT JOIN `photo` p
                        ON p.`user_pk`=up.`id`
                        WHERE (`from_message`=".  mysql_real_escape_string($_SESSION['userdata']['id'])." AND `to_message`=".  mysql_real_escape_string($topk).") 
                        OR (`from_message`=".  mysql_real_escape_string($topk)." AND `to_message`=".  mysql_real_escape_string($_SESSION['userdata']['id']).") ;";
                  $result= executeQuery($query);
                  if(mysql_num_rows($result))
                  {
                      $row=  mysql_fetch_assoc($result);
                ...
                if( executeQuery($query))
                {
                 ...
                 $query='INSERT INTO `message_details`(`id`,`mess_id`,`message`,`mess_from`,`msgtime`,`status_id`)VALUES('
                         ...
                         . '"'.  mysql_real_escape_string($_SESSION['userdata']['id']).'",CURRENT_TIMESTAMP,'
                         . '4);'  ;
                 executeQuery($query);
                 return $lastid;
                }
                ...
                $query='SELECT ap.`id`,ap.`weekofday`,
                       ...
                       `appointment_descb` apd
                        ON ap.`id`=apd.`appoint_id`
                        WHERE ap.`status_id`=4 AND apd.`status_id`=4 AND ap.`doctorid`='.$_SESSION['userdata']['id'].'
                        GROUP BY (apd.`appoint_id`)
                        ORDER BY (ap.`weekofday`)';
                $result=  executeQuery($query);
                    if(mysql_num_rows($result)>0){
                    ...
                    if($num)
                    {
                          for($i=0;$i<$num;$i++)
                          {
                                ...
                                $pdata[$i] .='<tr><td>'.$j++.'
                                         ...
                                         - '.$totimee[$k].'<br/> Location : '.$location[$k].'   
                                        <br/> no of slots : '.$frequency[$k].'<br/> Filled : '.$filled[$k].'</td>'
                                        . '<td><button type="button" class="btn btn-success  btn-md" id="viewappointee_'.$grpappid[$k].'"><i class="fa fa-file fa-fw fa-x2"></i>View</button></td></tr><tr><td colspan="3"><div id="viewappointeedis_'.$grpappid[$k].'"></div></td></tr>';
                                        $appids[]=$grpappid[$k];
                                        $dates[]=$grpdates[$k];
                                ...
                                $pdata1[$i] .='<tr id="row_id'.$grpappid[$k].'"><td>'.$p++.'</td>
                                        <td> Time : '.$fromtimee[$k].' - 
                                        '.$totimee[$k].'<br/> Location : '.$location[$k].'<br/>   
                                         no of slots : '.$frequency[$k].'</td>'
                                        . '<td><button type="button" class="btn btn-warning  btn-md" id="edit_app'.$grpappid[$k].'"><i class="fa fa-edit fa-fw fa-x2"></i>Edit</button><br/><br/>'
                                        . '<button class="btn btn-danger  btn-md" name="'.$grpappid[$k].'" id="delete_apptt_'.$grpappid[$k].'" data-toggle="modal" data-target="#myModal_'.$grpappid[$k].'"><i class="fa fa-trash fa-fw fa-x2"></i> Delete</button>&nbsp;
                                ...
                                $pdata1[$i] .='<tr><td colspan="3"><button  id="eplus_slots_'.$pdetails[$i]['weekofday'].'" type="button" class="text-primary btn btn-success  btn-md"><i class="fa fa-plus fa-fw fa-x2"></i> </button>
                ...
                public function fetchpatientassesmentdetails($pid) {
                   ...
                   $photo=$row12['photo'];
                   $query='SELECT * FROM `passesment` WHERE `patientid`='.$pid.' AND `doctorid`='.$_SESSION['userdata']['id'].' AND `status_id`=4; ';
                    $res=  executeQuery($query);
