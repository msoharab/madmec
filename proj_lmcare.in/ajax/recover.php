		...
		if( isset($_GET['r']) && isset($_GET['e']) ){
			...
			if(get_resource_type($link) == 'mysql link'){
				if(($db_select = selectDB(DBNAME_ZERO,$link)) == 1){
					$res0 =  mysql_fetch_assoc( executeQuery("SELECT EXISTS(SELECT * FROM `user_profie` WHERE `resetreq` = '".mysql_real_escape_string($_GET['r'])."' ) AS test;") );
					if( $res0['test'] == 1){
						$_SESSION['r'] = $_GET['r'];
						$_SESSION['e'] = $_GET['e'];
					}
	...
	function ResetPass(){
		...
		if(get_resource_type($link) == 'mysql link'){
			if(($db_select = selectDB(DBNAME_ZERO,$link)) == 1){
				...
				`resetreq` = NULL,
				`password` = '".mysql_real_escape_string($pass1)."'
				WHERE 
				...
				if($res){
					echo '<h1>successfully changed your password <br />
						<a href="'.URL.'">Login Now</a></h1>';
				}
				else{
...
<html lang="en">
  <head>
    ...
    <link href="<?php echo URL; ?>signin.css" rel="stylesheet">
    <!-- CUSTOM STYLES-->
	...
	<div class="navbar navbar-inverse navbar-fixed-top">
		<div class="container">
			<div class="navbar-header">
                            <a class="navbar-brand" href="#">LMcare</a>
			</div>
		</div>
	</div>
    <div class="container">
