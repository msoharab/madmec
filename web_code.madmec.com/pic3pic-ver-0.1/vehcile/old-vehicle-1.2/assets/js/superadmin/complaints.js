function complaints()
{
    ...
    function addVehicleType(formdata)
    {
        ...
        $.ajax({
            type: 'POST',
            url: ctrl.url,
            data: formdata + '&autoloader=true&action=complaints/addVehicleType',
            success: function (data, textStatus, xhr) {
                ...
                switch (data) {
                    ...
                    default:
                        if (res == "exist")
                        {
                            alert("complaint Type was Already Exist");
                        }
                        else
                        if (res)
                        {
                            alert("complaint Type has been Successfully Added");
                            $(ctrl.add.form).get(0).reset();
                        }
                        else
                        {
                            alert("complaint Type hasn't been Added");
                            $(ctrl.add.form).get(0).reset();
                        }
                        break;
                }
    ...
    function updateVehicleType(formdata)
    {
        ...
        $.ajax({
            type: 'POST',
            url: ctrl.url,
            data: formdata + '&autoloader=true&action=complaints/updateVehicleType',
            success: function (data, textStatus, xhr) {
                /*                                        console.log(data);*/
                data = $.trim(data);
                var res = $.parseJSON(data)
                switch (data) {
                    ...
                    default:
                        ...
                        {
                            alert("complaint Type has been Successfully Updated");
                            $(ctrl.add.form).get(0).reset();
                            fetchVehicleTypes();
                        }
                        else
                        {
                            alert("complaint Type hasn't been Updated");
                            $(ctrl.add.form).get(0).reset();
                            fetchVehicleTypes();
                        }
                        break;
    ...
    function fetchVehicleTypes()
    {
        $.ajax({
            type: 'POST',
            url: ctrl.url,
            data: '&autoloader=true&action=complaints/fetchVehicleType',
            success: function (data, textStatus, xhr) {
                /*                                        console.log(data);*/
                data = $.trim(data);
                var res = $.parseJSON(data)
                    ...
                    default:
                        ...
                        if (type.status == "success")
                        {
                            var header = '<div class="table-responsive"><table class="table table-responsive table-bordered table-hover" id="list_col_table"><thead>' +
                                    '<tr><th>#</th><th>Vehicle Type</th><th>Option</th></tr></thead><tbody>';
    ...
    function checkEmail(req, id)
    {
        ...
        $.ajax({
            ...
            data: {
                autoloader: true,
                action: 'complaints/checkVehicleType',
                vehicletype: vtype,
                typeid: typeid,
            },
            success: function (data, textStatus, xhr) {
    ...
    function editDetails(id, loc, alldata)
    {
        $(ctrl.list.disvehicletype).html('<form id="editvehicletypeform">' +
                '<div class="col-lg-12">' +
                '   <div class="col-lg-12">' +
                '       <strong><i class="fa fa-star text-danger">&nbsp;</i>Complaint Type : <i class="fa fa-caret-down fa-fw"></i></strong>' +
                '   </div>' +
                '   <div class="col-lg-12">' +
    ...
    function deleteVehicle(id)
    {
        $.ajax({
            ...
            data: {
                autoloader: true,
                action: 'complaints/deleteVehicleType',
                typeid: id
            },
            success: function (data, textStatus, xhr) {
                /*                                        console.log(data);*/
                ...
                switch (data) {
                    ...
                    default:
                        if (data)
                        {
                            alert("complaint Type has been Successfully Deleted");
                            fetchVehicleTypes();
                        }
                        else
                        {
                            alert("complaint Type hasn't been Deleted")
                            fetchVehicleTypes();
                        }
                        break;
                }
