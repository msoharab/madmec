function user()
{
    ...
    this.__construct = function (cctrl) {
        ...
        $(add.form).submit(function (evt) {
            ...
            if (adddonorform && add.emailcheck)
            {
                $(add.but).prop('disabled', 'disabled');
                $.ajax({
                    type: 'POST',
                    url: ctrl.url,
                    data: formdata + '&autoloader=true&action=user/addDonor',
...
//								$(add.st_loc).attr('name', ui.item.value);
    ...
    function fetchListOfIndividualDonor()
    {
        $.ajax({
            ...
            data: {
                autoloader: true,
                action: 'user/fetchListOfIndividualDonor',
            },
            success: function (data, textStatus, xhr) {
                /*                                        console.log(data);*/
                data = $.trim(data);
                    ...
                    default:
                        ...
                        if (type.status == "success")
                        {
                            var header = '<div class="table-responsive"><table class="table table-responsive table-bordered table-hover" id="list_col_table"><thead>' +
                                    '<tr><th>#</th><th>Full Name</th><th>Mobile</th><th>Email</th><th>Gender</th><th>Occupation</th><th>Option</th></tr></thead><tbody>';
