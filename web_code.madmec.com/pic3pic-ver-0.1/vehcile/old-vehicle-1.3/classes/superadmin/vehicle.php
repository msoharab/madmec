...
class vehicle {
    ...
    public function fetchVehicleModels(){
        $query = 'SELECT * FROM `vehicle_model` WHERE `status_id`=4 AND `vehicle_make_id`="'.  mysql_real_escape_string($this->parameters['makeid']).'";';
        $result = executeQuery($query);
        $fetchdata = array();
        $option = array();
        if (mysql_num_rows($result)) {
