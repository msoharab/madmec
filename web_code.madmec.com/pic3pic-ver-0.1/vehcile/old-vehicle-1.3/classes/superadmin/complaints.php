...
class complaints {
    ...
    public function addVehicleType() {
        $query = 'SELECT * FROM `complaint_type` WHERE `name`="' . mysql_real_escape_string($this->parameters['vehicletype']) . '" AND `status_id`=4 AND `vendor_id`="'.mysql_real_escape_string($_SESSION['USER_LOGIN_DATA']['USER_ID']).'"; ';
        ...
        {
        $query = 'INSERT INTO `complaint_type`(`id`,`name`,`vendor_id`)VALUES(NULL,'
    ...
    public function checkVehicleType() {
        $query = 'SELECT * FROM `complaint_type` WHERE `name`="' . mysql_real_escape_string($this->parameters['vehicletype']) . '" AND `status_id`=4 AND `id` !="' . mysql_real_escape_string($this->parameters['typeid']) . '" '
                . ' AND `vendor_id`="'.mysql_real_escape_string($_SESSION['USER_LOGIN_DATA']['USER_ID']).'";';
        echo $query;
        $result = executeQuery($query);
    ...
    public function fetchVehicleType() {
        $query = 'SELECT * FROM `complaint_type` WHERE `status_id`=4 AND `vendor_id`="'.mysql_real_escape_string($_SESSION['USER_LOGIN_DATA']['USER_ID']).'"';
        $result = executeQuery($query);
        $fetchdata = array();
        $data = '';
        $typeid = array();
    ...
    public function updateVehicleType() {
        $query = 'SELECT * FROM `complaint_type` WHERE `name`="' . mysql_real_escape_string($this->parameters['evehicletype']) . '" AND `status_id`=4 '
                . ' AND `id`!="' . mysql_real_escape_string($this->parameters['typeid']) . '"'
                . ' AND `vendor_id`="'.mysql_real_escape_string($_SESSION['USER_LOGIN_DATA']['USER_ID']).'";';
        $result = executeQuery($query);
        if(mysql_num_rows($result))
        {
