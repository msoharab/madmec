...
class uservehicle {
    ...
    public function fetchListOfVehicles() {
        ...
        if (mysql_num_rows($result)) {
            ...
            for ($i = 0; $i < sizeof($fetchdata); $i++) {
                $appdata .='<option value="'.$fetchdata[$i]['uvid'].'">'.$fetchdata[$i]['vehiclemodelname'] .'</option>';
                ...
                $vmakeids[$fetchdata[$i]['uvid']]=$fetchdata[$i]['vmakeid'];
            }
            $jsondata = array(
                "status" => "success",
                "data" => $data,
    ...
    public function fetchServiceCenters() {
        $query='SELECT * FROM `user_profile` WHERE `user_type_id`=2 AND `status`=4 AND `vendor_id`="'.  mysql_real_escape_string($this->parameters['makeid']).'"';
        $result=  executeQuery($query);
        $data=array();
        $fetchdata=array();
        $ids=array();
