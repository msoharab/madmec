...
class userappointment {
    ...
    public function checkForPreferedCenter() {
        ...
        if (mysql_num_rows($result)) {
            ...
            if(isset($this->parameters['change']))
            {
            ...
            if ((int) $row1['preferred_service_center']) {
                ...
                if (mysql_num_rows($result)) {
                    ...
                    $this->vendata = $row['user_name'] . '--' . $row['addressline'] . '--' . $row['town'] . '--' . $row['city'] . '--' . $row['district'] . '--' . $row['province'] . '--' . $row['cell_number'].''
                            . '&nbsp;&nbsp;&nbsp;<button type="button" name="changevendor" id="changevendor" class="btn btn-warning">Change</button>';
    ...
    public function fetchVendorAppointments(){
            $query='SELECT
                        apd.*,
                        apd.id AS appdescbid
                        FROM `appointment_descb` AS apd
                        LEFT JOIN appointment appt
                        ON appt.id=apd.appoint_id
                        WHERE apd.`status_id` = 4
                            AND DATE_FORMAT(CONCAT(apd.`date`," ",apd.`fromtime`), "%Y-%m-%d %H:%i:%s") >= DATE_FORMAT("'.  mysql_real_escape_string($this->parameters['date']).':00", "%Y-%m-%d %H:%i:%s")
                            AND appt.doctorid="'.  mysql_real_escape_string($this->parameters['serviceid']).' "
                            AND apd.location > apd.filled
                            AND  apd.date = DATE_FORMAT("'.  mysql_real_escape_string($this->parameters['date']).'", "%Y-%m-%d")
                            AND appt.`status_id` = 4
                            ORDER BY (apd.`id`)';
            $result=  executeQuery($query);
             $jsondata=array(
                    "status"  => "failure",
            ...
            if(mysql_num_rows($result)){
                ...
                while($row=  mysql_fetch_assoc($result))
                {
                 ...
                 $theader = '<div class="table-responsive"><table class="table table-striped table-bordered table-hover">
                                <tbody><tr><th>#</th><th>Slot Details</th><th>Option</th></tr><tbody>';
                 for($i=0;$i<sizeof($fetchdata);$i++)
                 {
                $data .='<tr><td>'.($i+1).'</td><td> Time : ' . $fetchdata[$i]['fromtime'] . '
                                         - ' . $fetchdata[$i]['totime'] . '
                                        <br/> Avaliable Slots : ' . ((int)$fetchdata[$i]['location'] - (int)$fetchdata[$i]['filled']) . '</td>'
                            . '<td><button type="button" class="btn btn-success  btn-md" data-toggle="modal" data-target="#myModal_flag'.$i.'" id="cblockslot"><i class="fa fa-book fa-fw "></i>Book</button>'
                        . '<div class="modal fade" id="myModal_flag'.$i.'" tabindex="-1" role="dialog" aria-labelledby="myUSRLISTDELModal" aria-hidden="true" style="display: none;">
							<div class="modal-dialog">
								<div class="modal-content" style="color:#000;">
									<div class="modal-header">
										<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
										<h4 class="modal-title" id="myUSRDELModalLabel">Are you really want to Book this Slot</h4>
									</div>
									<div class="modal-body" id="myUSRDEL">
										Do you really want to Book this Slot  <br />
										Press OK to Book !...
									</div>
									<div class="modal-footer">
										<button type="button" class="btn btn-danger" data-dismiss="modal" name=".modal-backdrop" id="blockslot'.$fetchdata[$i]['appdescbid'].'">Ok</button>
										<button type="button" class="btn btn-success" data-dismiss="modal" id="deletelltDELCancel">Cancel</button>
									</div>
								</div>
							</div>
						</div>'
                        . ''
                        . '</tr>';
                $appdescids[$i]=$fetchdata[$i]['appdescbid'];
                $appdates[$i]=$fetchdata[$i]['date'];
                 }
                $jsondata=array(
                    "status"  => "success",
                    "data"  => $theader.$data.'</<tbody></table>',
                    "dates" =>  $appdates,
                    "appdescids" => $appdescids,
                );
            }
            return $jsondata;
    }
...
//        echo $ctquery;
        ...
        if(mysql_num_rows($ctresult))
        {
            ...
            {
                $ctdata .='<div class="col-lg-2"><input type="checkbox" name="complainttypee" value="'.$ctfetchdata[$j]['id'].'"/> &nbsp;&nbsp;<strong>'.$ctfetchdata[$j]['name'].'</strong></div>';
            }
        }
        $ctdata .='<div class="col-lg-2"><input type="checkbox" name="complainttypee"  value="0"/> &nbsp;&nbsp;<strong>Others</strong></div>';
...
//                                             <br/> Location : '.$location[$k].'
                    $pdata[$i] .='<tr id="bookapprow'.$grpappid[$k].'"><td>' . $j++ . '
                            ...
                            . '<td><button type="button" class="btn btn-success  btn-md" id="viewappointee_' . $grpappid[$k] . '"><i class="fa fa-book fa-fw "></i>Book</button>                                           
...
//                                             Location : '.$location[$k].'<br/>
        ...
        $jsonmess = array(
            ...
            "ctdata" => $ctdata,
            "noofcomttype" => sizeof($ctfetchdata),
...
//        echo print_r($this->parameters);
        $query = 'SELECT * FROM  `bookinghistory` WHERE `apptdescb_id`=' . mysql_real_escape_string($this->parameters['inputdetl']['slodid']) . ''
                . '   AND `vehicle_id` = "'.mysql_real_escape_string($_SESSION["vehicleid"]).'" AND `patientid`=
        ...
        $data = array(
         ...
         if ($check) {
            ...
            $query1 = 'INSERT INTO `bookinghistory`(`id`,`apptdescb_id`,`date`,`patientid`,`vehicle_id`,`servicetype`,`bookservice`,`bookdetails`,`usercomment`,`status_id`) VALUES(null,"'
                    . mysql_real_escape_string($this->parameters['inputdetl']['slodid']) . '","'
                    . mysql_real_escape_string($this->parameters['inputdetl']['date']) . '","'
                    . mysql_real_escape_string($_SESSION["USER_LOGIN_DATA"]["USER_ID"]) . '","'
                    . mysql_real_escape_string($_SESSION["vehicleid"]) . '","'
                    . mysql_real_escape_string($this->parameters['inputdetl']['typeofservice']) . '","'
                    . mysql_real_escape_string($this->parameters['inputdetl']['bookaservice']) . '",'
                    . 'CURRENT_TIMESTAMP(),"'
                    . mysql_real_escape_string($this->parameters['inputdetl']['remark']) . '",'
                    . '4'
                    . ');';
            if (executeQuery($query1)) {
                $lastid=  mysql_insert_id();
                if (is_array($this->parameters['inputdetl']["complainttype"]) && sizeof($this->parameters['inputdetl']["complainttype"]) > -1) {
                    $query = 'INSERT INTO  `booked_complaints` (`id`,`bookhis_pk`,`complaint_type`,`status` ) VALUES';
                    for ($i = 0; $i < sizeof($this->parameters['inputdetl']["complainttype"]); $i++) {
                        if ($i == sizeof($this->parameters['inputdetl']["complainttype"]) - 1)
                            $query .= '(NULL,\'' . mysql_real_escape_string($lastid) . '\',\'' . mysql_real_escape_string($this->parameters['inputdetl']["complainttype"][$i]) . '\',4);';
                        else
                            $query .= '(NULL,\'' . mysql_real_escape_string($lastid) . '\',\'' . mysql_real_escape_string($this->parameters['inputdetl']["complainttype"][$i]) . '\',4),';
                    }
                    executeQuery($query);
                ...
                $query = 'UPDATE `appointment_descb` SET `filled` = `filled`+1 WHERE `id`="'
                        . mysql_real_escape_string($this->parameters['inputdetl']['slodid']) . '" ;';
                executeQuery($query);
                $query = 'UPDATE `user_vehicle` SET `preferred_service_center` ="'.$this->parameters['inputdetl']['serviceid'].'" WHERE `id`="'.  mysql_real_escape_string($_SESSION["vehicleid"]).'";';
                executeQuery($query);
            }
            executeQuery("COMMIT");
            $data = array(
