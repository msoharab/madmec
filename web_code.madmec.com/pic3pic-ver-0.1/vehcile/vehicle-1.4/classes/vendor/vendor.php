...
//                        executeQuery('COMMIT');
    ...
    public function configureappointment() {
        ...
        if (is_array($this->parameters["days"]) && sizeof($this->parameters["days"]) > -1) {
            ...
            $start_time = strtotime($this->parameters['fromtime'] . ":00");
            $end_time = strtotime($this->parameters['totime'] . ":00");
            $fifteen_mins = 15 * 60;
...
//                                            $from = explode(":",$this->parameters['fromtime']);
//                                            $fh = (int) $from[0] * 60;
//                                            $fm = (int) $from[1];
//                                            $ftot = (int) $fh+$fm;
//                                            $to = explode(":",$this->parameters['totime']);
//                                            $th = (int) $to[0] * 60;
//                                            $tm = (int) $to[1];
//                                            $totot = (int) $th+$tm;
//                                            $between = $totot - $ftot;
//                                            $slots = (int) $between / 15;
                $query1 = 'INSERT INTO  `appointment_descb` (`id`,`appoint_id`,`fromtime`,`totime`,`location`,`frequency`,`date` ) VALUES';
                for ($j = 0; $j < sizeof($array_of_time) && isset($array_of_time[$j + 1]); $j++) {
                    if ($j == sizeof($array_of_time) - 2) {
                        $tempres = executeQuery("SELECT DAYOFWEEK(NOW()) as weekofday");
                        $rowdat = mysql_fetch_assoc($tempres);
                        if ($this->parameters['days'][$i] < $rowdat['weekofday']) {
                            $query1 .= '(NULL,"' . mysql_real_escape_string($lastid) . '",'
                                    . '"' . mysql_real_escape_string($array_of_time[$j]) . '",'
                                    . '"' . mysql_real_escape_string($array_of_time[$j + 1]) . '",'
                                    . '"' . mysql_real_escape_string($this->parameters['location']) . '",'
                                    . '"' . mysql_real_escape_string($this->parameters['frequency']) . '",'
                                    . '(DATE_ADD(NOW(), INTERVAL (' . (mysql_real_escape_string($this->parameters['days'][$i])) . ' - DAYOFWEEK(NOW())) DAY)));';
                        } else {
                            $query1 .= '(NULL,"' . mysql_real_escape_string($lastid) . '",'
                                    . '"' . mysql_real_escape_string($array_of_time[$j]) . '",'
                                    . '"' . mysql_real_escape_string($array_of_time[$j + 1]) . '",'
    ...
    public function editappointment() {
        $query = 'UPDATE `appointment_descb` SET `location`="' . mysql_real_escape_string($this->parameters['location']) . '"  WHERE `id`=' . trim($this->parameters['appid']);
        executeQuery($query);
    }
...
//                                             <br/> Location : '.$location[$k].' 
                    $pdata[$i] .='<tr><td>' . $j++ . '
                                         ...
                                         - ' . $totimee[$k] . '  
                                        <br/> No of Vehicles : ' . $location[$k] . '<br/> Filled : ' . $filled[$k] . '</td>'
                            . '<td><button type="button" class="btn btn-success  btn-md" id="viewappointee_' . $grpappid[$k] . '"><i class="fa fa-file fa-fw "></i>View</button></td>'
                            . '</tr><tr><td colspan="3"><div id="viewappointeedis_' . $grpappid[$k] . '"></div></td></tr>';
                    $appids[] = $grpappid[$k];
                    $dates[] = $grpdates[$k];
...
//                                             Location : '.$location[$k].'<br/>
                    $pdata1[$i] .='<tr id="row_id' . $grpappid[$k] . '"><td>' . $p++ . '</td>
                            ...
                            . '<td><button type="button" class="btn btn-warning  btn-md" id="edit_app' . $grpappid[$k] . '" title="Edit"><i class="fa fa-edit fa-fw "></i></button> - '
                            . '<button title="Delete" class="btn btn-danger  btn-md" name="' . $grpappid[$k] . '" id="delete_apptt_' . $grpappid[$k] . '" data-toggle="modal" data-target="#myModal_' . $grpappid[$k] . '"><i class="fa fa-trash fa-fw "></i></button>
...
//                  "divfooter" => '</div></div></div></div></div>',
    ...
    public function getUpcomming() {
        $html = '';
        $obj = new dashboardVendor('data');
        $data = $obj->getUpcomming();
        for ($i = 0; $i < sizeof($data["data"]); $i++) {
            $html .= '<tr id="row-' . $data["data"][$i]["bookid"] . '">
                        <td>' . ($i + 1) . '</td>
                        ...
                        <td>' . $data["data"][$i]["bookdate"] . '</td>
                        <td>' . $data["data"][$i]["servicetype"] . '</td>
                        <td>' . $data["data"][$i]["bookservice"] . '</td>
                        <td><button type="button" class="btn btn-danger btn-md" id="clickme_' . $data["data"][$i]["bookid"] . '">In line</button>
                            <script>
                                $(document).ready(function(){
                                    $("#clickme_' . $data["data"][$i]["bookid"] . '").click(function(){
                                        var para = {
                                            autoloader:true,
                                            defaultaction:"doctor/pushInline",
                                            bookid:"' . $data["data"][$i]["bookid"] . '",
                                            rowid:"#row-' . $data["data"][$i]["bookid"] . '"
                                        };
                                        console.log(para);
                                        var obj = new doctorcontroller();
                                        console.log(obj);
                                        obj.__pushInline(para);
                                    });
                                });
                            </script>
                        </td>
    ...
    public function getInline() {
        ...
        for ($i = 0; $i < sizeof($data["data"]); $i++) {
            $html .= '<tr id="row-' . $data["data"][$i]["bookid"] . '">
                        ...
                        <td>
                        <div class="form-group">
                        <input class="form-control" id="commentme_' . $data["data"][$i]["bookid"] . '" placeholder="Comments" /><p class="help-block"></p>
                        <button type="button" class="btn btn-danger btn-md btn-block" id="clickme_' . $data["data"][$i]["bookid"] . '">Complete</button>
                        </div>
                        <script>
                            $(document).ready(function(){
                                $("#clickme_' . $data["data"][$i]["bookid"] . '").click(function(){
                                    var para = {
                                        autoloader:true,
                                        defaultaction:"doctor/pushComplete",
                                        bookid:"' . $data["data"][$i]["bookid"] . '",
                                        commentid:"#commentme_' . $data["data"][$i]["bookid"] . '",
                                        rowid:"#row-' . $data["data"][$i]["bookid"] . '"
                                    };
                                        console.log(para);
                                    var obj = new doctorcontroller();
                                        console.log(obj);
                                    obj.__pushComplete(para);
                                });
                            });
                        </script>
                        </td>
    ...
    public function pushInline() {
        ...
        if (isset($param["bookid"])) {
            $res = executeQuery('UPDATE `bookinghistory` SET `status_id` = "35" WHERE `id` = "'.mysql_real_escape_string($param["bookid"]).'";');
            if ($res) {
                $status = array(
                    "status" => 'success',
                    "flag" => $param["bookid"],
    ...
    public function pushComplete() {
        ...
        if (isset($param["bookid"])) {
            $res = executeQuery('UPDATE `bookinghistory` SET `status_id` = "36",`finalcomment` = "'.mysql_real_escape_string($param["comment"]).'" WHERE `id` = "'.mysql_real_escape_string($param["bookid"]).'";');
            if ($res) {
                $status = array(
                    "status" => 'success',
                    "flag" => $param["bookid"],
