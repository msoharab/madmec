    ...
    if (get_resource_type($link) == 'mysql link') {
        if (($db_select = selectDB(DBNAME_ZERO, $link)) == 1) {
            switch ($parameters["action"]) {
                case "signIn":
                    echo json_encode(userLogin($parameters));
                    break;
            }
        }
    }
...
function userLogin($parameters) {
    ...
    $query = 'SELECT a.*
                FROM `user_profile` a
                WHERE a.`email`=\'' . mysql_real_escape_string($parameters["user_name"]) . '\'
    ...
    if (get_resource_type($res) == 'mysql result') {
        if (mysql_num_rows($res) > 0) {
            ...
            // echo '<br />'.print_r($row);
            if ($row["email"] == $parameters["user_name"] &&
            ...
            } else if ($row["email"] == $parameters["user_name"] &&
                    $row["password"] != $parameters["password"]) {
                $userdata = array(
                    "USER_EMAIL" => NULL,
