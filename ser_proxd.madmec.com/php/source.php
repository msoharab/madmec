		...
		$link = MySQLconnect(DBHOST,DBUSER,DBPASS);
				...
				if(($db_select = selectDB(DBNAME_ZERO,$link)) == 1){
					$query='SELECT *
							FROM settings
							WHERE
							ac_user_name="'.$eml.'" AND ac_password = "'.$pas.'" AND password_status = (SELECT `id` FROM `status` WHERE `statu_name` = "on" AND `status` = 1) ';
						$res=executeQuery($query);
						if(mysql_num_rows($res))
							echo "valid";
						else
	...
	function AddFormSrcAc(){
		echo '<form role="form">
				<div class="form-group">
					...
					<input id="src_ac_no" onkeyDown="javascript:number_allow();" class="form-control" placeholder="Account Number">
					<span class="text-danger" id="err_src_ac_no">Invalid</span>
				</div>
				<div class="form-group">
					<span class="manditory">*</span>
					<label>Account Name</label>
					<input id="src_ac_name" onkeyDown="javascript:account_name() ;" class="form-control"  placeholder="Account Name">
					<span class="text-danger" id="err_src_ac_name">Invalid</span>
				</div>
				<div class="form-group">
					<span class="manditory">*</span>
					<label>Bank</label>
					<input id="src_bank"  onkeyDown="javascript:bank_name();"  class="form-control"  placeholder="Bank name(ex: HDFC)">
					<span class="text-danger" id="err_src_bank">Invalid</span>
				</div>
				<div class="form-group">
					<span class="manditory">*</span>
					<label>Brance</label>
				...
				if(mysql_num_rows($res)){
					...
					echo '<tr>
						...
						<td>
							...
							<button class="btn btn-danger" onclick="javasript:deactivate_src_ac('.$row['id'].');">
