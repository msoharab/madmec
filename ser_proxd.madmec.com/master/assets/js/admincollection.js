...
//		});
	...
	function fetchDistributor(){
        ...
        function  fetchmops()
        {
            $(colls.add.mopdiv).html(LOADER_FIV);
            $.ajax({
				type:'POST',
				url:window.location.href,
				data:{autoloader:true,action:'fetchmops',type:'master'},
				success:function(data, textStatus, xhr){
                                        console.log(data);
					data = $.trim(data);
					switch(data){
						...
						default:
							...
							if(details.status=="success")
                                                        {
                                                         $(colls.add.mopdiv).html('<select class="form-control" id="collmopsu"><option value="">Please Select MOP</option>'+details.data+'</select>');   
                                                        }
