...
class Food_Model extends BaseModel {
...
//        $searchqr = '';
        ...
        $searchqr = ' AND (ad.`user_name`           LIKE :col1
                        ...
                        OR up. `users_proof_id`     LIKE :col4
        ...
        if (isset($this->postBaseData["order"])) {
            ...
            switch ($column) {
                case 0:
                    $orderqr = ' ORDER BY ad.`users_pk` ' . $dir;
                    break;
                case 1:
        ...
        $query = 'SELECT
          ...
          FROM `users` AS ad  
         LEFT JOIN `users_email_ids` AS ue ON ue.`users_email_ids_users_pk` = ad.`users_pk`
        ...
        WHERE (ad.`users_pk` != NULL
        OR ad.`users_pk` IS NOT NULL)
        AND ad.`user_type_id` = 8
        AND ad.`user_status_id` = 4 ' . $searchqr . ' ' . $orderqr;
    ...
    public function EditUserPersonal($id = false) {
        $jsondata = array(
            "name" => $this->postBaseData["name"],
            "email" => $this->postBaseData["email"],
            "pass" => $this->generateRandomString(),
        ...
        $d = date("Y-m-d H:i:s");
        $this->db->beginTransaction();
        $q1 = 'SELECT ad.*,up.* FROM `users` ad,`users_proof` up WHERE ad.`user_proof_id` = up.`users_proof_id` AND `users_pk`=:id';
        ...
        $directory = $this->createdirectories(substr(md5(microtime()), 0, 6) . '_user_' . $id);
    ...
    public function AddBusinessDetails() {
        $jsondata = array(
            "user_pk" => $this->postBaseData["users_pk"],
        ...
        $stm = $this->db->prepare('SELECT
            t1.`user_directory`
            FROM `users` AS t1
            WHERE t1.`users_pk`= :email
            AND t1.`user_status_id` = :stat
            ');
        $res = $stm->execute(array(
            ":email" => ($jsondata["users_pk"]),
            ":stat" => 4,
        ));
        $count = $stm->rowCount();
        if ($count > 0) {
    ...
    public function ListUserBusiness() {
        ...
        $searchqr = ' AND (ad.`users_business_name`         LIKE :col1
                        ...
                        OR ad.`users_business_website`      LIKE :col4
                        OR up. `users_proof_id`     LIKE :col5
                        OR pp.`portal_proof_name`   LIKE :col6)';
        if (isset($this->postBaseData["order"])) {
            $column = (int) $this->postBaseData["order"][0]["column"];
            $dir = $this->postBaseData["order"][0]["dir"];
            switch ($column) {
        ...
        LEFT JOIN `users` AS t1 ON ad.`users_business_users_pk` = t1.`users_pk`
        LEFT JOIN `users_proof` AS up ON t1.`users_pk` = up.`users_proof_id` 
        LEFT JOIN `portal_proof` AS pp ON up.`users_proof_portal_proof_id` = pp.`portal_proof_id`
        WHERE (ad.`users_business_id` != NULL
        OR ad.`users_business_id` IS NOT NULL)
        AND t1.`user_type_id` = 8
        AND ad.`users_business_status_id` = 4
        AND t1.`user_status_id` = 4' . $searchqr . ' ' . $orderqr;
        $stm = $this->db->prepare($query);
        $keyword = "%" . ($this->postBaseData["search"]["value"]) . "%";
        $stm->bindParam(':col1', $keyword, PDO::PARAM_STR);
        $stm->bindParam(':col2', $keyword, PDO::PARAM_STR);
    ...
    public function UserRequestNew() {
        ...
        OR ad.`users_pk` IS NOT NULL)
        AND ad.`user_type_id` = 8
        AND ad.`user_status_id` = 1' . $searchqr . ' ' . $orderqr . ' ;';
        $stm = $this->db->prepare($query);
        $res = $stm->execute();
        if ($res) {
            $num_posts = $stm->rowCount();
    ...
    public function UserRequestRejected() {
        ...
        OR ad.`users_pk` IS NOT NULL)
        AND ad.`user_type_id` = 8
        AND ad.`user_status_id` = 42 ' . $searchqr . ' ' . $orderqr . ' ;';
        $stm = $this->db->prepare($query);
        $res = $stm->execute();
        if ($res) {
            $num_posts = $stm->rowCount();
    ...
    public function ProcessUser() {
        ...
        $query = 'UPDATE `users`
                ...
                WHERE `users_pk`="' . ($this->postBaseData['id']) . '"';
        $stm = $this->db->prepare($query);
        $res = $stm->execute();
        if ($res) {
            $this->db->commit();
