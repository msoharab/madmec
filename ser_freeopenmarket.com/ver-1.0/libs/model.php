...
class BaseModel extends configure {
    ...
    public function getUserTypes($id = true) {
        ...
        $stm = $this->db->prepare('SELECT * FROM `users_type` WHERE `users_type_id` = :uid AND `users_type_status_id` = :stat');
        $res = $stm->execute(array(
            ":uid" => ($id),
            ":stat" => 4,
        ));
    ...
    public function getUserDirectory($id = false) {
        $dir = '';
        if ($id) {
            $query = 'SELECT `user_directory` FROM `users` WHERE `users_pk`=:id2';
    ...
    public function fetchGenderDB($listtype = false) {
        ...
        $stm = $this->db->prepare('SELECT * FROM `users_gender` WHERE `users_gender_status_id` = :stat ORDER BY `users_gender_id`');
        $res = $stm->execute(array(
            ":stat" => 4
        ));
        $res = $stm->execute();
        if ($res) {
            ...
            switch ($listtype) {
                case "checkbox":
                    for ($i = 0; $i < $data["count"]; $i++) {
                        $data["html"] .= '<li style="cursor:pointer">' .
                                ...
                                'class="' . $data["class"] . '" ' .
                                'id="' . $data["id"] . '_' . $i . '" ' .
                                'value="' . $result[$i]["users_gender_id"] . '" />' .
                                '&nbsp;&nbsp;' . ucfirst(trim($result[$i]["users_gender_name"])) . '</li>';
                    }
                    break;
                case "radio":
                    for ($i = 0; $i < $data["count"]; $i++) {
                ...
                default:
                    for ($i = 0; $i < $data["count"]; $i++) {
                        $data["html"] .= '<option value="' . $result[$i]["users_gender_id"] . '">' .
                                ucfirst(trim($result[$i]["users_gender_name"])) .
                                '</option>';
                    }
                    break;
            }
    ...
    public function fetchUserTypesDB($listtype = false) {
        ...
        $usertypes = isset($this->postBaseData["usertype_ids"]) && is_array($this->postBaseData["usertype_ids"]) ? implode(",", $this->postBaseData["usertype_ids"]) :
                "3,4,5,6,7,8,9";
        $stm = $this->db->prepare('SELECT * FROM `users_type` WHERE `users_type_status_id` = :stat AND `users_type_id` IN (' . $usertypes . ') ORDER BY `users_type_id` ');
        $res = $stm->execute(array(
            ":stat" => 4
        ));
        $res = $stm->execute();
        if ($res) {
            ...
            switch ($listtype) {
                case "checkbox":
                    for ($i = 0; $i < $data["count"]; $i++) {
                        $data["html"] .= '<li style="cursor:pointer">' .
                                ...
                                'class="' . $data["class"] . '" ' .
                                'id="' . $data["id"] . '_' . $i . '" ' .
                                'value="' . $result[$i]["users_type_id"] . '" />' .
                                '&nbsp;&nbsp;' . ucfirst(trim($result[$i]["users_type_type"])) . '</li>';
                    }
                    break;
                case "radio":
                    for ($i = 0; $i < $data["count"]; $i++) {
                ...
                default:
                    for ($i = 0; $i < $data["count"]; $i++) {
                        $data["html"] .= '<option value="' . $result[$i]["users_type_id"] . '">' .
                                ucfirst(trim($result[$i]["users_type_type"])) .
                                '</option>';
                    }
                    break;
            }
    ...
    public function updatePortalPhotoDetails($photo = false, $picid = 0) {
        $res3 = false;
        if (is_array($photo) && $picid > 0) {
            if (count($photo) > 0) {
                $query3 = 'UPDATE `portal_photo` SET `original_pic` = :name,
                        `ver1` = :des,
                        `ver2` = :now,
                        `ver3` = :web,
                        `ver4` = :fb,
