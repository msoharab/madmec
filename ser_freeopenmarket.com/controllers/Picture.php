...
class Picture extends BaseController {
    ...
    public function UploadPic($target = false) {
        ...
        if (isset($this->postFile) && count($this->postFile) > 0) {
            switch ($target) {
                case"gym": {
                        if (isset($this->postFile['gymlogo']['name'])) {
                            $file_name[] = $this->postFile['gymlogo']['name'];
                            $file_size[] = $this->postFile['gymlogo']['size'];
                            $file_tmp[] = $this->postFile['gymlogo']['tmp_name'];
                            $file_type[] = $this->postFile['gymlogo']['type'];
                            $temp = explode('.', $this->postFile['gymlogo']['name']);
                            $temp = $temp[count($temp) - 1];
                            $file_ext[] = strtolower($temp);
                        }
                        if (isset($this->postFile['gymheader']['name'])) {
                            $file_name[] = $this->postFile['gymheader']['name'];
                            $file_size[] = $this->postFile['gymheader']['size'];
                            $file_tmp[] = $this->postFile['gymheader']['tmp_name'];
                            $file_type[] = $this->postFile['gymheader']['type'];
                            $temp = explode('.', $this->postFile['gymheader']['name']);
                            $temp = $temp[count($temp) - 1];
                            $file_ext[] = strtolower($temp);
                        }
                        if (isset($this->postFile['inView']['name'])) {
                            $file_name[] = $this->postFile['inView']['name'];
                            $file_size[] = $this->postFile['inView']['size'];
                            $file_tmp[] = $this->postFile['inView']['tmp_name'];
                            $file_type[] = $this->postFile['inView']['type'];
                            $temp = explode('.', $this->postFile['inView']['name']);
                            $temp = $temp[count($temp) - 1];
                            $file_ext[] = strtolower($temp);
                        }
                        $chdir = $this->PictureMod->getDirectory($this->jsonData["id"], "gym_profile", "id");
                        break;
                    }
                case"user": {
                        $chdir = $this->PictureMod->getDirectory($this->jsonData["id"], "user_profile", "id");
                        break;
                    }
            }
