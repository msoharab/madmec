...
//clear webcache
$files = glob(WEBCACHE_ROOT); // get all file names
...
<div class="content-top">
	<div class="container">
		<div class="col-md-12">
			<center><h3>Welcome to Habeeb Enterprises - Home</h3></center><hr />
		</div>
	</div>
</div>
<div class="content-mid">
	<div class="container">
		<div class="col-lg-12">
			<img src="<?php echo URL ?>assets/video/video.gif" style="width:100%"/>
		</div>
	</div>
</div>
...
<div class="footer">
	<div class="container">
		...
		<div class="copy-right">
			<p> &copy; 2016 MadMec. All Rights Reserved | Design by  <a href="http://www.madmec.com" target="_new"> MadMec</a></p>
		</div>
	</div>
</div>
<!-- //footer-->
...
file_put_contents($myini_file, $myini);
	/*
	$myini_file = $bin_path .'my.ini';
	...
	*/
	...
	/* MySQL Database */
	mysqli_select_db($mysqli, "habeebshop");
	...
	$sales_ids = array();
	if ($result = mysqli_query($mysqli, "SELECT `id` FROM `bill` WHERE DATE_FORMAT(`doc`,'%Y-%c-%d')  <= DATE_FORMAT('". $date->format("Y-m-d") ."','%Y-%c-%d') ;")) {
		while($row = mysqli_fetch_row($result)) {
			array_push($sales_ids,$row[0]);
		}
	}
	$sales_ids = implode(",",$sales_ids);
	mysqli_query($mysqli, "DELETE FROM `bill` WHERE DATE_FORMAT(`doc`,'%Y-%c-%d')  <= DATE_FORMAT('". $date->format("Y-m-d") ."','%Y-%c-%d') ;");
	//mysqli_query($mysqli, "DELETE FROM `sales` WHERE (DATE_FORMAT(`doc`,'%Y-%c-%d')  <= DATE_FORMAT('". $date->format("Y-m-d") ."','%Y-%c-%d')) OR `bill_id` IN (".$sales_ids.");");
	mysqli_query($mysqli, "DELETE FROM `sales` WHERE (DATE_FORMAT(`doc`,'%Y-%c-%d')  <= DATE_FORMAT('". $date->format("Y-m-d") ."','%Y-%c-%d'));");
	$mysqli->close();
?>
