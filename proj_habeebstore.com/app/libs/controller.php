...
class BaseController extends configure {
    ...
    public function gotoDashboard() {
        if (isset($_SESSION["USERDATA"]["loggedin"]) && $_SESSION["USERDATA"]["loggedin"] == 1) {
            header('Location:' . $this->config["EGPCSURL"] . $this->config["CTRL_19"]);
        }
    }
    public function gotoIndex() {
        if (!isset($_SESSION["USERDATA"])) {
            header('Location:' . $this->config["EGPCSURL"] . $this->config["CTRL_1"]);
        }
    }
    ...
    public function fetchProduct() {
        $this->baseview->setjsonData($this->basemodel->fetchProductDB(isset($this->postPara['listtype']) ? $this->postPara['listtype'] : false));
        echo $this->baseview->renderJson();
    }
