function HeaderController()
{
    ...
    this.__constructor = function (para) {
        members = para;
        member = para.index.header.filter;
    ...
    function filterListLead() {
        ...
        if (attr) {
            $(members.index.list.outputDiv).html(LOADER_ONE);
            $(recentObjCountry).remove();
            $(recentObjlanguge).remove();
            ...
            $.ajax({
                ...
                success: function (data, textStatus, xhr) {
                    window.setTimeout(function () {
                        var obj = new indexLeadController();
                        obj.__constructor(members);
                        obj.publicFilterlead(data);
                    }, 400);
                },
                error: function (xhr, textStatus) {
                },
