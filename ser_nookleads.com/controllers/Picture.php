...
class Picture extends BaseController {
    private $para, $UserId;
    private $PictureMod, $absolutepath, $relativepath, $BusinessMod;
    function __construct($para = false) {
        parent::__construct();
        $this->para = $para;
        $this->PictureMod = new Picture_Model();
        $this->PictureMod->setLeadData($this->postPara);
    ...
    public function UploadPic($target = false) {
        ...
        if (isset($this->postFile['file'])) {
            ...
            if (empty($errors) == true) {
                $path1 = '';
                $file = '';
                if ($target === 'business' && isset($_SESSION["BUSINESS_ID"])) {
                    $chdir = $this->BusinessMod->getBusinessDirectory($_SESSION["BUSINESS_ID"]);
                    $path1 = $this->config["DOC_ROOT"] . $chdir;
                    $this->relativepath = $chdir;
                    $file = $this->UserId . '_original_pic' . time() . '.' . $file_ext;
