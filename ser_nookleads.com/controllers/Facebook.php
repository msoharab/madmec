...
class Facebook extends BaseController {
    ...
    public function Index() {
        ...
        $fb = new Facebook\Facebook([
            'app_id' => '1728128874111475',
            'app_secret' => 'eb1db4c7f235055634feaa2628dcc667',
        ...
        $loginUrl = $helper->getLoginUrl(
                $this->config["URL"] .
                $this->config["CTRL_26"] . 'OAuthPHPResponse', $permissions);
        $this->baseview->FBRequest = '<a href="' . $loginUrl . '" class="btn btn-facebook">Login / Register with Facebook!</a>';
    ...
    public function OAuthPHPResponse() {
        ...
        try {
            ...
            $this->baseview->FBError = 'Graph returned an error: ' . $e->getMessage();
        } catch (Facebook\Exceptions\FacebookSDKException $e) {
            // When validation fails or other local issues
            $this->baseview->FBError = 'Facebook SDK returned an error: ' . $e->getMessage();
        }
        ...
        if ($this->baseview->FBData != NULL) {
            ...
            if (isset($stat["status"]) && $stat["status"] === 'failed') {
                ...
                if (isset($stat["status"]) && $stat["status"] === 'success') {
                    $this->baseview->FBData = '<script type="text/javascript">window.location.href = "' . $this->config["URL"] . $this->config["CTRL_7"] . '"</script>';
                } else {
                    $this->baseview->FBData = '<script type="text/javascript">window.location.href = "' . $this->config["URL"] . '"</script>';
                }
            } else {
                $this->loginMod->setLeadData(
                        array(
                            "autoloader" => true,
                            "action" => "signIn",
