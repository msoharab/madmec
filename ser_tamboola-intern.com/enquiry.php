			...
			while($row = mysql_fetch_assoc($res)){
				...
				if(sizeof($id)){
					...
					for($j=0;$j<sizeof($id);$j++){
						...
						$listofenquiries[$i]['handled_by'][$j] = ltrim($handled_by[$j] , ",");
						$listofenquiries[$i]['referred_by'][$j] = ltrim($referred_by[$j] , ",");
						$listofenquiries[$i]['jop'][$j] = ltrim($jop[$j] , ",");
						...
						$listofenquiries[$i]['interested_in'][$j] = str_replace("♥♥♥♥♥","&nbsp;&nbsp;",ltrim($interested_in[$j] , ","));
						$listofenquiries[$i]['ads_type'][$j] = ltrim($ads_type[$j] , ",");
                                                }
					}
				}
				$i++;
		...
		if($num_posts > 0){
			...
			for($i=$initial;$i<=$final && $i <= $num_posts && isset($post_ids[$i]['id']);$i++){
				...
				for($j=0;$j<$enq_today && isset($post_ids[$i]['id'][$j]) && $post_ids[$i]['id'][$j] != '';$j++){
					$htm .= '<div id="enquiry_'.$post_ids[$i]['id'][$j].'">
							...
							<li><a href="add_user.php?enq_id='.$post_ids[$i]['id'][$j].'" >Add</a></li>
							<li>
								...
								<script language="javascript" type="text/javascript" >
									$("#trigger_del_'.$post_ids[$i]['id'][$j].'").on("click",function(){
									var del = {
										id		:		'.$post_ids[$i]['id'][$j].',
										index	:		'.$i.',
							...
							<p>
							<dl  class="dl-horizontal">
							<dt>Medium of <br />advertisement</dt>
					...
					for($k=0;$k<$follow_ups;$k++){
						$htm .= '<h4><span id="followup_date_'.$post_ids[$i]['followup_id'][$j][$k].'" style="display:none;"><img class="img-circle" src="'.URL.ASSET_IMG.'spinner_grey_120.gif" border="0" width="30" height="30" /></span>Follow up on : '.date("j-M-Y",strtotime($post_ids[$i]['followup_date'][$j][$k])).'</h4>
								<div class="form-group">
								<textarea rows="4" cols="100" class="form-control" id="comments_'.$post_ids[$i]['followup_id'][$j][$k].'" >';
							if(!empty($post_ids[$i]['comments'][$j][$k]))
								$htm .= $post_ids[$i]['comments'][$j][$k];
							$htm .= '</textarea>
									<a href="javascript:void(0);" id="follow_up_'.$post_ids[$i]['followup_id'][$j][$k].'" class="btn btn-lg btn-success btn-block"><i class="fa fa-upload fa-fw"></i> Update</a>
									</div>
									...
									<script language="javascript" type="text/javascript" >
									$("#follow_up_'.$post_ids[$i]['followup_id'][$j][$k].'").on("click",function(){
									var followstatus = {
	...
	public static function returnSearchQuery(&$searchQuery,$para){
		...
		switch($parameters["list_type"]){
			case "offer":
				...
				$offers = $obj -> getOffers($num,false,$parameters["fct_type"]);
				if($offers != NULL){
