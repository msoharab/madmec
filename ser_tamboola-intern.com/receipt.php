...
class receipts{
		...
		$rec = array();
				...
				if( !empty($this->parameters["nameemail"]) || !empty($date)){
					$sub1 = (strlen($this->parameters["nameemail"]) > 1) ? 
							" (b.`name` LIKE '".mysql_real_escape_string($this->parameters["nameemail"])."%' OR b.`email` LIKE '".mysql_real_escape_string($this->parameters["nameemail"])."%' )" 
							: "(a.`customer_pk` IS NOT NULL)";
				...
				if(get_resource_type($res) == 'mysql result'){
					if(mysql_num_rows($res) > 0){
						...
						while( $row = mysql_fetch_assoc($res)){
							...
							/* Photo */
							if($row['ver2'])
								$rec[$i]['photo'] = URL.ASSET_DIR.$row['ver2'];
							else
	...
	function fetch_img($customer_pk){
		...
		if(mysql_num_rows($res2) > 0){
			...
			$img = URL.ASSET_DIR.$row2['ver3'];
		}
		else{
			$img = URL.ASSET_DIR.'images/anonymous.png';
		}
		return $img;
	}
}	
