...
class clientadd{
		...
		$users = array();
	  	...
	  	$query = 'SELECT
			  ...
			  GROUP BY (upgp.`user_pk`)
				...
				WHERE ut.`type` = "Admin" AND ut.`status`= (SELECT `id` FROM `status` WHERE `statu_name` = "Show" AND `status` = 1)
			...
			if($total){
				for($i=0;$i<$total;$i++){
					...
					$users[$i]["gymemailid"] = explode("☻☻♥♥☻☻",$users[$i]["gymemailid1"]);
					//$console_php->log($users[$i]["gymemailid"]);
					$users[$i]["email_pk"] = explode("☻☻♥♥☻☻",$users[$i]["email_pk"]);
					$users[$i]["gymcell_no"] = explode("☻☻♥♥☻☻",$users[$i]["gymcell_num"]);
					$users[$i]["cnumber_pk"] = explode("☻☻♥♥☻☻",$users[$i]["cnumber_pk"]);
					$users[$i]["email"] = explode("☻☻♥♥☻☻",$users[$i]["email"]);
					$users[$i]["gymemail"] = explode("☻☻♥♥☻☻",$users[$i]["gymemail"]);
					$users[$i]["gym_name"] = explode("☻☻♥♥☻☻",$users[$i]["gym_name"]);
					$users[$i]["gym_type"] = explode("☻☻♥♥☻☻",$users[$i]["gym_type"]);					
					$users[$i]["db_host"] = explode("☻☻♥♥☻☻",$users[$i]["db_host"]);
					$users[$i]["db_username"] = explode("☻☻♥♥☻☻",$users[$i]["db_username"]);
					$users[$i]["db_name"] = explode("☻☻♥♥☻☻",$users[$i]["db_name"]);
					$users[$i]["db_password"] = explode("☻☻♥♥☻☻",$users[$i]["db_password"]);
					$users[$i]["gym_directory"] = explode("☻☻♥♥☻☻",$users[$i]["gym_directory"]);
					$users[$i]["gym_short_logo"] = explode("☻☻♥♥☻☻",$users[$i]["gym_short_logo"]);
					$users[$i]["gym_header_logo"] = explode("☻☻♥♥☻☻",$users[$i]["gym_header_logo"]);
					$users[$i]["gym_telephone"] = explode("☻☻♥♥☻☻",$users[$i]["gym_telephone"]);
					$users[$i]["gym_currency_code"] = explode("☻☻♥♥☻☻",$users[$i]["gym_currency_code"]);
					$users[$i]["gym_postal_code"] = explode("☻☻♥♥☻☻",$users[$i]["gym_postal_code"]);
    				$users[$i]["gym_city"] = explode("☻☻♥♥☻☻",$users[$i]["gym_city"]);
    				$users[$i]["gym_province"] = explode("☻☻♥♥☻☻",$users[$i]["gym_province"]);
    				$users[$i]["gym_zipcode"] = explode("☻☻♥♥☻☻",$users[$i]["gym_zipcode"]);
					$users[$i]["gym_district"] = explode("☻☻♥♥☻☻",$users[$i]["gym_district"]);
					$users[$i]["gym_country"] = explode("☻☻♥♥☻☻",$users[$i]["gym_country"]);
					$users[$i]["gym_addressline"] = explode("☻☻♥♥☻☻",$users[$i]["gym_addressline"]);
					$users[$i]["gym_gmap"] = explode("☻☻♥♥☻☻",$users[$i]["gym_gmaphtml"]);
					$users[$i]["gym_website"] = explode("☻☻♥♥☻☻",$users[$i]["gym_website"]);
					$users[$i]["gym_status"] = explode("☻☻♥♥☻☻",$users[$i]["gym_status"]);
					$users[$i]["gym_id"] = explode("☻☻♥♥☻☻",$users[$i]["gym_id"]);
				}
       	...
       	if($num_posts > 0){
			...
			for($i=0;$i<$num_posts;$i++){
				...
				$html .='<tr>
					...
					<td>'.$users[$i]["acs_id"].'</td>
					<td>'.$users[$i]["directory"].'</td>
					<td>'.$email.'</td>
					<td>'.$cnumber.'</td>
					<td>'.$users[$i]["date_of_join"].'</td>
					<td class="text-center"><button type="button" class="btn btn-md" id="allGYMuser_'.$users[$i]["usrid"].'" value="'.$users[$i]["usrid"].'" title="All Branch" ><i class="fa fa-th-list fa-1x fa-fw"></i></button></td>
	...
	// list the gym data
	function displayUsrGymData($gymid) {
		require_once(DOC_ROOT.INC.'FirePHPCore/FirePHP.class.php');
			         $console_php = FirePHP::getInstance(true);
		$c=0;
      $gymusers = array();
		...
		if($gymusers != NULL)
       	...
       	if($num_posts > 0){
			...
			//	require_once(DOC_ROOT.INC.'FirePHPCore/FirePHP.class.php');
		//$console_php = FirePHP::getInstance(true);
		//$console_php->log($gymusers[$i]["gym_id"]);
              if(is_array($gymusers[$i]["gym_name"])){
                ...
                for($j=0;$j<(sizeof($gymusers[$i]["gym_name"])-1) && isset($gymusers[$i]["gym_name"][$j]) && $gymusers[$i]["gym_name"][$j] != '';$j++){
					//$console_php->log($gymusers[$i]["gym_id"][$j]);
					//$console_php->log(ltrim($gymusers[$i]["gym_id"][$j] ,','));
					
					$temp=ltrim($gymusers[$i]["gym_id"][$j] ,',');
					//$console_php->log("temp".$temp[0]);
                 // $gymid[$j] = '17';
              //    $console_php->log($gymid[$j]);
             ...
             $datadocumnetgyminfo='<div class="row">
						...
						<div class="row">
							<div class="col-lg-12">
								<strong><span class="text-danger"><i class="fa fa-star fa-fw"></i></span> Branch Type <i class="fa fa-caret-down fa-fw"></i></strong>
							</div>
							<div class="col-lg-12">
						...
						<div class="row">
							<div class="col-lg-12">
								<strong><span class="text-danger"><i class="fa fa-star fa-fw"></i></span> DB Host <i class="fa fa-caret-down fa-fw"></i></strong>
							</div>
							<div class="col-lg-12">
						...
						<div class="row">
							<div class="col-lg-12">
								<strong><span class="text-danger"><i class="fa fa-star fa-fw"></i></span> DB UserName <i class="fa fa-caret-down fa-fw"></i></strong>
							</div>
							<div class="col-lg-12">
						...
						<div class="row">
							<div class="col-lg-12">
								<strong><span class="text-danger"><i class="fa fa-star fa-fw"></i></span> DB Name <i class="fa fa-caret-down fa-fw"></i></strong>
							</div>
							<div class="col-lg-12">
						...
						<div class="row">
							<div class="col-lg-12">
								<strong><span class="text-danger"><i class="fa fa-star fa-fw"></i></span> DB Password <i class="fa fa-caret-down fa-fw"></i></strong>
							</div>
							<div class="col-lg-12">
						...
						<div class="row">
							<div class="col-lg-12">
								<strong><span class="text-danger"><i class="fa fa-star fa-fw"></i></span> Short-Logo Size <i class="fa fa-caret-down fa-fw"></i></strong>
							</div>
							<div class="col-lg-12">
						...
						<div class="row">
							<div class="col-lg-12">
								<strong><span class="text-danger"><i class="fa fa-star fa-fw"></i></span> Header-logo Size <i class="fa fa-caret-down fa-fw"></i></strong>
							</div>
							<div class="col-lg-12">
                    ...
                    <div class="row">
						...
						<div class="row">
							...
							<div class="col-lg-12">
								...
								<p class="help-block" id="gymstlmsg'.$temp.'">Press enter or go button to move to next feild.</p>
							</div>
							<div class="col-lg-12">&nbsp;</div>
						</div>
						<!-- Address Line -->
...
<!-- end_picedit_box -->
	...
	//edit cell number
	function loadClientCellNumForm() {
		$user_id=$this->parameters["uid"]!=""		?	' AND a.`id` LIKE CONCAT('.$this->parameters["uid"].')'	:	'';
		$clientQuery["var"]=$user_id;
		$users = clientadd::mmtable_list($clientQuery);
		$html = '';
