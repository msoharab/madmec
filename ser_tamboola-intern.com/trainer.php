...
class trainer{
	...
	public function AddDummyEmail(){
		...
		// executeQuery('INSERT INTO `dummy_email_ids` (`id`,`user_id`,`status`) VALUES(NULL,\''.$email.'\',\'assigned\');');
		return $email;
	}
	...
	public function trainerPhotoUpload(){
		...
		$obj=new trainer();
		$photo_id=$this->parameters["photo_id"];
			...
			if(get_resource_type($link1) == 'mysql link'){
				if(($db_select = selectDB(DBNAME_ZERO,$link1)) == 1){
					$query='UPDATE `photo` SET `ver2`= \''.mysql_real_escape_string($dbpath).'\'
										WHERE `id` = \''.mysql_real_escape_string($photo_id).'\'';	
	...
	public function displayListTrainer(){
		...
		if($num_posts > 0){
			...
			for($i=0;$i < $num_posts;$i++){
				$html = '<tr>
							...
							<td class="text-center"><button class="btn btn-danger btn-md" id="tra_but_trash_'.$trainer[$i]["id"].'" data-toggle="modal" data-target="#myTRADELModal_'.$trainer[$i]["id"].'" ><i class="fa fa-trash fa-fw "></i> Delete</button>&nbsp;</td>';
							if(($trainer[$i]["status"])=='11'){	
								$html .= '<td class="text-center"><button class="btn btn-primary btn-md" id="tra_but_flag_'.$trainer[$i]["id"].'" data-toggle="modal" data-target="#myModal_flag'.$trainer[$i]["id"].'"><i class="fa fa-flag fa-fw "></i> Flag</button>&nbsp;</td>';
							}else if(($trainer[$i]["status"])=='7') {
								$html.='<td class="text-center"><button class="btn btn-primary btn-md" id="tra_but_unflag_'.$trainer[$i]["id"].'" data-toggle="modal" data-target="#myModal_unflag'.$trainer[$i]["id"].'"><i class="fa fa-flag fa-fw "></i> Unflag</button>&nbsp;</td>';
							}$html.='<td class="text-center"><button class="btn btn-success btn-md" id="tra_but_edit_'.$trainer[$i]["id"].'"><i class="fa fa-edit fa-fw "></i> Edit</button>
							</td>
						</tr>';
													...
													<div class="modal-content" style="color:#000;">
														<div class="modal-header">
															<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
																<h4 class="modal-title" id="myTRADELModalLabel_'.$trainer[$i]["id"].'">Select Cell Numbers to send SMS</h4>
														</div>
		...
		$listtrainer[0]["name"]= $_SESSION["listoftrainers"]["listofPeoples"];
		return $listtrainer;
	}
	public function edittrainer(){
		...
		$trainer_id=$this->parameters["trainerid"]!=""		?	' AND a.`id` LIKE CONCAT('.$this->parameters["trainerid"].')'	:	'';
						...
						<div class="panel panel-primary">
							...
							<div class="panel-body" id="trainer" style="display:block;">
								<ul>
									...
									<li><strong>Date Of Birth : '.$trainer[0]["dob"].'</strong></li>
									<li><strong>Date Of Join : '.$doj[0].'</strong></li>
								</ul>
							</div>
											...
											<div class="col-lg-6">
												<div class="form-group">
													...
													<span>
														...
														<p class="help-block" id="un_msg">Press tab to move to next field.</p>
													</span>
												</div>
											</div>
											<div class="col-lg-6">
												<div class="form-group">
													...
													<span>
														...
														<p class="help-block" id="em_msg">Press tab to move to next field.</p>
													</span>
												</div>
											</div>
										</div>
											...
											<div class="col-lg-6">
												<div class="form-group">
													...
													<span>
														...
														<p class="help-block" id="cell_msg">Press tab to move to next field.</p>
													</span>
												</div>
											</div>
											<div class="col-lg-6">
												<div class="form-group">
													...
													<span>
														...
														<p class="help-block" id="mob_msg">Press tab to move to next field.</p>
													</span>
												</div>
											</div>
										</div>
										...
										<div class="col-lg-12">
											<div class="col-lg-6">
												<div class="form-group">
													...
													<input name="dob" type="text" class="form-control" id="tra_dob" readonly="readonly" value="'.$trainer[0]["dob"].'"/>
											...
											<div class="col-lg-6">
												<div class="form-group">
													...
													<input name="doj" type="text" class="form-control" id="tra_doj" readonly="readonly" value="'.$doj[0].'"/>
													<p class="help-block">Press tab to move to next field.</p>
												</div>
											</div>
										</div>
		...
		echo '<form action="control.php" name="updatePic" id="changePic" method="post" enctype="multipart/form-data">
				 <fieldset>
		...
		$link1 = MySQLconnect(DBHOST,DBUSER,DBPASS);
			if(get_resource_type($link1) == 'mysql link'){
				...
				$query = 'UPDATE `user_profile` SET 
						  ...
						  WHERE `id` = \''.mysql_real_escape_string($this->parameters["uid"]).'\'';
					if(executeQuery($query)){
							$flag=true;
						}	  
				}
	...
	public function deleteTrainer() {
		$flag = false;
		$id = $this->parameters["entry"];
		$query = NULL;
		$obj=new trainer();
		executeQuery("SET AUTOCOMMIT=0;");
		executeQuery("START TRANSACTION;");
		$query = 'UPDATE  `employee` SET `status`=12 WHERE `id` = "'.mysql_real_escape_string($id).'";';
		if(executeQuery($query)){
			executeQuery("COMMIT;");
			if($obj->deleteMasterTrainer($id)){
				$flag = true;
			}
		}
		return $flag;
		...
		$link1 = MySQLconnect(DBHOST,DBUSER,DBPASS);
			if(get_resource_type($link1) == 'mysql link'){
				if(($db_select = selectDB(DBNAME_ZERO,$link1)) == 1){
					executeQuery("SET AUTOCOMMIT=0;");
					executeQuery("START TRANSACTION;");
					$query = 'UPDATE  `user_profile` SET `status`=12 WHERE `id` = "'.mysql_real_escape_string($id).'";';
					if(executeQuery($query)){
						$flag = true;
						executeQuery("COMMIT;");
					}
	...
	public function unflagTrainer() {
		...
		executeQuery("SET AUTOCOMMIT=0;");
		executeQuery("START TRANSACTION;");
		$query = 'UPDATE  `employee` SET `status`=11 WHERE `id` = "'.mysql_real_escape_string($id).'";';
				if(executeQuery($query)){
					executeQuery("COMMIT");
					if($obj->unflagMasterTrainer($id)){
						$flag = true;
					}
				}
		return $flag;
		...
		if(get_resource_type($link) == 'mysql link'){
			if(($db_select = selectDB(DBNAME_ZERO,$link)) == 1){
				...
				if($total){
					...
					$console_php->log($query1);
					$console_php->log($query3);
					$console_php->log($query4);
					if(executeQuery($query1)){
						$res = executeQuery($query);
						if($res){
							//executeQuery($query0);
