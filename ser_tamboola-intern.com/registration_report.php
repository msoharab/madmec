			...
			if($query){
						...
						if(mysql_num_rows($res)){
							...
							while($row = mysql_fetch_assoc($res)){
								...
								$feefields[$i]['user_id'] = $row['user_id'];									/* C */
				...
				if($feefields != NULL){
					...
					*/
					...
					$objPHPExcel->getActiveSheet()
								...
								->setCellValueByColumnAndRow(5,($i+5),($ptotal + $ototal))
								->setCellValueByColumnAndRow(4,($i+6),'Due')
								->setCellValueByColumnAndRow(5,($i+6),($atotal - ($ptotal + $ototal)));
				}
			}
		}
		return $ptotal;
		...
		if($fromdate != 0 && $todate != 0){
			...
			if($fromdate < $todate && $fromdate != 0 && $todate != 0){
				$query = 'SELECT 
							...
							a.`customer_pk`,
							CONCAT(\'+91 \',b.`cell_number`) AS cell,
							c.`package_type_id` AS `pack_name`,
						...
						LEFT JOIN (
							SELECT 
								...
								due.`due_status`
							FROM `money_transactions` AS mtr
							LEFT JOIN `packages` AS pack ON  mtr.`transaction_type` LIKE CONCAT(\'%\',pack.`package_type_id`,\' - \',pack.`number_of_sessions`,\' - \',pack.`cost`,\'%\')
							LEFT JOIN (
								SELECT 
									GROUP_CONCAT(`customer_pk`) AS inv_users,
									GROUP_CONCAT(`location`) AS inv_urls,
								...
								FROM `invoice`
								GROUP BY(`transaction_id`)
							) AS inv ON inv.`transaction_id` = mtr.`id` AND inv.`name` LIKE CONCAT(\'%\',pack.`package_type_id`,\' - \',pack.`number_of_sessions`,\' - \',pack.`cost`,\'%\')
							LEFT JOIN (
								SELECT 
									`id`,
									`money_trans_id`,
							...
							WHERE mtr.`transaction_type` LIKE CONCAT(\'%\',pack.`package_type_id`,\' - \',pack.`number_of_sessions`,\' - \',pack.`cost`,\'%\')
							GROUP BY(mtr.`receipt_no`)
							ORDER BY(mtr.`id`)
						) AS mo_tr
							ON STR_TO_DATE(a.`payment_date`, \'%Y-%m-%d\') = STR_TO_DATE(mo_tr.`mt_pod`, \'%Y-%m-%d\')
							AND a.`id` = mo_tr.`action_id`
							AND a.`customer_pk` = mo_tr.`mt_uid`
						WHERE mo_tr.`mt_tt` = CONCAT(c.`package_type_id`,\' - \',c.`number_of_sessions`,\' - \',c.`cost`)
			...
			echo '<center><h2>Invalid Date Range!!!!!</h2></center>';
		}
	}
			...
			if($fromdate < $todate && $fromdate != 0 && $todate != 0){
				...
				if($onedate)
					$query = 'SELECT 
								...
								a.`user_id`,
								CONCAT(\'+91 \',b.`cell_number`) AS cell,
								c.`package_type` AS `pack_name`,
								c.`number_of_sessions` AS `sessions`,
								STR_TO_DATE(a.`payment_date`, \'%Y-%m-%d\') AS `payment_date`,
								CONCAT(c.`package_type`,\' - \',c.`number_of_sessions`,\' - \',c.`cost`) AS tran_type,
								a.`amount` AS oldfee,
								a.`receipt_no`,
								c.`cost` AS actfee,
								mo_tr.`mt_pk`,
								...
								mo_tr.`due_status`
							FROM `fee_packages` AS a
							INNER JOIN `users` AS b ON a.`user_id` = b.`email_id`
							INNER JOIN `packages` AS c ON a.`package_id` = c.`id`
							LEFT JOIN (
								SELECT 
									mtr.`id`  					AS mt_pk,
									mtr.`user_id` 				AS mt_uid,
									GROUP_CONCAT(mtr.`mop_id`) 	AS mopid,
									mtr.`pay_date`  			AS mt_pod,
									mtr.`transaction_type`  	AS mt_tt,
									mtr.`receipt_no`  			AS mt_rpt,
									...
									due.`due_status`
								FROM `money_transactions` AS mtr
								LEFT JOIN `packages` AS pack ON  mtr.`transaction_type` LIKE CONCAT(\'%\',pack.`package_type`,\' - \',pack.`number_of_sessions`,\' - \',pack.`cost`,\'%\')
								LEFT JOIN (
									SELECT 
										GROUP_CONCAT(`user_id`) AS inv_users,
										GROUP_CONCAT(`location`) AS inv_urls,
										`transaction_id`,
										`name`
									FROM `invoice`
									GROUP BY(`transaction_id`)
								) AS inv ON inv.`transaction_id` = mtr.`id` AND inv.`name` LIKE CONCAT(\'%\',pack.`package_type`,\' - \',pack.`number_of_sessions`,\' - \',pack.`cost`,\'%\')
								LEFT JOIN (
									SELECT 
										`id`,
										`money_trans_id`,
										...
										`status`  AS due_status
									FROM `money_trans_due`
								) AS due ON due.`money_trans_id` = mtr.`id` AND due.`due_user` = mtr.`user_id`
								WHERE mtr.`transaction_type` LIKE CONCAT(\'%\',pack.`package_type`,\' - \',pack.`number_of_sessions`,\' - \',pack.`cost`,\'%\')
								GROUP BY(mtr.`receipt_no`)
								ORDER BY(mtr.`id`)
							) AS mo_tr
								ON STR_TO_DATE(a.`payment_date`, \'%Y-%m-%d\') = STR_TO_DATE(mo_tr.`mt_pod`, \'%Y-%m-%d\')
								AND a.`id` = mo_tr.`action_id`
								AND a.`user_id` = mo_tr.`mt_uid`
							WHERE mo_tr.`mt_tt` = CONCAT(c.`package_type`,\' - \',c.`number_of_sessions`,\' - \',c.`cost`)
							AND STR_TO_DATE(a.`payment_date`, \'%Y-%m-%d\') = STR_TO_DATE(\''.$onedate.'\', \'%Y-%m-%d\');';			
			}
			if($query){
				$res = executeQuery($query);
