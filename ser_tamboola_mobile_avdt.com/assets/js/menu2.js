function menu2()
{
    ...
    this.__construct = function (attrctrl) {
        ...
        window.setTimeout(function () {
            ...
            $(ctrl.listgyms.listgymsmenubut).click(function () {
                fetchListOFGyms();
            });
        }, 400);
    }
            ...
            $.ajax({
                ...
                success: function (data, textStatus, xhr) {
                    ...
                    switch (data) {
                        ...
                        default:
                            $(loader).hide();
                            $(ctrl.msgDiv).html('<h2>Record success fully added</h2>');
...
/*			}*/
    ...
    ;
    /*Fetching List of Gyms*/
    function fetchListOFGyms()
    {
        $(ctrl.listgyms.dislistofgyms).html(LOADER_ONE);
        $.ajax({
            url: ctrl.url,
            type: 'POST',
            data: {autoloader: true, action: 'fetchlistgyms', type: 'master'},
            success: function (data, textStatus, xhr) {
                console.log(data);
                data = $.trim(data);
                console.log(xhr.status);
                switch (data) {
                    ...
                    default:
                        ...
                        {
                            $(ctrl.listgyms.dislistofgyms).html('<table  class="table table-striped table-bordered table-hover" id="listofgym-datable"><thead><th>#</th><th>Gym Details</th></thead><tbody>' + det.data + '</tbody></table>');
                            window.setTimeout(function (){
                               $('#listofgym-datable').dataTable() ;
                            });
                        }
                        else
                        {
                            $(ctrl.listgyms.dislistofgyms).html('<span class="text-danger"><strong>no record found</strong></span>');
                        }
                        break;
                }
            },
