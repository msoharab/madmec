<?php
class Gym_Model extends BaseModel {
    ...
    public function ListGym() {
        ...
        $query = 'SELECT
         ...
         LEFT JOIN `gym_email_ids` AS ue ON ue.`user_pk` = ad.`id`
            ...
            if ($num_posts) {
                ...
                for ($i = 0; $i < count($result); $i++) {
                    array_push($data, array(
                        ...
                        "View" => '<a href="' . $this->config["URL"] . $this->config["CTRL_22"] . 'ViewGym/' . base64_encode($result[$i]["gymid"]) . '" target="_self" class="btn btn-block btn-info">View<a>',
                        "Edit" => '<a href="' . $this->config["URL"] . $this->config["CTRL_22"] . 'EditGym/' . base64_encode($result[$i]["gymid"]) . '" target="_self" class="btn btn-block btn-warning">Edit<a>',
                        "Delete" => '<a href="' . $this->config["URL"] . $this->config["CTRL_22"] . 'DeleteGym/' . base64_encode($result[$i]["gymid"]) . '" target="_self" class="btn btn-block btn-danger">Delete<a>'
                    ));
                }
    ...
    public function EditGym() {
        $jsondata = array(
            "id" => base64_decode($this->postBaseData["id"]),
            "gymtype" => $this->postBaseData["gymtype"],
            "gymname" => $this->postBaseData["gymname"],
            "regisfee" => $this->postBaseData["regisfee"],
            "servtax" => $this->postBaseData["servtax"],
        ...
        $res3 = $stm3->execute(array(
            ":id1" => mysql_real_escape_string($jsondata["telecode"]),
            ":id2" => mysql_real_escape_string($jsondata["telephone"]),
