<?php
	...
	//Customer Classes
	require_once(DOC_ROOT.CLASSES.CUSTOMER.CUST_SEARCG_GYM);
			...
			if (($db_select = selectDB(DBNAME_ZERO, $link)) == 1) {
				if (!ValidateAdmin()) {
					...
					switch ($parameters["action"]) {
                        ...
                        $enq = new enquiry_su();
                        echo json_encode($enq->SentCredentials());
                        break;
                    case 'sendCredential' :
                        $enq = new enquiry_su($_POST);
                        echo json_encode($enq->sendCredential());
                        break;
						case "UpdateFollowUp": {
                            $follow_cmt = array(
	...
	if ((isset($_SESSION["USER_LOGIN_DATA"]["USER_TYPE"]))) {
		switch($_SESSION["USER_LOGIN_DATA"]["USER_TYPE"]){
			...
			case 'Customer':{
				require_once(DOC_ROOT . INC . 'res-customer-header.php');
				break;
			}
			case 'MMAdmin':{
				require_once(DOC_ROOT . INC . 'superadmin.php');
				break;
			}
			default:{
				session_destroy();
