	...
	public function accountStats(){
		...
		if($num){
			for($j=0;$j<sizeof($facility_type);$j++){
				...
				$total1 = 0;
				$temp = $Fee;
				$Fee .=  "<tr><td colspan='7' align='center'><h4>".$facility_type[$j]."</h4></td></tr>
			...
			$Fee .= "<tr><td  colspan='8'><h3>Today's collection is 0 Rs</h3></td></tr>";
		}
		$query ='SELECT
			a.`id`,
			a.`name`,
		...
		if($num){
			for($j=0;$j<sizeof($reg_type);$j++){
				...
				$Fee .= "<tr><td colspan='7' ><h4>".$reg_type[$j]."</h4></td></tr>
			...
			$Fee .= "<tr><td  colspan='5'><h3>Today's Group Registration collection is 0 Rs</h3></td></tr>";
		}
		$Fee .= "</table>";
