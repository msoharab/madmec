<?php
	class addcustomer {
		...
		public function customerPhotoUpload() {
			...
			if ($uploadOk == 0) {
				...
				if (move_uploaded_file($_FILES["file"]["tmp_name"], $fname)) {
					...
					$obj->updateCUSTMasterPhoto($dbpath, $photo_id);
					} else {
					echo "Sorry, there was an error uploading your file.";
				}
			}
                ...
                if (executeQuery($query1)) {
                    ...
                    if (executeQuery($query2)) {
                        ...
                        if (executeQuery($query3)) {
							...
							$filedirectory = DOC_ROOT . ASSET_DIR . $data["directory"] . '/profile/' . $filename;
							$invloc=ASSET_DIR . $data["directory"] . '/profile/' . $filename;
							$urlpath = URL . ASSET_DIR . $data["directory"] . '/profile/' . $filename;
		...
		public function list_cust($data, $tabid) {
			...
			$ltcust = new addcustomer();
			$ltcust->ListOutCust($listusers, $data, $tabid);
		}
		public function ListOutCust($listusers, $data, $tabid) {
			...
			-->
			<th>DOJ</th>
			<th class="text-center">Edit - Delete - Flag/Unflag</th>
			</tr></thead>');
			$name = '';
			$email = '';
			$totalname = 0;
			...
			for ($i = 0; $i < $total; $i++) {
				...
				<td class="text-center"><button type="button" id="listEDITcust_' . $i . '" title="Edit" ><i class="fa fa-edit fa-fw"></i></button> - <button type="button" class="btn btn-md" id="listDELcust_' . $i . '" title="Delete" data-toggle="modal" data-target="#myUSRLISTDELModal_' . $i . '"><i class="fa fa-trash-o fa-fw"></i></button> - '.$flagbut.'</td>
				</tr>
		...
		public function editCustomerList($fid, $id, $tabid) {
			...
			THEN "' . USER_ANON_IMAGE . '"
			ELSE CONCAT("' . URL . ASSET_DIR . '",a.`ver2`)
			END AS photo,
			a.`master_pk`,
			a.`name`,
			a.`email`,
			...
			//  $cust = array();
			//  $cust=addcustomer::list_cust($fid,$cid);
			$res = executeQuery($query);
			if (mysql_num_rows($res)) {
				$i = 0;
				while ($row1 = mysql_fetch_assoc($res)) {
			...
			<button type="button" class="btn btn-danger btn-md" id="subcustinfo_but_edit_' . $temp . '">
			...
			<div class="panel-footer">
			<button class="btn btn-info " id="usrphoto_but_edit_' . $listusers[0]["id"] . '" data-toggle="modal" data-target="#myModal_Photo1"><i class="fa fa-edit fa-fw"></i> Edit</button>
			</div>
			</div>
			</div>
			<div class="col-lg-4">
		...
		public function editslaveListCust($data) {
			...
			$upmc = new addcustomer();
			$upmc->updateMasterCustomer($data);
			return $updateAdd;
		}
		...
		public function editCustomerlistgrp($fid, $cid, $tabid, $i, $j, $mtid) {
			...
			<div class="panel-footer">
			<button type="button" class="btn btn-danger btn-md" id="subcustinfo_but_edit_cgp' . $temp . '">
			<i class="fa fa-edit fa-fw "></i> Edit
			</button>
			</div>
			</div>
			...
			if (isset($listinfocustgp[$i]["fee"]["offer_name"][$j])) {
				...
				if ($count > 0) {
					for ($l = 0; $l < $count && isset($listinfocustgp[0]["fee"]["offer_name"][$l]) && $listinfocustgp[0]["fee"]["offer_name"][$l] != ''; $l++) {
						$offerdata .= '<tr><td>' . ($l + 1) . '</td><td>' . trim($listinfocustgp[0]["fee"]["offer_name"][$l], ',') . '</td><td>' . trim($listinfocustgp[0]["fee"]["duration"][$l], ',') . '</td><td>' . trim($listinfocustgp[0]["fee"]["payment_date"][$l], ',') . '</td><td>' . trim($listinfocustgp[0]["fee"]["valid_from"][$l], ',') . '</td><td>' . trim($listinfocustgp[0]["fee"]["valid_till"][$l], ',') . '</td></tr>';
					}
				}
			}
			// package data
			...
			if (isset($listinfocustgp[$i]["fee_package"]["package_type"][$j])) {
				...
				if ($count1 > 0) {
					for ($l = 0; $l < $count1 && isset($listinfocustgp[0]["fee_package"]["id"][$l]) && $listinfocustgp[0]["fee_package"]["id"][$l] != ''; $l++) {
						$packdata .= '<tr><td>' . ($l + 1) . '</td><td>' . trim($listinfocustgp[0]["fee_package"]["package_type"][$l], ',') . '</td><td>' . trim($listinfocustgp[0]["fee_package"]["num_sessions"][$l], ',') . '</td><td>' . trim($listinfocustgp[0]["fee_package"]["payment_date"][$l], ',') . '</td></tr>';
					}
				}
			}
			//  attendance data
			...
			//  transaction data
			$accountdata = ' ';
			if (isset($listinfocustgp[$i]["accounts"]["inv_tt"][$j])) {
				...
				if (isset($listinfocustgp[$i]["accounts"]["mt_uid"][$j])) {
					...
					if ($count3 > 0) {
						for ($j = 0; $j < $count3; $j++) {
							if ($j % 2 != 0)
                            $accountdata .= '<tr><td>' . ($j + 1) . '</td><td>' . $listinfocustgp[0]['accounts']['inv_tt'][$j] . '</td><td><a href=' . $listinfocustgp[0]['accounts']['inv_urls'][$j] . '>' . $listinfocustgp[0]['accounts']['mt_rpt'][$j] . '</a></td><td>' . $listinfocustgp[0]['accounts']['mt_pod'][$j] . '</td><td>' . trim($listinfocustgp[0]['accounts']['mop'][$j], ',') . '</td><td><font color=red>' . $listinfocustgp[0]['accounts']['due_amount'][$j] . '</font></td><td>' . $listinfocustgp[0]['accounts']['due_date'][$j] . '</td><td>' . $listinfocustgp[0]['accounts']['due_status'][$j] . '</td></tr>';
							else
                            $accountdata .= '<tr><td>' . ($j + 1) . '</td><td>' . $listinfocustgp[0]['accounts']['inv_tt'][$j] . '</td><td><a href=' . $listinfocustgp[0]['accounts']['inv_urls'][$j] . '>' . $listinfocustgp[0]['accounts']['mt_rpt'][$j] . '</a></td><td>' . $listinfocustgp[0]['accounts']['mt_pod'][$j] . '</td><td>' . trim($listinfocustgp[0]['accounts']['mop'][$j], ',') . ')' . '</td><td><font color=red>' . $listinfocustgp[0]['accounts']['due_amount'][$j] . '</font></td><td>' . $listinfocustgp[0]['accounts']['due_date'][$j] . '</td><td>' . $listinfocustgp[0]['accounts']['due_status'][$j] . '</td></tr>';
						}
					}
			...
			echo '<div class="panel-group" id="accorlistcgp' . $temp . '">
			<div class="panel panel-default panel-success" id="subusercgp_' . $temp . '">
			<div class="panel-heading">
			<div class="row">
			<div class="col-md-6">
			...
			<div id="subuser_list_cgp' . $temp . '" class="panel-collapse collapse">
			<ul class="nav nav-pills">
			<div class="col-lg-12">&nbsp;</div>
			...
			<div class="tab-content">
			<div class="tab-pane fade in active" id="info_subuser_list_cgp' . $temp . '">
			<p>' . str_replace($this->order, $this->replace, $personalinfo) . '</p>
			</div>
			<div class="tab-pane fade" id="offer_subuser_list_cgp' . $temp . '">
			</div>
			<div class="tab-pane fade" id="package_subuser_list_cgp' . $temp . '">
			</div>
			<div class="tab-pane fade" id="attendance_subuser_list_cgp' . $temp . '">
			<div class="row"><center><div class="clo-lg-12">&nbsp;</div><div class="col-lg-12">' . $attendance_html . '</div><div class="col-lg-12">&nbsp;</div></center></div>
			</div>
			<div class="tab-pane fade" id="transaction_subuser_list_cgp' . $temp . '">
			</div>
			</div>
			</div>
			</div>
		...
		public function list_group($fid, $tabid) {
			...
			$ltgp = new addcustomer();
			$ltgp->ListofAllGroup($groupslist, $fid, $tabid);
		}
		public function ListofAllGroup($groupslist, $fid, $tabid) {
			...
			if ($groupslist != null) {
				...
				for ($i = 0; $i < $total; $i++) {
					...
					<td class="text-center"><button type="button" id="listGROUP_' . $i . '" title="List" ><i class="fa fa-list fa-fw"></i></button></td>
					<td class="text-center"><button type="button" class="btn btn-md" id="listDELgrp_' . $i . '" title="Delete" data-toggle="modal" data-target="#myUSRLISTDELModal_' . $i . '"><i class="fa fa-trash-o fa-fw"></i></button></td>';
					if (($groupslist[$i]["status"]) != 7) {
						echo '<td class="text-center"><button type="button" class="btn btn-success btn-md" id="listFLAGgrp_' . $i . '" title="Flag" data-toggle="modal" data-target="#myModal_flag' . $i . '"><i class="fa fa-flag-o fa-fw"></i>Flag</button></td>';
						} else if (($groupslist[$i]["status"]) == 7) {
						echo '<td class="text-center"><button class="btn btn-danger btn-md" id="listUNFLAGgrp_' . $i . '" data-toggle="modal" data-target="#myModal_unflag' . $i . '"><i class="fa fa-flag-o fa-fw"></i>Unflag</button></td>';
					}
					echo '</tr>';
					echo '<div class="modal fade" id="myUSRLISTDELModal_' . $i . '" tabindex="-1" role="dialog" aria-labelledby="myUSRLISTDELModal_' . $i . '" aria-hidden="true" style="display: none;">
					<div class="modal-dialog">
					<div class="modal-content" style="color:#000;">
					<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			...
			//  else{
			//  echo '<tr><td colspan="10"><strong><center><font color="red">No Group Available</font></center></strong></td></tr>';
			//  }
		}
						...
						if (isset($guserlist[$i]["grcust"]["grcust_pk"][0])) {
							for ($j = 0; $j < sizeof($guserlist[$i]["grcust"]["grcust_pk"]) && isset($guserlist[$i]["grcust"]["grcust_pk"][$j]) && $guserlist[$i]["grcust"]["grcust_pk"][$j] != ''; $j++) {
								...
								if ($j != 0) {
									...
									<td class="text-center"><button type="button" id="listcustEDITgrp_' . $j . '" title="Edit" ><i class="fa fa-edit fa-fw"></i></button></td>
									<td class="text-center"><button type="button" class="btn btn-md" id="listcustDELgrp_' . $j . '" title="Delete" data-toggle="modal" data-target="#myUSRLISTGRPDELModal_' . $j . '"><i class="fa fa-trash-o fa-fw"></i></button></td>');
									if ($status == 2) {
										echo '<td class="text-center"><button type="button" class="btn btn-success btn-md" id="listcustFLAGgrp_' . $j . '" title="Flag" data-toggle="modal" data-target="#myModal_flag' . $j . '"><i class="fa fa-flag-o fa-fw"></i>Flag</button></td>';
										} else if ($status == 7) {
										echo '<td class="text-center"><button class="btn btn-danger btn-md" id="listcustUNFLAGgrp_' . $j . '" data-toggle="modal" data-target="#myModal_unflag' . $j . '"><i class="fa fa-flag-o fa-fw"></i>Unflag</button></td>';
									}
									echo '</tr>';
									} else {
									echo str_replace($this->order, $this->replace,'<tr style="display:none">
									<td>' . ($j - 1) . '</td>
									<td>' . $name . '</td>
									<td>' . $email . '</td>
									<td>' . $cell . '</td>
									...
									<td>' . date('d-M-Y h:i:s A', strtotime($doj)) . '</td>
									<td class="text-center"><button type="button" id="listcustEDITgrp_' . $j . '" title="Edit" ><i class="fa fa-edit fa-fw"></i></button></td>
									<td class="text-center"><button type="button" class="btn btn-md" id="listcustDELgrp_' . $j . '" title="Delete" data-toggle="modal" data-target="#myUSRLISTGRPDELModal_' . $j . '"><i class="fa fa-trash-o fa-fw"></i></button></td>');
									if ($status == 2) {
										echo '<td class="text-center"><button type="button" class="btn btn-success btn-md" id="listcustFLAGgrp_' . $j . '" title="Flag" data-toggle="modal" data-target="#myModal_flag' . $j . '"><i class="fa fa-flag-o fa-fw"></i>Flag</button></td>';
								...
								echo str_replace($this->order, $this->replace,'<div style="display:none" class="modal fade" id="myUSRLISTGRPDELModal_' . $j . '" tabindex="-1" role="dialog" aria-labelledby="myUSRLISTDELModal_' . $j . '" aria-hidden="true" style="display: none;">
								<div class="modal-dialog">
								<div class="modal-content" style="color:#000;">
								<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
		...
		public function addGroup() {
			$flag = false;
			$obj = new addcustomer($this->parameters);
			$gpdata = NULL;
			for ($i = 1; $i <= $this->parameters["num"]; $i++) {
				$gpdata[$i]["max_mop"] = $this->parameters["gpdata"][$i]["max_mop"];
				$gpdata[$i]["amount"] = $this->parameters["gpdata"][$i]["amount"];
				$gpdata[$i]["sum_amount"] = $this->parameters["gpdata"][$i]["sum_amount"];
				$gpdata[$i]["mod_pay"] = $this->parameters["gpdata"][$i]["mod_pay"];
				$gpdata[$i]["transaction_number"] = isset($this->parameters["gpdata"][$i]["transaction_number"]) ? $this->parameters["gpdata"][$i]["transaction_number"] : '';
				$gpdata[$i]["groupname"] = $this->parameters["gpdata"][$i]["groupname"];
				$gpdata[$i]["groupemail"] = $this->parameters["gpdata"][$i]["groupemail"];
				$gpdata[$i]["name"] = $this->parameters["gpdata"][$i]["name"];
				$gpdata[$i]["email"] = $this->parameters["gpdata"][$i]["email"];
				$gpdata[$i]["acs_id"] = $this->parameters["acsid"];
				$gpdata[$i]["auth"] = $this->parameters["auth"];
				$gpdata[$i]["cellcode"] = $this->parameters["gpdata"][$i]["cellcode"];
				$gpdata[$i]["cellnum"] = $this->parameters["gpdata"][$i]["cellnum"];
				$gpdata[$i]["dob"] = $this->parameters["gpdata"][$i]["dob"];
				$gpdata[$i]["sex_type"] = $this->parameters["gpdata"][$i]["sex_type"];
				$gpdata[$i]["doj"] = $this->parameters["gpdata"][$i]["doj"];
				$gpdata[$i]["occu"] = $this->parameters["gpdata"][$i]["occu"];
				$gpdata[$i]["emnm"] = $this->parameters["gpdata"][$i]["emnm"];
				$gpdata[$i]["emnum"] = $this->parameters["gpdata"][$i]["emnum"];
				$gpdata[$i]["addr"] = $this->parameters["gpdata"][$i]["addr"];
				$gpdata[$i]["town"] = $this->parameters["gpdata"][$i]["town"];
				$gpdata[$i]["city"] = $this->parameters["gpdata"][$i]["city"];
				$gpdata[$i]["district"] = $this->parameters["gpdata"][$i]["district"];
				$gpdata[$i]["province"] = $this->parameters["gpdata"][$i]["province"];
				$gpdata[$i]["country"] = $this->parameters["gpdata"][$i]["country"];
				$gpdata[$i]["company"] = $this->parameters["gpdata"][$i]["company"];
				$gpdata[$i]["gtype"] = $this->parameters["gpdata"][$i]["gtype"];
				$gpdata[$i]["pass"] = $this->parameters["pass"];
				$gpdata[$i]["GYM_HOST"] = $this->parameters["GYM_HOST"];
				$gpdata[$i]["GYM_USERNAME"] = $this->parameters["GYM_USERNAME"];
				$gpdata[$i]["GYM_DB_PASSWORD"] = $this->parameters["GYM_DB_PASSWORD"];
				$gpdata[$i]["GYM_DB_NAME"] = $this->parameters["GYM_DB_NAME"];
			}
			if ($gpdata) {
				if ($obj->AddBulkGroup($gpdata)) {
					$flag = true;
				}
			}
		}
		public function AddBulkGroup($fields) {
			$flag = false;
			$query = false;
			$obj = new addcustomer($this->parameters);
			$total = sizeof($fields);
			...
			if (get_resource_type($link) == 'mysql link') {
				if (($db_select = selectDB(DBNAME_ZERO, $link)) == 1) {
					...
					$active = getStatusId("active");
					$show = getStatusId("show");
					$registered = getStatusId("registered");
					if ($total != 0) {
						...
						for ($i = 1; $i <= $total; $i++) {
							...
							if ($i == $total) {
								//  $query .= '(NULL,\''.mysql_real_escape_string($fields['NAME'][$k]).'\',\''.mysql_real_escape_string($fields['EMAIL'][$k]).'\',\''.mysql_real_escape_string($fields['ACS_ID'][$k]).'\',\''.mysql_real_escape_string($password[$k]).'\',\''.mysql_real_escape_string($pass).'\',\''.mysql_real_escape_string($fields['GENDER'][$k]).'\',\''.mysql_real_escape_string($fields['MOBILE'][$k]).'\',default,NULL,\''.mysql_real_escape_string($curr_time).'\',NULL,\''.mysql_real_escape_string($fields['TRAINERGYM'][$k]).'\',\''.mysql_real_escape_string($fields['TRAINERAER'][$k]).'\',\''.mysql_real_escape_string($fields['TRAINERDAN'][$k]).'\',\''.mysql_real_escape_string($fields['TRAINERYOG'][$k]).'\',\''.mysql_real_escape_string($fields['TRAINERZUM'][$k]).'\');';
								$query .= '(NULL,
								\'' . mysql_real_escape_string($fields[$k]['name']) . '\',
								\'' . mysql_real_escape_string($fields[$k]['email']) . '\',
								\'' . mysql_real_escape_string($acsid) . '\',
								\'' . mysql_real_escape_string(($photo_pk + $k)) . '\',
								\'' . mysql_real_escape_string($directory_customer) . '\',
								\'' . mysql_real_escape_string($pass) . '\',
								\'' . mysql_real_escape_string($apass) . '\',
								\'' . mysql_real_escape_string($fields[$k]['sex_type']) . '\',
								\'' . mysql_real_escape_string($fields[$k]['dob']) . '\',
								\'' . mysql_real_escape_string($fields[$k]['cellcode']) . '\',
								\'' . mysql_real_escape_string($fields[$k]['cellnum']) . '\',
								\'' . mysql_real_escape_string($registered) . '\',
								NULL,
								\'' . mysql_real_escape_string($fields[$k]['doj']) . '\');';
						...
						if (executeQuery($query1)) {
							...
							if ($res) {
								...
								if ($res) {
									...
									if ($res) {
										$obj->slaveAddBulkGroup($data, $fields);
										$flag = true;
		...
		public function slaveAddBulkGroup($data, $fields) {
			$flag = false;
			$query = false;
			$obj = new addcustomer($this->parameters);
			$gpdata = array(
			...
			$total = sizeof($fields);
			$k = 1;
			$link = MySQLconnect($fields[$k]['GYM_HOST'], $fields[$k]['GYM_USERNAME'], $fields[$k]['GYM_DB_PASSWORD']);
			if (get_resource_type($link) == 'mysql link') {
				if (($db_select = selectDB($fields[$k]['GYM_DB_NAME'], $link)) == 1) {
					$password = array();
					$show = getStatusId("show");
					$registered = getStatusId("registered");
					$joined = getStatusId("joined");
					...
					if ($total != 0) {
						$query1 = 'SELECT `id` FROM `photo` ORDER BY `id` DESC LIMIT 1';
						$photo_pk = mysql_result(executeQuery($query1), 0);
						$directory_customer = createdirectories(substr(md5(microtime()), 0, 6) . '_dummycust_' . ($data[$k - 1]["user_id"]));
						$dummyQr = 'INSERT INTO  `customer` (`id`,`name`,`email`,`directory`,`status`) VALUES(NULL,\'' . $fields[$k]["groupname"] . '\',\'' . $fields[$k]["groupemail"] . '\',\'' . $directory_customer . '\',1)';
						$dmres = executeQuery($dummyQr);
						$query2 = 'SELECT `id` FROM `customer` ORDER BY `id` DESC LIMIT 1';
						$user_pk = mysql_result(executeQuery($query2), 0);
						$owner = $user_pk;
						$email = $fields[$k]["groupemail"];
						$query4 = 'SELECT `id` FROM `money_transactions` ORDER BY `id` DESC LIMIT 1';
						$mey_pk = mysql_result(executeQuery($query4), 0);
						$money_pk = $mey_pk;
						$query5 = 'INSERT INTO `money_transactions` (`id`, `mop_id`, `pay_date`, `transaction_type`, `receipt_no`, `transaction_id`, `transaction_number`, `total_amount`, `customer_pk`, `status`) VALUES';
						for ($i = 1; $i <= $fields[1]["max_mop"]; $i++) {
							if ($i == $fields[1]["max_mop"]) {
								$query5 .='(NULL,
								\'' . mysql_real_escape_string($fields[$i]['mod_pay'][$i]) . '\',
								\'' . mysql_real_escape_string($curr_time) . '\',
								\'Registration\',
								\'00000055\',
								NULL,
								\'' . mysql_real_escape_string($fields[$i]['transaction_number']) . '\',
								\'' . mysql_real_escape_string($fields[$i]['amount'][$i]) . '\',
								' . $owner . ',
								\'' . mysql_real_escape_string($show) . '\');';
								} else {
								$query5 .='(NULL,
						...
						' . $owner . ',
						\'' . mysql_real_escape_string($show) . '\')';
						//  all querry for customer save entry
						...
						for ($i = 1; $i <= $total; $i++) {
							if ($i == $total) {
								...
								\'' . mysql_real_escape_string($fields[$k]['name']) . '\',
								\'' . mysql_real_escape_string($fields[$k]['email']) . '\',
								\'' . mysql_real_escape_string($data[$i - 1]["acsid"]) . '\',
								\'' . mysql_real_escape_string(($photo_pk + $k)) . '\',
								\'' . mysql_real_escape_string($fields[$k]['cellcode']) . '\',
								\'' . mysql_real_escape_string($fields[$k]['cellnum']) . '\',
						...
						if (executeQuery($query5)) {
							if (executeQuery($query6)) {
								if (executeQuery($query1)) {
									$res = executeQuery($query);
									if ($res) {
										$obj->slaveAddmemberGroup($gpdata, $fields);
										$flag = true;
									}
								}//  1
							}//  4
		...
		public function slaveAddmemberGroup($data, $fields) {
			$flag = false;
			$query = false;
			$fee = 500;
			$total = sizeof($fields);
			...
			if ($total != 0) {
				...
				`customer_pk`) VALUES
				(NULL,
				\'' . mysql_real_escape_string($fields[1]['groupemail']) . '\',
				\'' . mysql_real_escape_string($data[0]["email"]) . '\',
				' . $fee . ',
				' . $total . ',
				\'' . mysql_real_escape_string($fields[1]['gtype']) . '\',
				\'' . mysql_real_escape_string($show) . '\',
				\'' . mysql_real_escape_string($data[0]["owner_id"]) . '\'
				)';
			}
			if (executeQuery($query)) {
				$query1 = 'SELECT `id` FROM `groups` ORDER BY `id` DESC LIMIT 1';
				$group_pk = mysql_result(executeQuery($query1), 0);
				$dummyQr = 'INSERT INTO  `group_members` (`id`,`group_id`,`customer_pk`,`status`)
				VALUES(NULL,\'' . mysql_real_escape_string($group_pk) . '\',\'' . mysql_real_escape_string($data[0]["owner_id"]) . '\',2)';
				$dmres = executeQuery($dummyQr);
				$query2 = 'INSERT INTO `group_members` (`id`,`group_id`,`date_of_join`,`customer_pk`,`status`) VALUES';
				$k = 1;
				for ($i = 1; $i <= $total; $i++) {
					if ($i == $total) {
						$query2 .= '(NULL,
						...
						\'' . mysql_real_escape_string($fields[$k]['doj']) . '\',
						\'' . mysql_real_escape_string($data[$i - 1]["cust_id"]) . '\',
		...
		public function listRegCust($tabId) {
			...
			/*
			$total = sizeof($listusers);
			echo str_replace($this->order, $this->replace, '<table class="table table-striped table-bordered table-hover" cellpadding="0" cellspacing="0" id="listREGcusttabl">
			<thead>
			<tr>
			<th>No</th>
			<th>Customer Name</th>
			...
			for ($i = 0; $i < $total; $i++) {
				...
				<td>' . $listusers[$i]["occupation"] . '</td>';
				if (($listusers[$i]["status"]) != 7) {
					// $tr .= '<td class="text-center"><button type="button" class="btn btn-primary btn-md" id="listFLAGcust_' . $i . '" title="Flag" data-toggle="modal" data-target="#myregModal_flag' . $i . '"><i class="fa fa-flag-o fa-fw"></i>Flag</button></td>';
					$flagbut = '<button type="button" class="btn btn-primary btn-md" id="listFLAGcust_' . $i . '" title="Flag" data-toggle="modal" data-target="#myregModal_flag' . $i . '"><i class="fa fa-flag-o fa-fw"></i></button>';
					} else if (($listusers[$i]["status"]) == 7) {
					// $tr .= '<td class="text-center"><button class="btn btn-danger btn-md" id="listUNFLAGcust_' . $i . '" data-toggle="modal" data-target="#myregModal_unflag' . $i . '"><i class="fa fa-flag-o fa-fw"></i>Unflag</button></td>';
					$flagbut = '<button class="btn btn-danger btn-md" id="listUNFLAGcust_' . $i . '" data-toggle="modal" data-target="#myregModal_unflag' . $i . '"><i class="fa fa-flag-o fa-fw"></i></button>';
				}
				$tr .= '<td class="text-center"><button type="button" id="listEDITreg_' . $i . '" title="Edit" ><i class="fa fa-edit fa-fw"></i></button> - <button type="button" class="btn btn-md" id="listDELreg_' . $i . '" title="Delete" data-toggle="modal" data-target="#myUSRregDELModal_' . $i . '"><i class="fa fa-trash-o fa-fw"></i></button> - '.$flagbut.'</td></tr>';
				echo str_replace($this->order, $this->replace, $tr.'<div class="modal fade" id="myUSRregDELModal_' . $i . '" tabindex="-1" role="dialog" aria-labelledby="myUSRLISTDELModal_' . $i . '" aria-hidden="true" style="display: none;">
				<div class="modal-dialog">
				<div class="modal-content" style="color:#000;">
				<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				...
				<div class="modal-footer">
				<button type="button" class="btn btn-danger" data-dismiss="modal" name=".modal-backdrop" id="deletelregDELOk_' . $i . '">Ok</button>
				<button type="button" class="btn btn-success" data-dismiss="modal" id="deletelltDELCancel_' . $i . '">Cancel</button>
				</div>
				</div>
				</div>
				</div>
				<div class="modal fade" id="myregModal_flag' . $i . '" tabindex="-1" role="dialog" aria-labelledby="myModal_flag_Label_' . $i . '" aria-hidden="true" style="display: none;">
				<div class="modal-dialog">
				<div class="modal-content" style="color:#000;">
				<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				...
				<div class="modal-footer">
				<button type="button" class="btn btn-danger" data-dismiss="modal" name=".modal-backdrop" id="flaglregOk_' . $i . '">Ok</button>
				<button type="button" class="btn btn-success" data-dismiss="modal" id="flaglltCancel_' . $i . '">Cancel</button>
				</div>
				</div>
				</div>
				</div>
				<div class="modal fade" id="myregModal_unflag' . $i . '" tabindex="-1" role="dialog" aria-labelledby="myModal_unflag_Label_' . $i . '" aria-hidden="true" style="display: none;">
				<div class="modal-dialog">
				<div class="modal-content" style="color:#000;">
				<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				...
				<div class="modal-footer">
				<button type="button" class="btn btn-danger" data-dismiss="modal" name=".modal-backdrop" id="unflaglregOk_' . $i . '">Ok</button>
				<button type="button" class="btn btn-success" data-dismiss="modal" id="unflaglltCancel_' . $i . '">Cancel</button>
				</div>
				</div>
				</div>
				...
				$(document).ready(function () {
					var ltcust = {
						...
						master_pk : "' . $listusers[$i]["master_pk"] . '",
						index : ' . $i . ',
						editBtn : "#listEDITreg_' . $i . '",
						delBtn : "#listDELreg_' . $i . '",
						delOkBtn : "#deletelregDELOk_' . $i . '",
						flagBtn : "#listFLAGcust_' . $i . '",
						flagokBtn : "#flaglregOk_' . $i . '",
						unflagBtn : "#listUNFLAGcust_' . $i . '",
						unflagokBtn : "#unflaglregOk_' . $i . '",
						factid : null,
						tabId : "' . $tabId . '",
						statusId : "' . $listusers[$i]['status'] . '",
					};
					var obj = new controlCustomeList();
					obj.listREGcusttabledata(ltcust);
				});
				</script>');
				//  }
			}
			...
			$(document).ready(function () {
				var att = {
					tableid : "#listREGcusttabl",
				};
				var obj = new controlCustomeList();
				obj.listREGcusttabledata(att);
			});
			</script>');
			*/
		}
		public function editREGCustomerList($cid, $tabid, $i) {
			$temp = 1;
			$listusers = array();
			$listusers = $_SESSION["listofreg"];
			$personalinfo = str_replace($this->order, $this->replace,'<div class="row">
			<div class="col-lg-12">&nbsp;</div>
			<div class="col-lg-8">
			<div class="panel panel-green">
			...
			<div class="panel-body" id="subcustdata_' . $temp . '">
			<ul>
			<li><strong>Name :</strong>' . $listusers[$i]['name'] . '</li>
			<li><strong>Email :</strong>' . $listusers[$i]['email_id'] . '</li>
			<li><strong>Cell Number  :</strong>' . $listusers[$i]['cell_number'] . '</li>
			<li><strong>DOB :</strong>' . date('d-M-Y', strtotime($listusers[$i]['dob'])) . '</li>
			<li><strong>Joining Date :</strong>' . date('d-M-Y h:i:s A', strtotime($listusers[$i]['date_of_join'])) . '</li>
			<li><strong>Occupation :</strong>' . $listusers[$i]['occupation'] . '</li>
			</ul>
			</div>
			<div class="panel-body" id="subcustinfoEDITdata_' . $temp . '" style="display:none">
			<form id="subcust_info_edit_form_' . $temp . '">
			...
			<div class="col-lg-12">
			<input class="form-control" placeholder="Name" name="custname_' . $temp . '" type="text" id="custname_' . $temp . '" maxlength="100" value="' . $listusers[$i]['name'] . '"/>
			<p class="help-block" id="custnmsg_' . $temp . '">Enter/ Select.</p>
			</div>
			</div>
			<div class="col-lg-4">
			...
			<div class="col-lg-12">
			<input class="form-control" placeholder="Email Id" name="custemail_' . $temp . '" type="text" id="custemail_' . $temp . '" maxlength="100" value="' . $listusers[$i]['email_id'] . '"/>
			<p class="help-block" id="custemmsg_' . $temp . '">Enter/ Select.</p>
			</div>
			</div>
			<div class="col-lg-4">
			...
			<div class="col-lg-12">
			<input class="form-control" placeholder="DB Host" name="custcell_' . $temp . '" type="text" id="custcell_' . $temp . '" maxlength="100" value="' . $listusers[$i]['cell_number'] . '"/>
			<p class="help-block" id="custcellmsg_' . $temp . '">Enter/ Select.</p>
			</div>
			</div>
			<div class="col-lg-12">&nbsp;</div>
			...
			<div class="col-lg-12">
			<input class="form-control" placeholder="DOB" name="custdob_' . $temp . '" type="text" id="custdob_' . $temp . '" maxlength="100" value="' . date('d-M-Y', strtotime($listusers[$i]['dob'])) . '" readonly="readonly"/>
			<p class="help-block" id="custdobmsg_' . $temp . '">Enter/ Select.</p>
			</div>
			</div>
			<div class="col-lg-4">
			...
			<div class="col-lg-12">
			<input class="form-control" placeholder="Date OF Joining" name="custdoj_' . $temp . '" type="text" id="custdoj_' . $temp . '" maxlength="100" value="' . $listusers[$i]['date_of_join'] . '" readonly="readonly"/>
			<p class="help-block" id="custdojmmsg_' . $temp . '">Enter/ Select.</p>
			</div>
			</div>
			<div class="col-lg-4">
			...
			<div class="panel-body" id="usrphoto_' . $temp . '">
			<img src="' . $listusers[$i]['photo'] . '" width="150" class="img-circle" />
			</div>
			<div class="panel-footer">
			<button class="btn btn-info btn-md" id="usrphoto_but_edit_' . $temp . '" data-toggle="modal" data-target="#myModal_Photo1">
			<i class="fa fa-edit fa-fw "></i> Edit
			</button>
			</div>
			</div>
			...
			$trandata = '';
			$count3 = sizeof($listusers[$i]['accounts']['inv_tt']);
			if ($count3 > 0) {
				for ($j = 0; $j < $count3; $j++) {
					$trandata .= '<tr><td>' . ($j + 1) . '</td><td>' . $listusers[$i]['accounts']['inv_tt'][$j] . '</td><td><a style=\'cursor:pointer;\' onclick=\'window.open(\"' . URL. $listusers[0]['accounts']['inv_urls'][$j] . '\");\' href=\'javascript:void(0);\'>' . $listusers[$i]['accounts']['mt_rpt'][$j] . '</a></td><td>' . $listusers[$i]['accounts']['mt_pod'][$j] . '</td><td>' . $listusers[$i]['accounts']['mop'][$j] . '</td></tr>';
				}
			}
			echo str_replace($this->order, $this->replace,'<div class="col-lg-12 text-right"><button class="text-center btn btn-danger btn-md" id="custeditlist_Back_But_' . $temp . '"><i class="fa fa-reply fa-fw "></i>Back</button></div><ul class="nav nav-pills">
			<div class="col-lg-12">&nbsp;</div>
			<li class="active"><a href="#info_subuser_list_' . $temp . '" data-toggle="tab">Personal Info</a></li>
			<li><a href="#transaction_subuser_list_' . $temp . '" data-toggle="tab">Transaction</a></li>
			</ul>
			...
			<div class="tab-pane fade table-responsive" id="transaction_subuser_list_' . $temp . '">
			</div>
			</div>
			<form action="control.php" name="updatePic" id="changePic" method="post" enctype="multipart/form-data">
			<fieldset>
			...
			<input type="hidden" name="autoloader" value="true" />
			<input type="hidden" name="type" value="slave" />
			<input type="hidden" name="photo_id" value="' . $listusers[$i]["id"] . '"/>
			<input type="hidden" name="user_id" value="' . $listusers[$i]["master_pk"] . '"/>
			<div class="modal" id="myModal_Photo1" tabindex="-1" role="dialog" aria-labelledby="myModal_Photo_Label_' . $listusers[$i]["id"] . '" aria-hidden="true" style="display: none;">
			<div class="modal-dialog">
			<div class="modal-content" style="color:#000;">
			<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			<h4 class="modal-title" id="myModal_Photo_Label_' . $listusers[$i]["id"] . '">Update Customer Photo</h4>
			</div>
			<div class="modal-body">
			<!-- begin_picedit_box -->
			<div class="picedit_box">
			...
			<div class="modal-footer">
			<button type="submit" name="submit" class="btn btn-success" id="addusrBut">Change Picture</button>
			<button type="button" class="btn btn-danger" data-dismiss="modal" id="photoCancel_' . $listusers[$i]["id"] . '">Cancel</button>
			</div>
			</div>
			</div>
			</div>
			</fieldset>
			</form><script langauge="javascript" type="text/javascript">
			$(document).ready(function(){
			var ltcust = {
			cust_id			:	"' . $listusers[$i]["id"] . '",
			master_pk		:	"' . $listusers[$i]["master_pk"] . '",
