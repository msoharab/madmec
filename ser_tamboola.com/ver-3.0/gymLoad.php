	...
	class PageLoad {
		...
		public function fetchOwnerUser() {
			...
			if (mysql_num_rows($result)) {
				...
				for ($i = 0; $i < sizeof($fetchdata); $i++) {
					$data .='<tr><td>' . ($i + 1) . '</td><td>' . $fetchdata[$i]['user_name'] . ' -- ' . $fetchdata[$i]['email_id'] . '-- ' . $fetchdata[$i]['cell_number'] . '</td><td>';
                    ...
                    for ($j = 0; $j < sizeof($tempgymname) && $tempgymname[$j] != ""; $j++) {
                        $data .='<span class="text-left">' . ($j + 1) . ' -- ' . trim($tempgymname[$j], ',') . ' -- ' . trim($tempgymemail[$j], ',') . ' -- ' . trim($tempgymemail[$j], ',') . ' -- ' . trim($tempgymtown[$j], ',') . ' -- ' . trim($tempgymcity[$j], ',') . ' -- </span>';
                        $data .='<button type="button" class="btn btn-danger pull-right" title="Delete GYM" data-toggle="modal" data-target="#myModal_open22' . trim($tempgymassnids[$j], ',') . '"><i class="fa fa-trash" id="deletegydm_' . $fetchdata[$i]['id'] . '"></i></button>';
