function managersController() {
    ...
    this.__AddManagers = function () {
        var managers = members.AddManager;
        var fields = members.AddManager.fields;
        fetchGender({
            mem: managers.gender,
            fields: fields,
            index: Number(1)
        ...
        bindAddManagersEvents();
        bindAssignManagersEvents();
    };
    function bindAddManagersEvents() {
        var managers = members.AddManager;
        ...
        $(form).submit(function (e) {
            if (checkusr) {
                ...
                frmdata.append("dob", $('#' + fields[2]).val());
                frmdata.append("utype", $('#' + fields[3]).val());
                frmdata.append("email", $('#' + fields[4]).val());
                frmdata.append("cellnumber", $('#' + fields[5]).val());
                ...
                frmdata.append("addressline", $('#' + fields[10]).val());
                frmdata.append("zipcode", $('#' + fields[11]).val());
                frmdata.append("doctype", $('#' + fields[12]).val());
                frmdata.append("docnumber", $('#' + fields[13]).val());
                frmdata.append("docpath", $('#' + fields[14]).val());
                AddUserDB(frmdata);
            }
            else {
                alert('Please correct the credentials..');
