            ...
            if ($query) {
                ...
                if (mysql_num_rows($res)) {
                    ...
                    while ($row = mysql_fetch_assoc($res)) {
                        ...
                        $userfields[$i]['inv_urls'] = URL . '' . $row['inv_urls']; /* J */
    ...
    public function BalanceSheet() {
        ...
        if ($fromdate != 0 || $todate != 0) {
            ...
            $objPHPExcel->getProperties()->setCreator($this->parameters["GYMNAME"])
                    ...
                    ->setCategory("Reports");
            if ($expensesfields != NULL) {
                $myWorkSheet = new PHPExcel_Worksheet($objPHPExcel, substr('Expenses report', 0, 30));
                // Attach the “My Data” worksheet as the first worksheet in the PHPExcel object
                $objPHPExcel->addSheet($myWorkSheet, 0);
                /* Set active sheet */
                $objPHPExcel->setActiveSheetIndexByName(substr('Expenses report', 0, 30));
                /* Set page size */
                $objPHPExcel->getActiveSheet()
                        ->getPageSetup()
                        ->setOrientation(PHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE);
            ...
            if ($paymentsfields != NULL) {
                $myWorkSheet = new PHPExcel_Worksheet($objPHPExcel, substr('Payments report', 0, 30));
                // Attach the “My Data” worksheet as the first worksheet in the PHPExcel object
                $objPHPExcel->addSheet($myWorkSheet, 0);
                /* Set active sheet */
                $objPHPExcel->setActiveSheetIndexByName(substr('Payments report', 0, 30));
                /* Set page size */
                $objPHPExcel->getActiveSheet()
                        ->getPageSetup()
                        ->setOrientation(PHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE);
            ...
            $myWorkSheet = new PHPExcel_Worksheet($objPHPExcel, substr('Balance sheet', 0, 30));
            // Attach the “My Data” worksheet as the first worksheet in the PHPExcel object
            $objPHPExcel->addSheet($myWorkSheet, 0);
            /* Set active sheet */
            $objPHPExcel->setActiveSheetIndexByName(substr('Balance sheet', 0, 30));
            /* Set page size */
            $objPHPExcel->getActiveSheet()
                    ->getPageSetup()
                    ->setOrientation(PHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE);
    ...
    public function FeeReport($from, $to, $fromdate, $todate, $objPHPExcel, $fid, $fnm) {
		...
		AND c.`facility_id` = "' . $fct_id . '"';
            ...
            if ($query) {
                ...
                if ($feefields != NULL) {
                    $myWorkSheet = new PHPExcel_Worksheet($objPHPExcel, substr($fname . ' fee collection report', 0, 30));
                    // $myWorkSheet = new PHPExcel_Worksheet($objPHPExcel, $fname);
                    $headers = array('font' => array('bold' => true, 'color' => array('rgb' => '000000'), 'size' => 10, 'name' => 'Arial'), 'alignment' => array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER, 'vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER),);
                    $sheet = $objPHPExcel->getActiveSheet();
                    $rows = 1;
                    // Attach the “My Data” worksheet as the first worksheet in the PHPExcel object
                    $objPHPExcel->addSheet($myWorkSheet, 0);
                    /* Set active sheet */
                    $objPHPExcel->setActiveSheetIndexByName(substr($fname . ' fee collection report', 0, 30));
                    $objPHPExcel->getProperties()->setCreator($this->parameters["GYMNAME"])
                            ->setLastModifiedBy($this->parameters["GYMNAME"])
            ...
            if ($query) {
                ...
                if (mysql_num_rows($res)) {
                    ...
                    while ($row = mysql_fetch_assoc($res)) {
                        ...
                        $feefields[$i]['inv_urls'] = URL . '' . $row['inv_urls'];           /* M */
                    ...
                    for ($i = 1; $i <= sizeof($feefields); $i++) {
                        ...
                        if ($feefields[$i]['inv_urls'] != NULL || $feefields[$i]['inv_urls'] != '') {
                            $objPHPExcel->getActiveSheet()
                                    ->getCellByColumnAndRow(10, ($i + 1))
                                    ->getHyperlink()
                                    ->setUrl(trim($feefields[$i]['inv_urls']));              /* M */
                        }
                    }
                    // $objPHPExcel->getActiveSheet()
        ...
        if ($fromdate != 0 && $todate != 0) {
            ...
            if ($query) {
                ...
                if (mysql_num_rows($res)) {
                    ...
                    $feefields = NULL;
                if ($feefields != NULL) {
                    $myWorkSheet = new PHPExcel_Worksheet($objPHPExcel, substr('Package fee collection report', 0, 30));
                    // Attach the “My Data” worksheet as the first worksheet in the PHPExcel object
                    $objPHPExcel->addSheet($myWorkSheet, 0);
                    /* Set active sheet */
                    $objPHPExcel->setActiveSheetIndexByName(substr('Package fee collection report', 0, 30));
                    $objPHPExcel->getProperties()->setCreator($this->parameters["GYMNAME"])
                            ->setLastModifiedBy($this->parameters["GYMNAME"])
                            ->setTitle("Package Fee Collection")
                            ->setSubject("Fee Collection Report")
        ...
        if ($fromdate != 0 || $todate != 0) {
            ...
            if ($query) {
                ...
                if (mysql_num_rows($res)) {
                    ...
                    $userfields = NULL;
                if ($userfields != NULL) {
                    $myWorkSheet = new PHPExcel_Worksheet($objPHPExcel, substr('Customer Registrations fee', 0, 30));
                    // Attach the “My Data” worksheet as the first worksheet in the PHPExcel object
                    $objPHPExcel->addSheet($myWorkSheet, 0);
                    /* Set active sheet */
                    $objPHPExcel->setActiveSheetIndexByName(substr('Customer Registrations fee', 0, 30));
                    $objPHPExcel->getProperties()->setCreator($this->parameters["GYMNAME"])
                            ->setLastModifiedBy($this->parameters["GYMNAME"])
                            ->setTitle("Customer Registrations")
                            ->setSubject("Customer Registrations Report")
