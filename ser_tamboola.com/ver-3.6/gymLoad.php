        ...
        if (isset($_SESSION["USER_LOGIN_DATA"]["GYM_DETAILS"]) && is_array($_SESSION["USER_LOGIN_DATA"]["GYM_DETAILS"])) {
            for ($i = 0; $i < sizeof($_SESSION["USER_LOGIN_DATA"]["GYM_DETAILS"]); $i++) {
                if (isset($_SESSION["USER_LOGIN_DATA"]["USER_TYPE"]) && $_SESSION["USER_LOGIN_DATA"]["USER_TYPE"] == "Owner") {
                    $htm .= str_replace($this->order, $this->replace, '<li class="text-muted">
                    <a href="javascript:void(0);" name="' . $i . '" class="gymLink" id="loadDash_'.$i.'" style="text-decoration:none;">
                        <img src="' . LOGO_1 . '" class="img-circle" width="25" />&nbsp;' . ucwords(substr($_SESSION["USER_LOGIN_DATA"]["GYM_DETAILS"][$i]["GYM_NAME"], 0, 12)) . '
                     </a>
                    </li><li class="divider"></li>
                    <script language="javascript">
                    $("#loadDash_'.$i.'").click(function (evt) {
                    	evt.preventDefault();
						var gymdynamic = {
							nav : ".gymLink",
							outdiv : "#printrs",
                    	...
                    	window.setTimeout(function () {
                    		...
                    		loadSinglePageDash(dashboard);
                    	}, 300);
                    });
                    </script>');
                } else {
                    $htm .= str_replace($this->order, $this->replace, '<li>
                    <a href="javascript:void(0);" name="' . $i . '" class="gymLink">
                        <img src="' . LOGO_1 . '" class="img-circle" width="25" />&nbsp;' . ucwords(substr($_SESSION["USER_LOGIN_DATA"]["GYM_DETAILS"][$i]["GYM_NAME"], 0, 12)) . '
                     </a>
                    </li><li class="divider"></li>
					                    <script language="javascript">
