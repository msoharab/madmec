...
function userLogin($parameters) {
	...
	WHERE up.`email_id`=\'' . mysql_real_escape_string($parameters["email_id"]) . '\'
	AND up.`password`=\'' . mysql_real_escape_string($parameters["password"]) . '\'
	AND up.`status` NOT IN (SELECT `id` FROM `status` WHERE (`statu_name` = "Left" OR
								`statu_name` = "Hide" OR
        ...
        if (mysql_num_rows($res) > 0) {
            ...
            if ($row['userstatus'] == 14) {
                ...
                if ($row["email_id"] == $parameters["email_id"] &&
                    ...
                    //check gym names under user
                    if ($userdata["USER_TYPE"] == "Owner" || $userdata["USER_TYPE"] == "Admin") {
                        ...
                        $query = 'SELECT
									...
									FROM `gym_profile` AS g
									  JOIN `userprofile_gymprofile` AS ug
										ON g.`id` = ug.`gym_id`
									WHERE ug.`status` = (SELECT
														   `id`
														 FROM `status`
														 WHERE `statu_name` = "Active"
															 AND STATUS = 1)
										AND g.`status` = 4
										AND ug.`user_pk` = "' . mysql_real_escape_string($row['id']) . '";';
//                    echo $query;
//                    exit(0);
                        $res = executeQuery($query);
                        if (mysql_num_rows($res) > 0) {
                            while ($row = mysql_fetch_assoc($res)) {
                                executeQuery("UPDATE `" . $row["db_name"] . "`.`customer_attendence` SET `status` = 12 WHERE STR_TO_DATE(NOW(),'%Y-%m-%d') > STR_TO_DATE(`in_time`,'%Y-%m-%d') AND `status` = 11;");
                                executeQuery("UPDATE `" . $row["db_name"] . "`.`employee_attendence` SET `status` = 12 WHERE STR_TO_DATE(NOW(),'%Y-%m-%d') > STR_TO_DATE(`in_time`,'%Y-%m-%d') AND `status` = 11;");
                                array_push($fetchdata, $row);
                            }
                            if (isset($fetchdata[0])) {
                                $_SESSION["SETGYM"] = array(
                                    "GYM_IND" => 0,
                                    "GYM_ID" => $fetchdata[0]["id"],
                                    "GYM_NAME" => $fetchdata[0]["gym_name"],
                                );
                            }
//                            print_r($fetchdata);
//                            print_r($_SESSION["SETGYM"]);
//                            exit(0);
                        }
                        $query4 = 'SELECT g.*
				FROM `gym_profile` AS g
				...
				WHERE ug.`status`=(SELECT `id` FROM `status` WHERE `statu_name`=\'Active\' AND STATUS=1)
                                AND g.`status`=14
                                AND ug.`user_pk`="' . mysql_real_escape_string($userdata['USER_ID']) . '";';
