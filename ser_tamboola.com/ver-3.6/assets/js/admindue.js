                ...
                switch (data) {
                    ...
                    default:
                        ...
                        if (res.status == "success") {
                            ...
                            var tableHeading = '<thead><th>#</th><th>Owner Name</th><th>Cell Number</th><th>Email Id</th><th>Username</th><th>Due Amount</th><th>Due Date</th><th>MOP</th><th>PAY</th></tr></thead><tbody>';
                            var footer = '</tbody></table>';
                            $(due.disdues).html(header + tableHeading + res.data + footer);
                            window.setTimeout(function () {
                                $('#list_dues_table').dataTable();
                                for (j = 0; j < res.ids.length; j++) {
                                    $('#pay' + res.ids[j]).bind('click', {
                                        tuserpk: res.userpks[j],
                                        tdueamt: res.dueamount[j]
                                    }, function (event) {
                                        var dueamount = event.data.tdueamt;
                                        var userpk = event.data.tuserpk;
                                    });
                                    /*                                                alert($('#duemop_'+res.ids[j]).val());*/
                                    $('#payOk_' + res.ids[j]).bind('click', {
                                        tuserpk: res.userpks[j],
                                        tdueamt: res.dueamount[j],
                                        tids: res.ids[j]
                                    }, function (event) {
