					...
					if (executeQuery($query2)) {
						...
						if (executeQuery($query3)) {
							...
							for ($i = 0; $i < $total; $i++) {
								...
								$filedirectory = DOC_ROOT . ASSET_DIR . $directory . '/profile/' . $filename;
								$invloc=ASSET_DIR . $directory . '/profile/' . $filename;
								$urlpath = URL . ASSET_DIR . $directory . '/profile/' . $filename;
