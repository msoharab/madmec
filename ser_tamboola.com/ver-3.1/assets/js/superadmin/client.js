function clientController() {
    ...
    this.__construct = function (clnt) {
        ...
        fetchuserngyms();
        $(ct.oreqmenuBut).click(function () {
            fetchOwnerRequest();
        });
        $(ct.gymreqmenuBut).click(function () {
            fetchGYMRequest();
        })
    };
    function fetchOwnerRequest()
    {
        $('#displayownerreq').html(LOADER_ONE);
        $.ajax({
            url: ct.url,
            type: 'POST',
            data: {
                autoloader: true,
                action: 'fetchOwnerRequest',
                type: 'master'
            },
                    ...
                    default:
                        if (data.status == "success")
                        {
                            var header = '<div class="table-responsive"><table class="table table-responsive table-bordered table-hover" id="list_col_table"><thead>' +
                                    '<tr><th>#</th><th>Name</th><th>Email</th><th>Mobile</th><th>Gender</th><th>Option</th></tr></thead><tbody>';
                            var footer = '</tbody></table></div>';
                            $('#displayownerreq').html(header + data.data + footer);
                            window.setTimeout(function () {
                                ...
                                if (data.userids.length)
                                {
                                    ...
                                    {
                                        $("#list_col_table").on("click", "#accept_" + data.userids[i], {
                                            donorid: data.userids[i],
                                            mobile : data.mobiles[i],
                                        }, function (evt) {
                                            $("#accept_" + evt.data.donorid).prop('disabled', 'disabled');
                                             $("#deletereq" + evt.data.donorid).prop('disabled', 'disabled');
                                            RequestAcceptnReject(evt.data.donorid, evt.data.mobile,"accept");
                                        });

                                        $("#list_col_table").on("click", "#deletereq" + data.userids[i], {
                                            donorid: data.userids[i],
                                            mobile : data.mobiles[i],
                                        }, function (evt) {
                                            $("#deletereq" + evt.data.donorid).prop('disabled', 'disabled');
                                            $("#accept_" + evt.data.donorid).prop('disabled', 'disabled');
                                            RequestAcceptnReject(evt.data.donorid, evt.data.mobile,"reject");
                                        });
                                    }
                                }
                            }, 500);
                        ...
                        {
                            $('#displayownerreq').html('<strong class="text-danger">no records');
                        }
                        break;
                }
            },
    ...
    function fetchGYMRequest() {
        $('#displaygymreq').html(LOADER_ONE);
        $.ajax({
            url: ct.url,
            type: 'POST',
            data: {
                autoloader: true,
                action: 'fetchGYMRequest',
                type: 'master'
            },
            success: function (data, textStatus, xhr) {
                data = $.parseJSON($.trim(data));
                switch (data) {
                    ...
                    default:
                        if (data.status == "success")
                        {
                            var header = '<div class="table-responsive"><table class="table table-responsive table-bordered table-hover" id="list_col_table12"><thead>' +
                                    '<tr><th>#</th><th>Owner</th><th>GYM</th><th>Email</th><th>Mobile</th><th>GymType</th><th>City</th><th>Option</th></tr></thead><tbody>';
                            var footer = '</tbody></table></div>';
                            $('#displaygymreq').html(header + data.data + footer);
                            window.setTimeout(function () {
                                $('#list_col_table12').dataTable();
                                if (data.userids.length)
                                {

                                    for (i = 0; i < data.userids.length; i++)
                                    {
                                        $("#list_col_table12").on("click", "#accept_" + data.userids[i], {
                                            donorid: data.userids[i],
                                            dbname: data.db_name[i],
                                        }, function (evt) {
                                            $("#accept_" + evt.data.donorid).prop('disabled', 'disabled');
                                            $("#deletereq" + evt.data.donorid).prop('disabled', 'disabled');
                                            RequestAcceptnRejectGYM(evt.data.donorid, "accept", evt.data.dbname);
                                        });

                                        $("#list_col_table12").on("click", "#deletereq" + data.userids[i], {
                                            donorid: data.userids[i],
                                            dbname: data.db_name[i],
                                        }, function (evt) {
                                            $("#deletereq" + evt.data.donorid).prop('disabled', 'disabled');
                                            $("#accept_" + evt.data.donorid).prop('disabled', 'disabled');
                                            RequestAcceptnRejectGYM(evt.data.donorid, "reject", evt.data.dbname);
                                        });
                                    }
                                }
                            }, 500);
                        ...
                        {
                            $('#displaygymreq').html('<strong class="text-danger">no records');
                        }
                        break;
                }
            },
    ...
    function RequestAcceptnReject(userid,cell_number, req)
    {
        $.ajax({
            url: ct.url,
            type: 'POST',
                    ...
                    default:
                        if (data)
                            if (data)
                            {
                                if (req == "accept")
                                {
                                    alert("Owner Request has been Successfully Activated");
                                }
                                else
                                {
                                    alert("Owner Request has been Successfully Deleted");
                                }
                                fetchOwnerRequest();
                            }
                            else
                            {
                                if (req == "accept")
                                {
                                    alert("Owner Request hasn't Successfully Activated");
                                }
                                else
                                {
                                    alert("Owner Request hasn't  Deleted");
                                }
                                fetchOwnerRequest();
                            }
                        break;
    ...
    function RequestAcceptnRejectGYM(userid, req, dbname)
    {
        $.ajax({
            url: ct.url,
            type: 'POST',
                ...
                switch (data) {
                    ...
                    default:
                        if (data)
                        {
                            if (req == "accept")
                            {
                                alert("GYM Request has been Successfully Activated");
                            }
                            else
                            {
                                alert("GYM Request has been Successfully Deleted");
                            }
                            fetchGYMRequest();
                        }
                        else
                        {
                            if (req == "accept")
                            {
                                alert("GYM Request hasn't Successfully Activated");
                            }
                            else
                            {
                                alert("GYM Request hasn't  Deleted");
                            }
                            fetchGYMRequest();
                        }
                        break;
            ...
            success: function (data, textStatus, xhr) {
                ...
                switch (data) {
                    ...
                    case 'login':
                        ...
                        break;
                    default:
                        var listgymusers = $.parseJSON($.trim(data));
