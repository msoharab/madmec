...
class report {
    ...
    public function addEmployee() {
        ...
        $query1 = 'INSERT INTO  `report` (`id`,`report_name`,`description`,`created_at`,`updated_at`,`status_id` )
            VALUES(
            NULL,
    ...
    public function fetchListOfEmployees() {
        ...
        if (mysql_num_rows($result)) {
            ...
            for ($i = 0; $i < sizeof($fetchdata); $i++) {
                $data .='<tr><td>' . ($i + 1) . '</td><td>' . $fetchdata[$i]['report_name'] . '</td><td>' . $fetchdata[$i]['description'] .
							...
							<div class="modal-dialog">
								<div class="modal-content" style="color:#000;">
									...
									<div class="modal-body" id="myUSRDEL_' . $i . '">
										Do you really want to delete {' . $fetchdata[$i]['report_name'] . '} <br />
										Press OK to delete ??
									</div>
    ...
    public function CheckEmail() {
        if (isset($this->parameters['donorid'])) {
            $query = "SELECT * FROM `report` WHERE `report_name` = '" . mysql_real_escape_string(strtolower($this->parameters['email'])) . "' AND `id` !='" . mysql_real_escape_string($this->parameters['donorid']) . "';";
            $reslut = executeQuery($query);
            return mysql_num_rows($reslut);
        }
        $query = "SELECT * FROM `report` WHERE `report_name`='" . mysql_real_escape_string(strtolower($this->parameters['email'])) . "';";
        $reslut = executeQuery($query);
        return mysql_num_rows($reslut);
    }
    ...
    public function updateDonorDetails() {
        if ($this->parameters['request'] == "personal") {
            $query = 'UPDATE `report` SET `report_name`="' . mysql_real_escape_string($this->parameters['ename']) . '",
    ...
    public function deleteDonor() {
        $query = 'UPDATE `report` SET `status_id`=6 WHERE `id`=' . mysql_real_escape_string($this->parameters['donorid']) . ';';
        return executeQuery($query);
    }
}
