...
class user {
    ...
    public function addEmployee() {
        ...
        $query1 = 'INSERT INTO  `pic3pic_continents` (`id`,`continent_name`,`created_at`,`updated_at`,`description`,`status_id` )
            VALUES(
            NULL,
...
//        echo $_SESSION["USER_LOGIN_DATA"]['USER_TYPE'];
    ...
    public function CheckEmail() {
        if (isset($this->parameters['donorid'])) {
            $query = "SELECT * FROM `pic3pic_continents` WHERE `continent_name` = '" . mysql_real_escape_string(strtolower($this->parameters['email'])) . "' AND `id` !='" . mysql_real_escape_string($this->parameters['donorid']) . "';";
            $reslut = executeQuery($query);
            return mysql_num_rows($reslut);
        }
        $query = "SELECT * FROM `pic3pic_continents` WHERE `continent_name`='" . mysql_real_escape_string(strtolower($this->parameters['email'])) . "';";
        $reslut = executeQuery($query);
        return mysql_num_rows($reslut);
    }
    ...
    public function deleteDonor() {
        $query = 'UPDATE `pic3pic_continents` SET `status_id`=6 WHERE `id`=' . mysql_real_escape_string($this->parameters['donorid']) . ';';
        return executeQuery($query);
    }
}