...
class import {
    ...
    public function fetchContinents($continet = false) {
        $query = 'SELECT 
                    con.id,
                    con.Country AS continent_name
                FROM `pic3pic_countries` AS con
                LEFT JOIN `pic3pic_continents` AS cnt ON cnt.id = con.continent_id
    ...
    public function fetchListOfProjects($continet = false) {
        ...
        $query = 'SELECT 
                    con.*,
                    cnt.Country
                FROM `pic3pic_languages` AS con
                LEFT JOIN `pic3pic_countries` AS cnt ON cnt.id = con.country_id
                WHERE con.status_id = 4
                AND cnt.`id` = "'.  mysql_real_escape_string($continet).'"
                ORDER BY con.`Language Name` ASC;';
        $result = executeQuery($query);
        if (mysql_num_rows($result)) {
            while ($row = mysql_fetch_assoc($result)) {
                $fetchdata[] = $row;
    ...
    public function checkProjectName() {
        if (isset($this->parameters['donorid'])) {
            $query = "SELECT * FROM `pic3pic_languages` WHERE `Language Name`='" . mysql_real_escape_string($this->parameters['proname']) . "' AND `id` !='" . mysql_real_escape_string($this->parameters['donorid']) . "' AND  country_id > 0;";
            $reslut = executeQuery($query);
            return mysql_num_rows($reslut);
        }
        $query = "SELECT * FROM `pic3pic_languages` WHERE `Language Name`='" . mysql_real_escape_string($this->parameters['proname']) . "'  AND  country_id > 0;";
        $reslut = executeQuery($query);
        return mysql_num_rows($reslut);
    }
