    ...
    public function fetchListOfProjects() {
        ...
        if (mysql_num_rows($result)) {
            ...
            for ($i = 0; $i < sizeof($fetchdata); $i++) {
                $data .='<tr><td>' . ($i + 1) . '</td><td>' . $fetchdata[$i]['Country'] . '</td><td>' . $fetchdata[$i]['Capital'] . '</td><td>' . $fetchdata[$i]['ISO'] . '</td><td>' . $fetchdata[$i]['CurrencyCode'] . '</td><td>' . $fetchdata[$i]['continent_name'] . '</td>'
                        . ''
