...
class activity {
    ...
    public function addEmployee() {
        ...
        $query1 = 'INSERT INTO  `sections` (`id`,`section_name`,`description`,`created_at`,`updated_at`,`status_id` )
            VALUES(
            NULL,
            "'.mysql_real_escape_string(strtolower($this->parameters['name'])).'",
             "'.mysql_real_escape_string(strtolower($this->parameters['addrs'])).'",
                NOW(),
                NULL,
                default);';
    ...
    public function fetchListOfEmployees() {
        ...
        if (mysql_num_rows($result)) {
            ...
            for ($i = 0; $i < sizeof($fetchdata); $i++) {
                $data .='<tr><td>' . ($i + 1) . '</td><td>' . $fetchdata[$i]['section_name'] . '</td><td>' . $fetchdata[$i]['description'] .
                        '</td><td>' . $fetchdata[$i]['updated_at'] . '</td>'
							...
							<div class="modal-dialog">
								<div class="modal-content" style="color:#000;">
									...
									<div class="modal-body" id="myUSRDEL_' . $i . '">
										Do you really want to delete {' . $fetchdata[$i]['section_name'] . '} <br />
										Press OK to delete ??
									</div>
    ...
    public function CheckEmail() {
        if (isset($this->parameters['donorid'])) {
            $query = "SELECT * FROM `sections` WHERE `section_name` = '" . mysql_real_escape_string(strtolower($this->parameters['email'])) . "' AND `id` !='" . mysql_real_escape_string($this->parameters['donorid']) . "';";
            $reslut = executeQuery($query);
            return mysql_num_rows($reslut);
        }
        $query = "SELECT * FROM `sections` WHERE `section_name`='" . mysql_real_escape_string(strtolower($this->parameters['email'])) . "';";
        $reslut = executeQuery($query);
        return mysql_num_rows($reslut);
    }
    ...
    public function updateDonorDetails() {
        if ($this->parameters['request'] == "personal") {
            $query = 'UPDATE `sections` SET `section_name`="' . mysql_real_escape_string($this->parameters['ename']) . '",
    ...
    public function deleteDonor() {
        $query = 'UPDATE `sections` SET `status_id`=6 WHERE `id`=' . mysql_real_escape_string($this->parameters['donorid']) . ';';
        return executeQuery($query);
    }
}
