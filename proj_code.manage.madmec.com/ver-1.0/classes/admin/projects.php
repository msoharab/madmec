...
class adminProjects {
    ...
    public function addProject() {
        $query = 'INSERT INTO `countries`(`id`, `continent_id`, `ISO`, `ISO3`, `ISO-Numeric`, `Country`, 
    ...
    public function updateProjectDetails() {
        $query = 'UPDATE `countries` SET `continent_id`="' . mysql_real_escape_string($this->parameters['selectcont']) . '",
    ...
    public function deleteProject() {
        $query = 'UPDATE `countries` SET `status_id`="6" WHERE `id`="' . mysql_real_escape_string($this->parameters['typeid']) . '"';
        return executeQuery($query);
    }

    public function checkProjectName() {
        if (isset($this->parameters['donorid'])) {
            $query = "SELECT * FROM `countries` WHERE `Country`='" . mysql_real_escape_string($this->parameters['proname']) . "' AND `id` !='" . mysql_real_escape_string($this->parameters['donorid']) . "';";
            $reslut = executeQuery($query);
            return mysql_num_rows($reslut);
        }
        $query = "SELECT * FROM `countries` WHERE `Country`='" . mysql_real_escape_string($this->parameters['proname']) . "';";
        $reslut = executeQuery($query);
        return mysql_num_rows($reslut);
    }
