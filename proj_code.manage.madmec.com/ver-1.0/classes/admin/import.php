...
class import {
    ...
    public function addProject() {
        $query = 'INSERT INTO `languages`(`id`, `country_id`, `ISO 639-3`, `ISO 639-2`, `ISO 639-1`, `Language Name`,
             `status_id`) VALUES
            (NULL,
    ...
    public function updateProjectDetails() {
        $query = 'UPDATE `languages` SET `country_id`="' . mysql_real_escape_string($this->parameters['selectcont']) . '",
    ...
    public function checkProjectName() {
        if (isset($this->parameters['donorid'])) {
            $query = "SELECT * FROM `languages` WHERE `Language Name`='" . mysql_real_escape_string($this->parameters['proname']) . "' AND `id` !='" . mysql_real_escape_string($this->parameters['donorid']) . "';";
            $reslut = executeQuery($query);
            return mysql_num_rows($reslut);
        }
        $query = "SELECT * FROM `languages` WHERE `Language Name`='" . mysql_real_escape_string($this->parameters['proname']) . "';";
        $reslut = executeQuery($query);
        return mysql_num_rows($reslut);
    }
