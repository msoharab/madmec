function user() {
    ...
    this.__construct = function (cctrl) {
        ...
        $(add.form).validate({
            ...
            messages: {
                name: {
                    required: "Enter the Continent Name",
        ...
        $(add.form).submit(function (evt) {
            ...
            if (add.emailcheck == 0) {
                ...
                $.ajax({
                    type: 'POST',
                    url: ctrl.url,
                    data: formdata + '&autoloader=true&action=adminuser/addEmployee',
                    success: function (data, textStatus, xhr) {
                        ...
                        switch (data) {
                            ...
                            default:
                                if (data) {
                                    alert("Continent has been Successfully Added");
                                    $(add.err_email).html("");
                                    adddonorform = false;
                                    $(add.form).get(0).reset();
                                } else {
                                    alert("Continent hasn't been Added");
                                    $(add.err_email).html("");
                                    adddonorform = false;
                                    $(add.form).get(0).reset();
                                }
            ...
            success: function (data, textStatus, xhr) {
                ...
                switch (data) {
                    ...
                    default:
                        if (Number(data) > 0) {
                            add.emailcheck = 0
                            $(add.err_email).html("<strong class='text-danger'>Continent Name Already Exist</strog>");
                        } else {
                            add.emailcheck = 1;
                            $(add.err_email).html("<strong class='text-success'>Continent Name Is Unique</strog>");
                        }
                        break;
                }
            },
    ...
    function fetchListOfEmployees() {
        $.ajax({
            ...
            data: {
                autoloader: true,
                action: 'adminuser/fetchListOfEmployees',
            },
            success: function (data, textStatus, xhr) {
                /* console.log(data);*/
                data = $.trim(data);
                    ...
                    default:
                        ...
                        if (type.status == "success") {
                            var header = '<div class="table-responsive"><table class="table table-responsive table-bordered table-hover" id="list_col_table"><thead>' +
                                    '<tr><th>#</th><th>Name</th><th>Created At</th><th>Updated At</th><th>Description</th><th>Option</th></tr></thead><tbody>';
    ...
    function viewDetails(donorid, loc, talldata) {
        ...
        var data = '<div class="col-lg-12">' +
                ...
                '<tr><td><strong>Name</strong></td>  <td>' + talldata[loc]['user_name'] + '</td></tr>' +
                '<tr><td><strong>Created At</strong></td> <td>' + talldata[loc]['created_at'] + '</td></tr>' +
                '<tr><td><strong>Updated At</strong></td>  <td>' + talldata[loc]['updated_at'] + '</td> </tr>' +
                '<tr><td><strong>Description</strong></td>  <td>' + talldata[loc]['description'] + '</td> </tr>' +
                '</tbody></table>';
        data += ' ' +
                '</div></div>' +
                '</div>';
    ...
    function personalEditDetails(donorid, loc, talldata) {
        ...
        var details = '<form id="personaleditform" autocomplete="off">' +
                '<div class="col-lg-12">' +
                ' <strong><i class="fa fa-star text-danger">&nbsp;</i>Continent Name : <i class="fa fa-caret-down fa-fw"></i></strong>' +
                ' </div>' +
                ' <div class="col-lg-12">' +
                '  <input  id="ename" name="ename" type="text" value="' + talldata[loc]['user_name'] + '" class="form-control" placeholder="Name"/>' +
                ...
                ' </div>' +
                '<div class="col-lg-12">' +
                '   <textarea class="form-control" placeholder="Address" name="addrs" id="addrs" >' + talldata[loc]['description'] + '</textarea>' +
            ...
            $('#personaleditform').submit(function (evt) {
                ...
                if (editpersonalflag && add.eemailcheck) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: ctrl.url,
                        data: formdata + '&autoloader=true&action=adminuser/updateDonorDetails&donorid=' + donorid + '&request=personal',
                        success: function (data, textStatus, xhr) {
                            /*                                        console.log(data);*/
                            data = $.trim(data);
                            switch (data) {
                                ...
                                default:
                                    if (data) {
                                        alert("Continent Details has been Successfully Updated");
                                        add.eempidcheck == 2;
                                        add.eemailcheck == 2;
                                        fetchListOfEmployees();
                                    } else {
                                        alert("Continent Details hasn't been Updated");
                                        add.eempidcheck == 2;
                                        add.eemailcheck == 2;
                                        viewDetails(donorid, loc, talldata);
                                    }
            ...
            success: function (data, textStatus, xhr) {
                ...
                switch (data) {
                    ...
                    default:
                        if (Number(data)) {
                            add.eemailcheck = 0
                            $('#err_email1').html("<strong class='text-danger'>Continent Name Already Exist</strog>");
                        } else {
                            add.eemailcheck = 1;
                            $('#err_email1').html("<strong class='text-success'>Continent Name Is Unique</strog>");
                        }
                        break;
                }
            },
                ...
                switch (data) {
                    ...
                    default:
                        if (data) {
                            ...
                            alert("Continent hasn't been Deleted")
                            fetchListOfEmployees();
                        }
                        break;
                }
