function reports() {
        ...
        $(add.form).submit(function (evt) {
            ...
            if (add.emailcheck == 0) {
                ...
                $.ajax({
                    type: 'POST',
                    url: ctrl.url,
                    data: formdata + '&autoloader=true&action=report/addEmployee',
                    success: function (data, textStatus, xhr) {
                        ...
                        switch (data) {
                            ...
                            default:
                                if (data) {
                                    alert("Report has been Successfully Added");
                                    $(add.err_email).html("");
                                    adddonorform = false;
                                    $(add.form).get(0).reset();
                                } else {
                                    alert("Report hasn't been Added");
                                    $(add.err_email).html("");
                                    adddonorform = false;
                                    $(add.form).get(0).reset();
                                }
            ...
            success: function (data, textStatus, xhr) {
                ...
                switch (data) {
                    ...
                    default:
                        if (Number(data) > 0) {
                            add.emailcheck = 0
                            $(add.err_email).html("<strong class='text-danger'>Report Name Already Exist</strog>");
                        } else {
                            add.emailcheck = 1;
                            $(add.err_email).html("<strong class='text-success'>Report Name Is Unique</strog>");
                        }
                        break;
                }
            },
    ...
    function fetchListOfEmployees() {
        $.ajax({
            ...
            data: {
                autoloader: true,
                action: 'report/fetchListOfEmployees',
            },
            success: function (data, textStatus, xhr) {
                /* console.log(data);*/
                data = $.trim(data);
                    ...
                    default:
                        ...
                        if (type.status == "success") {
                            var header = '<div class="table-responsive"><table class="table table-responsive table-bordered table-hover" id="list_col_table"><thead>' +
                                    '<tr><th>#</th><th>Report Name</th><th>Description</th><th>Updated At</th><th>Option</th></tr></thead><tbody>';
    ...
    function viewDetails(donorid, loc, talldata) {
        ...
        var data = '<div class="col-lg-12">' +
                ...
                '<tr><td><strong>Report Name</strong></td>  <td>' + talldata[loc]['report_name'] + '</td></tr>' +
    ...
    function personalEditDetails(donorid, loc, talldata) {
        ...
        var details = '<form id="personaleditform" autocomplete="off">' +
                '<div class="col-lg-12">' +
                ' <strong><i class="fa fa-star text-danger">&nbsp;</i>Report Name : <i class="fa fa-caret-down fa-fw"></i></strong>' +
                ' </div>' +
                ' <div class="col-lg-12">' +
                '  <input  id="ename" name="ename" type="text" value="' + talldata[loc]['report_name'] + '" class="form-control" placeholder="Name"/>' +
            ...
            $('#personaleditform').submit(function (evt) {
                ...
                if (editpersonalflag && add.eemailcheck) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: ctrl.url,
                        data: formdata + '&autoloader=true&action=report/updateDonorDetails&donorid=' + donorid + '&request=personal',
                        success: function (data, textStatus, xhr) {
                            /*                                        console.log(data);*/
                            data = $.trim(data);
                            switch (data) {
                                ...
                                default:
                                    if (data) {
                                        alert("Report Details has been Successfully Updated");
                                        add.eempidcheck == 2;
                                        add.eemailcheck == 2;
                                        fetchListOfEmployees();
                                    } else {
                                        alert("Report Details hasn't been Updated");
                                        add.eempidcheck == 2;
                                        add.eemailcheck == 2;
                                        viewDetails(donorid, loc, talldata);
                                    }
            ...
            success: function (data, textStatus, xhr) {
                ...
                switch (data) {
                    ...
                    default:
                        if (Number(data)) {
                            add.eemailcheck = 0
                            $('#err_email1').html("<strong class='text-danger'>Report Name Already Exist</strog>");
                        } else {
                            add.eemailcheck = 1;
                            $('#err_email1').html("<strong class='text-success'>Report Name Is Unique</strog>");
                        }
                        break;
                }
            },
            ...
            success: function (data, textStatus, xhr) {
                ...
                switch (data) {
                    ...
                    default:
                        if (Number(data)) {
                            add.eempidcheck = 0
                            $('#eerr_empid').html("<strong class='text-danger'>Report Id Already Exist</strog>");
                        } else {
                            add.eempidcheck = 1;
                            $('#eerr_empid').html("");
                        }
                ...
                switch (data) {
                    ...
                    default:
                        if (data) {
                            ...
                            alert("Report hasn't been Deleted")
                            fetchListOfEmployees();
                        }
                        break;
                }
