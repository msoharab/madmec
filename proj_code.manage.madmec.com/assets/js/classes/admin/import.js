...
//            minTime : dat.getHours()+':'+(dat.getMinutes()),
            ...
            success: function (data, textStatus, xhr) {
                ...
                switch (data) {
                    ...
                    default:
                        ...
                        $('#selectcontinent1').html('<select class="form-control" name="selectcont0" required="" id="selectcont0"><option value="">Please Select Continent</option>' + details + '</select>');
                        $('#selectcontinent2').html('<select class="form-control" name="selectcont1" required="" id="selectcont1"><option value="">Please Select Continent</option>' + details + '</select>');
                        $('#selectcont0').change(function (evt) {
                            evt.preventDefault();
                            var continet = Number($(this).val());
                            if (continet)
                                fetchContinents(continet, 'projectnamee');
                            else
                                alert('Select continent');
                        });
                        $('#selectcont1').change(function (evt) {
                            evt.preventDefault();
                            var continet = Number($(this).val());
                            if (continet)
                                fetchContinents(continet, 'projectnamee2');
                            else
                                alert('Select continent');
                        });
                        break;
    ...
    function fetchContinents(continet, output)
    {
        $.ajax({
            type: 'POST',
            url: ctrl.url,
            data: {
                autoloader: true,
                action: 'import/fetchContinents/' + continet,
            },
            success: function (data, textStatus, xhr) {
                /*  console.log(data); */
                data = $.trim(data);
                switch (data) {
                    ...
                    default:
                        ...
                        $('#' + output).html('<select class="form-control" name="selectcont' + output + '" required="" id="selectcont' + output + '"><option value="">Please Select Country</option>' + details + '</select>');
                        if (output === 'projectnamee2') {
                            $('#selectcont' + output).change(function (evt) {
                                evt.preventDefault();
                                var continet = Number($(this).val());
                                if (continet)
                                    fetchListOfProjects(continet);
                                else
                                    alert('Select continent');
                            });
                        }
    ...
    function fetchListOfProjects(continet)
    {
        $(ctrl.list.disvehicletype).html('Loading.....');
        $.ajax({
            type: 'POST',
            url: ctrl.url,
            data: '&autoloader=true&action=import/fetchListOfProjects/' + continet,
            success: function (data, textStatus, xhr) {
                /*                                        console.log(data);*/
                data = $.trim(data);
                ...
                switch (data) {
                    ...
                    default:
                        if (Number(data))
                        {
                            ...
                            $(ctrl.add.err_email).html("<strong class='text-danger'>Language Name Already Exist</strog>");
                        }
                        else
                        {
