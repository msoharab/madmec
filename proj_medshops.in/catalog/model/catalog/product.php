...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image, "
                        ...
                        . "AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' "
                        . "OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) "
		...
		if ($query->num_rows) {
			return array(
				...
				'model'            => $query->row['model'],
                                'manufacturedate'  => $query->row['manufacturedate'],
                                'expirydate'       => $query->row['expirydate'],
				'sku'              => $query->row['sku'],
				'upc'              => $query->row['upc'],
				'ean'              => $query->row['ean'],
				'jan'              => $query->row['jan'],
