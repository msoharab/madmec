...
function numbersonly(e){
    ...
    data1=parseFloat(document.getElementById('grand_total').value)+parseFloat(document.getElementById('MADMEC_total').value)-parseFloat(document.getElementById(id+'my_tot').value);
...
<div id="content">
	...
	<div class="page-full-width cf">
		...
		$contact=mysql_real_escape_string($_POST['contact']);
                         $count = $db->countOf("supplier_details", "supplier_name='$supplier' AND store_id=$ttstoreid");
						if($count==0)
						{
                                                         $db->query("insert into supplier_details(supplier_name,supplier_address,supplier_contact1,store_id) values('$supplier','$address','$contact','$ttstoreid')");   
                                                        }
			$temp_balance = $db->queryUniqueValue("SELECT balance FROM supplier_details WHERE supplier_name='$supplier' AND store_id=$ttstoreid");
			$temp_balance = (int) $temp_balance + (int) $balance;
			$db->execute("UPDATE supplier_details SET balance=$temp_balance WHERE supplier_name='$supplier' AND store_id=$ttstoreid");
...
jAlert('<?php echo  $msg; ?>');
			...
			if(isset($_GET['sid']))
			$id=$_GET['sid'];
                                $line = $db->queryUniqueObject("SELECT * FROM stock_entries WHERE stock_id='$id' AND store_id=$ttstoreid");
			?>
			<form name="form1" method="post" id="form1" action="">
				  ...
				  ?>
                    ...
                    <tr>
                      ...
                      <td>Address:</td>
                      <td><input name="address" placeholder="ENTER ADDRESS" type="text" value="<?php $quantity = $db->queryUniqueValue("SELECT supplier_address FROM supplier_details WHERE store_id=$ttstoreid AND supplier_name='".$line->stock_supplier_name."'"); echo $quantity; ?>" id="address" maxlength="200"  class="round default-width-input"  /></td>
                       
                      <td>contact:</td>
                      <td><input name="contact" placeholder="ENTER CONTACT" type="text" value="<?php $quantity = $db->queryUniqueValue("SELECT supplier_contact1 FROM supplier_details WHERE store_id=$ttstoreid AND  supplier_name='".$line->stock_supplier_name."'"); echo $quantity; ?>" id="contact1" maxlength="200"  class="round default-width-input" onkeypress="return numbersonly(event)" style="width:120px " /></td>
                  ...
                  <input type="hidden" id="edit_guid">
                         ...
                         <div style="overflow:auto ;max-height:300px;  ">
                           <table class="form" id="item_copy_final">
                               ...
                               <?php 
                               $sid=$line->stock_id;
				$max = $db->maxOf("count1", "stock_entries", "stock_id='$sid' AND store_id=$ttstoreid ");
		
				for($i=1; $i<=$max; $i++)
				{
                                              $line1 = $db->queryUniqueObject("SELECT * FROM stock_entries WHERE (store_id=$ttstoreid) AND (stock_id='$sid' and count1=$i)");
				
                                          $item= $db->queryUniqueValue("SELECT stock_id FROM stock_details WHERE store_id=$ttstoreid AND stock_name='".$line1->stock_name."'");  
                                            ?>
