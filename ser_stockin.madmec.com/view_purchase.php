           ...
           if(r){ 
  			...
  			success: function(data) {
    			window.location ='view_purchase.php';
                        ...
                        jAlert('Product Is Delete', 'MADMEC');
  			}
	});
            }
            return r;
...
<input name="dsubmit" type="button" value="Delete Selected" class="my_button round blue   text-upper" style="margin-left:5px;" onclick="return confirmDeleteSubmit()"/>
				...
				<table>
			<?php 
$SQL = "SELECT DISTINCT(stock_id) FROM  stock_entries where type='entry' AND store_id=$ttstoreid";
if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!="")
{
$SQL = "SELECT DISTINCT(stock_id) FROM  stock_entries WHERE (store_id=$ttstoreid) AND (stock_name LIKE '%".$_POST['searchtxt']."%' OR stock_supplier_name LIKE '%".$_POST['searchtxt']."%' OR stock_id LIKE '%".$_POST['searchtxt']."%' OR date LIKE '%".$_POST['searchtxt']."%' OR type LIKE '%".$_POST['searchtxt']."%') AND (type='entry')";
	...
	*/
	$query = "SELECT COUNT(DISTINCT stock_id) as num FROM $tbl_name where (type='entry') AND (store_id=$ttstoreid);";
if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!="")
{
$query = "SELECT COUNT(DISTINCT stock_id) as num FROM stock_entries WHERE (store_id=$ttstoreid) AND (stock_name LIKE '%".$_POST['searchtxt']."%' OR stock_supplier_name LIKE '%".$_POST['searchtxt']."%' OR stock_id LIKE '%".$_POST['searchtxt']."%' OR date LIKE '%".$_POST['searchtxt']."%' OR type LIKE '%".$_POST['searchtxt']."%') AND (type='entry');";
...
$page = $_GET['page'];
	...
	/* Get data. */
	$sql = "SELECT DISTINCT(stock_id) FROM stock_entries where (type='entry') AND (store_id=$ttstoreid) ORDER BY date desc LIMIT $start, $limit  ";

if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!="")
{
$sql = "SELECT DISTINCT(stock_id) FROM stock_entries WHERE (store_id=$ttstoreid) AND  (stock_name  LIKE '%".$_POST['searchtxt']."%' OR stock_supplier_name LIKE '%".$_POST['searchtxt']."%' OR stock_id LIKE '%".$_POST['searchtxt']."%' OR date LIKE '%".$_POST['searchtxt']."%' OR type LIKE '%".$_POST['searchtxt']."%') and (type='entry') ORDER BY date desc LIMIT $start, $limit";
...
while($row = mysql_fetch_array($result))
		...
		$line = $db->queryUniqueObject("SELECT * FROM stock_entries WHERE stock_id='$entryid' AND store_id=$ttstoreid");
