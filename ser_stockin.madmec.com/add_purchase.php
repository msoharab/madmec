...
if(isset($_POST['supplier']) and isset($_POST['stock_name'])){
		...
		$date = date( 'Y-m-d H:i:s', $selected_date );           
                                            ...
                                            {
                $count = $db->countOf("stock_avail", "name='$stock_name[$i]' AND store_id=$ttstoreid");
		if($count == 0)
		{
		$db->query("insert into stock_avail(name,quantity,store_id) values('$stock_name[$i]',$quty[$i],$ttstoreid)");
		//echo "<br><font color=green size=+1 >New Stock Entry Inserted !</font>" ;

		$db->query("insert into stock_details(stock_id,stock_name,stock_quatity,supplier_id,company_price,selling_price,store_id) values('$autoid','$stock_name[$i]',0,'$supplier',$cost[$i],$sell[$i],$ttstoreid)");
			  
		$db->query("INSERT INTO stock_entries(stock_id,stock_name, stock_supplier_name, quantity, company_price, selling_price, opening_stock, closing_stock, date, username, type, total, payment, balance, mode, description, due, subtotal,count1,billnumber,store_id) VALUES ( '$autoid1','$stock_name[$i]','$supplier',$quty[$i],$cost[$i],$sell[$i],0,$quty[$i],'$date','$username','entry',$total[$i],$payment,$balance,'$mode','$description','$due',$subtotal,$i+1,'$bill_no',$ttstoreid)"); 

		}

		else if($count==1) 
		{
				$amount = $db->queryUniqueValue("SELECT quantity FROM stock_avail WHERE name='$stock_name[$i]' AND store_id=$ttstoreid");
			$amount1 = $amount + $quty[$i];
			$db->execute("UPDATE stock_avail SET quantity=$amount1 WHERE name='$stock_name[$i]' AND store_id=$ttstoreid ");
		$db->query("INSERT INTO stock_entries(stock_id,stock_name,stock_supplier_name,quantity,company_price,selling_price,opening_stock,closing_stock,date,username,type,total,payment,balance,mode,description,due,subtotal,count1,billnumber,store_id) VALUES ('$autoid1','$stock_name[$i]','$supplier',$quty[$i],$cost[$i],$sell[$i],$amount,$amount1,'$date','$username','entry',$total[$i],$payment,$balance,'$mode','$description','$due',$subtotal,$i+1,'$bill_no','$ttstoreid')"); 
