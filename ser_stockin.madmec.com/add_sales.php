...
if(isset($_POST['payment'])){
				...
				$bill_no 	= "";
					...
					if($validated_data === false) {
						...
						$tax_dis=mysql_real_escape_string($_POST['tax_dis']);
                                                        $temp_balance = $db->queryUniqueValue("SELECT balance FROM customer_details WHERE customer_name='$customer' AND store_id='$ttstoreid'");
		...
		$count = $db->queryUniqueValue("SELECT quantity FROM stock_avail WHERE name='$name1' AND `store_id`='".$ttstoreid."'");
		...
		$db->query("insert into stock_sales (tax,tax_dis,discount,dis_amount,grand_total,transactionid,stock_name,selling_price,quantity,amount,date,username,customer_id,subtotal,payment,balance,due,mode,description,count1,billnumber,store_id) 
                            values($tax,'$tax_dis',$discount,$dis_amount,$payable,'$autoid','$name1',$rate,$quantity,$total,'$mysqldate','$username','$customer',$subtotal,$payment,$balance,'$due','$mode','$description',$i+1,'$bill_no','$ttstoreid')");

		$amount = $db->queryUniqueValue("SELECT quantity FROM stock_avail WHERE name='$name1' AND `store_id`='".$_SESSION['storeid']."'");
			$amount1 = $amount - $quantity;

		$db->query("insert into stock_entries (stock_id,stock_name,quantity,opening_stock,closing_stock,date,username,type,salesid,total,selling_price,count1,billnumber,store_id) values('$autoid','$name1',$quantity,$amount,$amount1,'$mysqldate','$username','sales','$autoid',$total,$rate,$i+1,'$bill_no','$ttstoreid')");
...
function numbersonly(e){
    ...
    document.getElementById('grand_total').value=parseFloat(document.getElementById('grand_total').value)+parseFloat(document.getElementById('MADMEC_total').value)-parseFloat(document.getElementById(id+'my_tot').value);
    document.getElementById('main_grand_total').value=parseFloat(document.getElementById('grand_total').value);
