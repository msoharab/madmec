<?php
  ...
  class DB
  {
   ...
   header("location:install.php?msg=$data");
    ...
    {
      return $this->queryUniqueValue("SELECT MAX(`$column`) FROM `$table` WHERE `store_id`='".$_SESSION['storeid']."'");
    }
    ...
    {
      return $this->queryUniqueValue("SELECT COUNT(*) FROM `$table` WHERE `store_id`='".$_SESSION['storeid']."'");
    }
