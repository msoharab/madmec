...
# Note that filters and validators are separate rule sets and method calls. There is a good reason for this. 
            $db->query("UPDATE store_details  SET log ='".$upload."',type='".$type."' WHERE id=$ttstoreid");
         ?>
...
<div id="header">
	...
	<div class="page-full-width cf">
		...
		<!--<a href="#" id="company-branding" class="fr"><img src="<?php if(isset($_SESSION['logo'])) { echo "upload/".$_SESSION['logo'];}else{ echo "upload/logo.png"; } ?>" alt="MadMec" /></a>-->
...
<?php
    ...
    $email=$_POST['email'];
    $pin=$_POST['pin'];
  if($db->query("UPDATE store_details  SET pin='".$pin."',city='".$city."',name='".$name."',email='".$email."',web='".$web."',address='".$address."',place='".$place."',phone='".$phone."' WHERE id=$ttstoreid "))
...
<div id="content">
	...
	<?php 
                $line = $db->queryUniqueObject("SELECT * FROM store_details WHERE id=$ttstoreid");
                ?>
	<table><tr><td>
			...
			<input type="submit" class="button round blue image-right ic-right-arrow" name="submit" value="Update" />
                                    </td><td><a href="dashboard.php" class="button blue round side-content">Dashboard</a></td></tr></table>
		</form>
