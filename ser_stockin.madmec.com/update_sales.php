...
function numbersonly(e){
    ...
    document.getElementById('MADMEC_total').value=document.getElementById(id+'to').value;
    ...
    document.getElementById('MADMEC_total').value="";
    ...
    main_total=document.getElementById('MADMEC_total').value;
    ...
    data1=parseFloat(document.getElementById('grand_total').value)+parseFloat(document.getElementById('MADMEC_total').value)-parseFloat(document.getElementById(id+'to').value);
...
//alert(parseFloat(document.getElementById('MADMEC_total').value));
balance_amount();
    document.getElementById(id+'my_tot').value=document.getElementById('MADMEC_total').value
    document.getElementById('quty').value="";
    document.getElementById('sell').value="";
    document.getElementById('stock').value="";
    document.getElementById('total').value="";
    ...
    function unique_check(){
       ...
       function total_amount(){
        ...
        document.getElementById('total').value=document.getElementById('sell').value * document.getElementById('quty').value
    document.getElementById('MADMEC_total').value=document.getElementById('total').value;
	...
	<div class="page-full-width cf">
		...
		$customer=mysql_real_escape_string($_POST['supplier']);
						...
						$contact=mysql_real_escape_string($_POST['contact']);			   
                                                     $count = $db->countOf("customer_details", "customer_name='$customer' AND store_id=$ttstoreid");
						if($count==0)
						{
                                                         $db->query("insert into customer_details(customer_name,customer_address,customer_contact1,store_id) values('$customer','$address','$contact','$ttstoreid')");   
                                                        }
		$payment=mysql_real_escape_string($_POST['payment']);
		$balance=mysql_real_escape_string($_POST['balance']);

			$newvalue =$balance;
			$oldvalue = $db->queryUniqueValue("SELECT balance FROM customer_details WHERE customer_name='$customer' AND store_id=$ttstoreid");
			$diff=$newvalue-$oldvalue;
			$temp_balance = (int) $temp_balance +  (int) $diff;
			$db->execute("UPDATE customer_details SET balance=$temp_balance WHERE customer_name='$customer' AND store_id=$ttstoreid");$selected_date=$_POST['due'];
		...
		$mode=mysql_real_escape_string($_POST['mode']);
			...
			$diff=$newvalue-$oldvalue;
			$temp_balance = (int) $temp_balance +  (int) $diff;
			$db->execute("UPDATE customer_details SET balance=$temp_balance WHERE customer_name='$customer' AND store_id=$ttstoreid");
		...
		$mysqldate = date( 'Y-m-d H:i:s', $selected_date );
		$username = $_SESSION['username'];
		$count = $db->queryUniqueValue("SELECT count(*) FROM stock_avail WHERE (name='$name1' and quantity >=$quantity) AND (store_id=$ttstoreid)");
		if($count == 1)
		{

		 $old_quantity= $db->queryUniqueValue("SELECT quantity FROM stock_sales WHERE (id='$autoid' and count1=$i) AND (store_id=$ttstoreid)");
		 $db->query("update stock_sales set tax=$tax,tax_dis='$tax_did', grand_total=$payable,discount=$discount,dis_amount=$dis_amount, stock_name='$name1',selling_price=$rate,quantity=$quantity,amount=$total,date='$mysqldate',username='$username',customer_id='$customer',subtotal=$subtotal,payment=$payment,balance=$balance,due='$due',mode='$mode',description='$description',billnumber='$billnumber' where id=$autoid AND store_id=$ttstoreid");
		$quantity_diff=$quantity-$old_quantity;
		$quantity=$quantity+$quantity_diff;
		$amount = $db->queryUniqueValue("SELECT quantity FROM stock_avail WHERE name='$name1' AND store_id=$ttstoreid");
			$amount1 = $amount - $quantity;

		 $db->query("update stock_entries set  stock_id='$autoid',stock_name='$name1',quantity=$quantity,opening_stock=$amount,closing_stock=$amount1,date='$mysqldate',username='$username',type='sales',salesid='$billnumber',total=$total,selling_price=$rate,billnumber='$billnumber' where (salesid='$autoid' and count1=$j) AND store_id=$ttstoreid");
...
<link rel="stylesheet"  href="dist/js/jquery.alerts.css" >
		...
		//echo "<script>window.open('add_sales_print.php?sid=$autoid','myNewWinsr','width=620,height=800,toolbar=0,menubar=no,status=no,resizable=yes,location=no,directories=no');</script>";
			...
			$line = $db->queryUniqueObject("SELECT * FROM stock_sales WHERE id='$id' AND  store_id=$ttstoreid");
			?>
			<form name="form1" method="post" id="form1" action="">
                      <input type="hidden" id="MADMEC_total" >
				...
				$max = $db->maxOf("count1", "stock_sales", "transactionid ='$sid' AND store_id=$ttstoreid");
		
				for($i=1; $i<=$max; $i++)
				{
                                              $line1 = $db->queryUniqueObject("SELECT * FROM stock_sales WHERE (transactionid ='$sid' and count1=$i) AND (store_id=$ttstoreid)");
				
                                          $item= $db->queryUniqueValue("SELECT transactionid  FROM stock_sales WHERE stock_name='".$line1->stock_name."' AND store_id=$ttstoreid");  
                                            ?>
